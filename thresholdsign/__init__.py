...
verify_smp_bundle for transferring or persisting an SMP together with
its root signature as one self-delimiting byte string.
"""
...
# decode_delta_report_bundle_archive_seal, no seal or root signature is
# checked and no state is kept, so verify_smp_bundle (which delegates to
# check_smp) remains the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
def encode_smp_bundle(bundle: SMPBundle) -> bytes:
    ...
    The encoding is, in order, the tag ``b"ts/smpb/v1"``, the
    length-prefixed integer ``VARINT(n)`` (the chain item count), the
    4-byte unsigned big-endian index count ``k`` followed by one
    canonical ``VARINT(i)`` per strictly increasing proven leaf
    position, the 4-byte unsigned big-endian seal count (which must
    equal ``k``) followed by one frame per disclosed seal — the 4-byte
    unsigned big-endian length ``len(E)`` followed by the raw bytes
    ``E = encode_delta_report_bundle_archive_seal(s)`` in the same
    order as the indices — the 4-byte unsigned big-endian companion
    count followed by the raw 32-byte companion digests in the proof's
    leaf-to-root, left-to-right order, and finally the same signature
    ...
    and the signer ids form a non-empty strictly increasing tuple of
    positive integers.
    ...
    signature structure (the exact bounds of :func:`check_smp`: a
    non-positive or over-64-bit ``n``, empty or non-strictly-increasing
    indices, an index outside ``0 <= i < n``, an ``s`` tuple that does
    not pair one-to-one with the indices, a structurally illegal
    nested seal, a companion that is not exactly 32 bytes, a companion
    count other than the unique one derived from ``n`` and ``i``, a
    non-positive ``R``, a negative ``z``, an empty or
    non-strictly-increasing signer id tuple) or an over-long frame
    raises ValueError — but the seals are not verified and the
    signature is not checked against the root:
    :func:`verify_smp_bundle` stays the way to verify a bundle
    ...
    for position, seal in enumerate(seals):
        ...
        if len(encoded_seal) > 0xFFFFFFFF:
            raise ValueError(
                f"smp bundle seal {position + 1} encoding too long"
            )
...
def decode_smp_bundle(blob: bytes) -> SMPBundle:
    ...
    Accepts only the single canonical form: the tag
    ``b"ts/smpb/v1"``, the length-prefixed integer ``n`` (a 4-byte
    unsigned big-endian length followed by its shortest unsigned
    big-endian value), the 4-byte non-zero index count ``k`` followed
    by one canonical ``VARINT(i)`` per non-empty, strictly increasing
    proven position with ``0 <= i < n < 2**64``, the 4-byte seal count
    (which must equal ``k``) followed by that many non-empty seal
    frames — a 4-byte non-zero length followed by bytes that
    :func:`decode_delta_report_bundle_archive_seal` accepts — the
    4-byte companion count followed by exactly that many raw 32-byte
    companion digests, where the count must be the unique one derived
    from ``n`` and the disclosed indices, and finally the signature
    frame exactly as an :class:`AuditProofBundle` uses: the
    length-prefixed integers ``R`` and ``z``, the 4-byte non-zero
    signer count and then exactly that many strictly increasing
    positive signer ids, with ``R > 0`` and ``z >= 0``. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero or
    over-long seal frame length, a non-canonical nested seal or
    integer (leading zero or over-long length), a non-positive or
    over-64-bit ``n``, empty indices, an index out of range or not
    strictly increasing, a seal count that does not equal the index
    count, a missing or extra companion relative to what ``n`` and the
    indices require, a companion that is not exactly 32 bytes, a zero
    ``R``, a zero signer count, a non-positive or non-increasing
    ...
    with :func:`decode_delta_report_bundle_archive_seal` (which
    verifies no bundle and checks no signature) and the root signature
    is not checked. A structurally legal bundle whose seals do not
    verify or whose signature does not sign the rebuilt root is
    returned normally, and :func:`verify_smp_bundle` reports it as
    ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    ...
    if index_count > count:
        raise ValueError("smp bundle index count must not exceed n")
    indices = []
    for position in range(index_count):
        index, offset = _read_varint(
            blob,
            offset,
            what=f"smp bundle index {position + 1}",
        )
    ...
    for position in range(seal_count):
        encoded_seal, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"smp bundle seal {position + 1}",
        )
    ...
    for _ in range(sibling_count):
        if offset + SMP_DIGEST_SIZE > len(blob):
            raise ValueError("truncated smp bundle companion digest")
        siblings.append(
    ...
    for _ in range(signer_count):
        signer_id, offset = _read_varint(
            blob, offset, what="smp bundle signer id"
        )
        if signer_id == 0:
            raise ValueError("smp bundle signer ids must be positive")
        if signer_ids and signer_id <= signer_ids[-1]:
            raise ValueError(
                "smp bundle signer ids must be strictly "
                "increasing and unique"
            )
        signer_ids.append(signer_id)
    if offset != len(blob):
...
def verify_smp_bundle(bundle: SMPBundle, key: SigningDKGResult) -> bool:
    ...
    is rebuilt from the companion digests, every disclosed seal is
    re-checked with
    :func:`verify_delta_report_bundle_archive_seal`, and the signature
    is verified on ``b"ds/r1" || U64(n) || root``. Returns ``True``
    only when all of them hold; a well-formed bundle with tampered
    seals, indices, companions or signature, or one presented under
    another key, returns ``False``.
