...
encode_hdsc_proof / decode_hdsc_proof, and the proof-plus-signature
...
# ---------------------------------------------------------------------------
# Canonical HDSC-proof bundle transport: a self-delimiting, byte-for-byte
# reproducible encoding of an HDSCProof together with the AggregateSignature
# on its b"ts/hdscp/r1" || U64(total) || root statement, for
# cross-implementation exchange and persistence. Decoding restores structure
# only — the nested proof goes through decode_hdsc_proof, its seals are not
# verified and no signature is checked, so verify_hdsc_proof_bundle remains
# the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
class HDSCProofBundle:
    """A multi-seal chain membership proof together with its root signature.

    The fields, in order, are ``proof`` (an :class:`HDSCProof`) and
    ``signature`` (the :class:`AggregateSignature` on the proof's
    ``b"ts/hdscp/r1" || U64(total) || root`` statement). The dataclass is
...
def encode_hdsc_proof_bundle(bundle: HDSCProofBundle) -> bytes:
    ...
    proof/signature field types, raise TypeError and illegal proof or
    signature structure (the exact bounds of :func:`encode_hdsc_proof`, a
    non-positive ``R``, a negative ``z``, an empty or
    non-strictly-increasing signer id tuple) or an over-long frame raises
    ValueError, exactly as :func:`check_hdsc_proof`'s structural checks do
    — but the seals are not verified and the signature is not checked
    against the root: :func:`verify_hdsc_proof_bundle` stays the way to
    verify a bundle afterwards. The output for a given bundle is unique
    and the encoding carries no network, storage or hidden state.
    """
...
def decode_hdsc_proof_bundle(blob: bytes) -> HDSCProofBundle:
    ...
    Accepts only the single canonical form: the tag ``b"ts/hdscpb/v1"``, a
    4-byte non-zero frame length followed by bytes that
    :func:`decode_hdsc_proof` accepts, the length-prefixed integers ``R``
    and ``z``, the 4-byte non-zero signer count ``k`` and then exactly
    ``k`` strictly increasing positive signer ids. A non-bytes argument
    raises TypeError; a wrong or missing tag, a zero or over-long proof
    frame length, a non-canonical nested proof, a zero ``R``, a
    non-canonical integer (leading zero or over-long length), a zero
    signer count, a non-positive or non-increasing signer id, truncation,
    or trailing bytes raises ValueError. A successfully decoded bundle
    re-encodes to exactly the input bytes.

    Decoding only restores the structure: the nested proof is decoded with
    :func:`decode_hdsc_proof` (which verifies no seal) and the signature
    is not checked. A structurally legal bundle whose seals do not verify
    or whose signature does not sign the root is returned normally, and
    ...
    for _ in range(signer_count):
        ...
        if signer_id == 0:
            raise ValueError(
                "hdsc proof bundle signer ids must be positive"
            )
        if signer_ids and signer_id <= signer_ids[-1]:
            raise ValueError(
...
def verify_hdsc_proof_bundle(
    ...
    This is a convenience wrapper over
    ``check_hdsc_proof(bundle.proof, bundle.signature, key)``: the Merkle
    root is rebuilt from the proof, every disclosed seal re-checked with
    :func:`verify_hds` and the signature verified on the existing
    ``b"ts/hdscp/r1" || U64(total) || root`` statement — no new signing
    message is introduced. Returns ``True`` only when all of them hold; a
    well-formed bundle with tampered seals, indices, siblings or
