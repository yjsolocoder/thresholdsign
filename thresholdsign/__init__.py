...
verify_history_multi_proof_bundle, and the single-seal
proof-plus-signature bundle SealHistoryProofBundle /
encode_history_proof_bundle / decode_history_proof_bundle /
verify_history_proof_bundle, publicly
...
# byte-for-byte reproducible encoding of a SealHistoryProof together with
# the AggregateSignature on its b"sh/r" || U64(total) || root statement, for
# cross-implementation exchange and persistence. Decoding restores structure
# only — the nested proof goes through decode_history_proof and no seal
# or signature is checked, so verify_history_proof_bundle remains the
# sole verifier afterwards.
# ---------------------------------------------------------------------------
...
def encode_history_proof_bundle(bundle: SealHistoryProofBundle) -> bytes:
    """Canonically encode a proof bundle for transport or persistence.

    The encoding is, in order, the tag ``b"ts/shpb/v1"``, the 4-byte
    unsigned big-endian length ``len(P)`` followed by
    ``P = encode_history_proof(bundle.proof)`` (never empty), and
    then the signature frame: ``VARINT(R)``, ``VARINT(z)``, the 4-byte
    unsigned big-endian signer count ``k`` and one ``VARINT(id)`` per
    ascending signer id. A ``VARINT`` is a 4-byte unsigned big-endian
    body length followed by the shortest unsigned big-endian value (zero
    ...
    Only a structurally legal :class:`SealHistoryProofBundle` is
    accepted — a non-bundle or non-:class:`SealHistoryProof` proof
    argument, or wrong proof/signature field types, raise TypeError and
    illegal proof or signature structure (the exact bounds of
    :func:`encode_history_proof`, a non-positive ``R``, a negative
    ``z``, an empty or non-strictly-increasing signer id tuple) or an
    over-long frame raises ValueError, exactly as
    :func:`check_history_proof`'s structural checks do — but the
    seal is not verified and the signature is not checked against the
    root: :func:`verify_history_proof_bundle` stays the way to
    verify a bundle afterwards. The output for a given bundle is unique
    and the encoding carries no network, storage or hidden state.
    """
    if not isinstance(bundle, SealHistoryProofBundle):
        raise TypeError(
            "bundle must be a SealHistoryProofBundle instance"
        )
    if not isinstance(bundle.proof, SealHistoryProof):
        raise TypeError(
            "bundle.proof must be a SealHistoryProof instance"
        )
...
def decode_history_proof_bundle(blob: bytes) -> SealHistoryProofBundle:
    ...
    4-byte non-zero frame length followed by bytes that
    :func:`decode_history_proof` accepts, the length-prefixed
    integers ``R`` and ``z``, the 4-byte non-zero signer count ``k`` and
    then exactly ``k`` strictly increasing positive signer ids. A
    non-bytes argument raises TypeError; a wrong or missing tag, a zero
    or over-long proof frame length, a non-canonical nested proof, a zero
    ...
    Decoding only restores the structure: the nested proof is decoded
    with :func:`decode_history_proof` (which verifies no seal) and
    the signature is not checked. A structurally legal bundle whose seal
    does not verify or whose signature does not sign the root is returned
    normally, and :func:`verify_history_proof_bundle` reports it as
    ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    ...
    if R == 0:
        raise ValueError(
            "seal history proof bundle signature R must be positive"
        )

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated seal history proof bundle signer count"
        )
    signer_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if signer_count == 0:
        raise ValueError(
            "seal history proof bundle must name at least one signer"
        )
    ...
    for _ in range(signer_count):
        ...
        if signer_id == 0:
            raise ValueError(
                "seal history proof bundle signer ids must be positive"
            )
        if signer_ids and signer_id <= signer_ids[-1]:
            raise ValueError(
    ...
    if offset != len(blob):
        raise ValueError(
            "trailing bytes after seal history proof bundle"
        )
    ...
    if encode_history_proof_bundle(bundle) != blob:
        raise ValueError(
            "non-canonical seal history proof bundle encoding"
        )
    return bundle
...
def verify_history_proof_bundle(
    ...
    This is a convenience wrapper over
    ``check_history_proof(bundle.proof, bundle.signature, key)``:
    the proven seal is re-checked with :func:`verify_seal`, the
    Merkle root is rebuilt from the proof and the signature verified on
    ``b"sh/r" || U64(total) || root``. Returns ``True`` only when all of
    them hold; a well-formed bundle with a tampered seal, index,
    siblings or signature, or one presented under another key, returns
    ``False``.

    A non-:class:`SealHistoryProofBundle` argument raises
    TypeError; illegal nested proof, signature or key structure raises
    TypeError/ValueError, exactly as
    :func:`check_history_proof` does.
    """
    if not isinstance(bundle, SealHistoryProofBundle):
        raise TypeError(
            "bundle must be a SealHistoryProofBundle instance"
