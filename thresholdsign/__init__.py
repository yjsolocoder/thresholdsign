...
proof-plus-two-signatures bundle AuditExtensionProofBundle /
...
# ---------------------------------------------------------------------------
# Canonical audit-extension proof bundle transport: a self-delimiting,
# byte-for-byte reproducible encoding of an AuditExtensionProof together with
# the AggregateSignature pair on its old and new root statements, for
# cross-implementation exchange and persistence. Decoding restores structure
# only — the nested proof goes through decode_extension, its leaf digests
# stay opaque and no signature is checked, so
# verify_audit_extension_proof_bundle remains the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
class AuditExtensionProofBundle:
    """A consistency proof together with its old and new root signatures.

    The fields, in order, are ``proof`` (an :class:`AuditExtensionProof`),
    ``old_signature`` (the :class:`AggregateSignature` on the proof's
    ``b"am/r" || U64(old_n) || old_root`` statement) and ``new_signature``
    (the :class:`AggregateSignature` on the ``b"am/r" || U64(n) ||
    new_root`` statement). The dataclass is frozen, positionally
    ...
    time — :func:`encode_audit_extension_proof_bundle` checks the structure
    and :func:`verify_audit_extension_proof_bundle` is the way to test a
    bundle afterwards.
    """
...
def encode_audit_extension_proof_bundle(
    ...
    """Canonically encode an audit extension proof bundle for transport.

    The encoding is, in order, the tag ``b"ts/aepb/v1"``, the 4-byte
    unsigned big-endian length ``len(P)`` followed by
    ``P = encode_extension(bundle.proof)`` (never empty), and then the old
    and new signature frames, each exactly the :class:`AuditProofBundle`
    signature frame: ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned
    big-endian signer count ``k`` and one ``VARINT(id)`` per ascending
    signer id. A ``VARINT`` is a 4-byte unsigned big-endian body length
    followed by the shortest unsigned big-endian value (zero is the single
    ...
    empty or non-strictly-increasing signer id tuple) or an over-long frame
    raises ValueError, exactly as :func:`check_extension`'s structural
    checks do — but the leaf digests are not parsed, the prefix relation is
    not checked and neither signature is verified against its root:
    ...
    if not isinstance(bundle, AuditExtensionProofBundle):
        raise TypeError(
            "bundle must be an AuditExtensionProofBundle instance"
        )
    if not isinstance(bundle.proof, AuditExtensionProof):
        raise TypeError(
            "bundle.proof must be an AuditExtensionProof instance"
        )
    if not isinstance(bundle.old_signature, AggregateSignature):
        raise TypeError(
            "bundle.old_signature must be an AggregateSignature instance"
        )
    if not isinstance(bundle.new_signature, AggregateSignature):
        raise TypeError(
            "bundle.new_signature must be an AggregateSignature instance"
        )
...
def _read_extension_bundle_signature(
    blob: bytes, offset: int, *, which: str
) -> tuple[AggregateSignature, int]:
    """Read one bundle signature frame at ``offset``, returning ``(signature, next)``.

    The frame is exactly the :func:`decode_audit_proof_bundle` signature
    frame: the length-prefixed integers ``R`` (non-zero) and ``z``, the
    4-byte non-zero signer count ``k`` and then exactly ``k`` strictly
    increasing positive signer ids.
    """
    R, offset = _read_varint(
        blob,
        offset,
        what=f"audit extension proof bundle {which} signature R",
    )
    z, offset = _read_varint(
        blob,
        offset,
        what=f"audit extension proof bundle {which} signature z",
    )
    if R == 0:
        raise ValueError(
            f"audit extension proof bundle {which} signature R must be "
            "positive"
        )

    if offset + 4 > len(blob):
        raise ValueError(
            f"truncated audit extension proof bundle {which} signer count"
        )
    signer_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if signer_count == 0:
        raise ValueError(
            f"audit extension proof bundle {which} signature must name at "
            "least one signer"
        )
    ...
    for _ in range(signer_count):
        signer_id, offset = _read_varint(
            blob,
            offset,
            what=f"audit extension proof bundle {which} signer id",
        )
        if signer_id == 0:
            raise ValueError(
                f"audit extension proof bundle {which} signer ids must be "
                "positive"
            )
        if signer_ids and signer_id <= signer_ids[-1]:
            raise ValueError(
...
def decode_audit_extension_proof_bundle(
    ...
    Decoding only restores the structure: the nested proof is decoded with
    :func:`decode_extension` (which keeps the leaf digests opaque and
    verifies nothing), the prefix relation is not checked and neither
    signature is verified. A structurally legal bundle whose signatures do
    not match its roots is returned normally, and
    ...
    if offset + 4 > len(blob):
        raise ValueError(
            "truncated audit extension proof bundle proof length"
        )
    proof_length = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if proof_length == 0:
        raise ValueError(
            "audit extension proof bundle proof must be non-empty"
        )
    if offset + proof_length > len(blob):
        raise ValueError("truncated audit extension proof bundle proof")
    encoded_proof = bytes(blob[offset:offset + proof_length])
    offset += proof_length
    proof = decode_extension(encoded_proof)

    old_signature, offset = _read_extension_bundle_signature(
        blob, offset, which="old"
    )
    new_signature, offset = _read_extension_bundle_signature(
        blob, offset, which="new"
    )
    if offset != len(blob):
    ...
    if encode_audit_extension_proof_bundle(bundle) != blob:
        raise ValueError(
            "non-canonical audit extension proof bundle encoding"
        )
    return bundle
...
def verify_audit_extension_proof_bundle(
    ...
    the proof's leaf digests and each signature is verified on its
    ``b"am/r" || U64(n) || root`` statement under ``key``. Returns
    ``True`` only when both hold; a well-formed bundle with tampered
    leaves or ``old_n``, a swapped or mismatched signature pair, or one
    presented under another key returns ``False``.
