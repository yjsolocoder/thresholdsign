...
transport and archive reassembly, the multi-cut batch form of the same
pair partition_history_delta / join_history_delta_segments for splitting
a long chain into several consecutive hop segments at once and folding an
ordered tuple of segments back into one chain, and the self-delimiting
segment-set transport encoding encode_history_delta_segments /
decode_history_delta_segments for archiving or transferring several
consecutive segments as one ordered object, and the
...
# ---------------------------------------------------------------------------
# Batch partitioning and ordered reassembly of incremental seal-history
# extension chains: the multi-cut form of slice_history_delta /
# concatenate_history_delta. partition_history_delta splits a long delta
# chain into several consecutive hop segments at once and
# join_history_delta_segments folds an ordered tuple of segments back into
# one chain; both work on expanded bundle chains and compact the result
# back into delta form, neither verifies any signature and neither keeps
# hidden state.
# ---------------------------------------------------------------------------
...
def partition_history_delta(
    ...
    ``cuts`` is a tuple of boundary indices into the expanded chain's
    ``items`` sequence: cut ``c`` ends one segment just before expanded
    hop ``c`` and starts the next segment at it, exactly as repeated
    :func:`slice_history_delta` calls over the intervals
    ``[0, cuts[0])``, ``[cuts[0], cuts[1])``, ...,
    ``[cuts[-1], len(items))`` would. The cuts must be strictly
    increasing (hence unique) and each must lie in
    ``1..len(items) - 1``, so every segment is non-empty; an empty
    ``cuts`` tuple returns the whole chain as the single segment. The
    segments are returned in chain order as a tuple of
    :class:`SealHistoryExtensionDeltaChain` values, each the delta form
    of its expanded interval. No signature is verified here and no state
    is kept — verification stays with :func:`verify_history_delta` on
    each segment, and :func:`join_history_delta_segments` folds the
    ...
    does any structural error the expanded chain or its nested proofs or
    signatures would raise on their own.
    """
    expanded = expand_history_delta(chain)
    if not isinstance(cuts, tuple):
    ...
    for cut in cuts:
        ...
        if cut < 1 or cut > hop_count - 1:
            raise ValueError(
                "each cut must lie in 1..len(items) - 1"
            )
        if cut <= previous:
            raise ValueError("cuts must be strictly increasing and unique")
        previous = cut
...
def join_history_delta_segments(
    ...
    are joined left to right by repeated :func:`concatenate_history_delta`
    calls, so each seam must satisfy the same two link conditions as that
    pairwise splice: the left segment's last expanded hop's leaves must
    equal the old prefix of the right segment's first expanded hop (a gap
    or overlap is rejected), and the two shared checkpoint signatures at
    the seam must be identical by value, kept exactly once in the result.
    A single segment is returned as the chain itself. In particular,
    joining the tuple produced by :func:`partition_history_delta` for any
    legal cuts restores the original chain value by value, and its
    canonical encoding matches the original byte for byte. No signature
    is verified here and no state is kept — verification stays with
    :func:`verify_history_delta` on the result.

    A non-tuple ``segments`` argument or a
    non-:class:`SealHistoryExtensionDeltaChain` element raises TypeError,
    as does any wrong field or element type inside a segment, exactly as
    :func:`expand_history_delta` does. An empty ``segments`` tuple raises
    ValueError, as does any leaf prefix or shared signature mismatch at a
    seam and any structural error a segment or its nested proofs or
    signatures would raise on its own.
    """
    if not isinstance(segments, tuple):
        raise TypeError("segments must be a tuple")
    for segment in segments:
...
# ---------------------------------------------------------------------------
# Canonical segment-set transport for incremental seal-history extension
# chains: a self-delimiting, byte-for-byte reproducible encoding of an
# ordered tuple of SealHistoryExtensionDeltaChain values, so several
# consecutive segments can be archived or cross-implementation transferred
# as a single object. Like the single-chain codec this restores structure
# only — no signature is checked, seams are not inspected and no state is
# kept — and join_history_delta_segments remains the sole place that
# judges order continuity and the seams between neighbours.
# ---------------------------------------------------------------------------

HISTORY_DELTA_SEGMENTS_WIRE_TAG = b"thresholdsign/history-delta-segments/v1"
...
def encode_history_delta_segments(
    ...
    Only the tuple container and the structural legality of each segment
    (via its own canonical encoder) are checked — the segments are not
    sorted, no seam between neighbours is inspected and no signature is
    checked: :func:`join_history_delta_segments` stays the way to judge
    order continuity and the seams, and :func:`verify_history_delta` the
    way to verify the joined result afterwards. A non-tuple ``segments``
    argument or a non-:class:`SealHistoryExtensionDeltaChain` element
    raises TypeError, as does any wrong field or element type inside a
    segment, exactly as :func:`encode_history_delta` does. An empty
    tuple, an over-long count or frame, or any structural error a segment
    or its nested proofs or signatures would raise on its own raises
    ...
    if segment_count == 0:
        raise ValueError("segments must be a non-empty tuple")
    if segment_count > 0xFFFFFFFF:
        raise ValueError("too many history delta segments")

    frames = []
    for index, segment in enumerate(segments):
        frame = encode_history_delta(segment)
        if len(frame) > 0xFFFFFFFF:
            raise ValueError(f"history delta segment {index + 1} too long")
        frames.append(frame)
...
def decode_history_delta_segments(
    ...
    afterwards. A non-bytes argument raises TypeError; a wrong or missing
    tag, a zero segment count, a zero or over-long frame length, a count
    mismatch, truncation, trailing bytes, or a frame whose nested chain
    encoding is illegal or non-canonical (including a frame that would
    not re-encode byte for byte) raises ValueError. A successfully
    decoded tuple re-encodes to exactly the input bytes.
    ...
    if not blob.startswith(HISTORY_DELTA_SEGMENTS_WIRE_TAG):
        raise ValueError("bad history delta segments tag")
    offset = len(HISTORY_DELTA_SEGMENTS_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError("truncated history delta segments count")
    segment_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if segment_count == 0:
        raise ValueError("history delta segments must be non-empty")
    ...
    for index in range(segment_count):
        frame, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"history delta segment {index + 1}",
        )
        segments.append(decode_history_delta(frame))
    if offset != len(blob):
        raise ValueError("trailing bytes after history delta segments")

    segments_tuple = tuple(segments)
    if encode_history_delta_segments(segments_tuple) != blob:
        raise ValueError("non-canonical history delta segments encoding")
    return segments_tuple
