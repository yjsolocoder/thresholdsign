...
archive SMPBundleArchive of whole bundles authenticated by one more
threshold Schnorr signature over the digest of a canonical framing of
the existing per-bundle encodings and the verifying public key, with
smp_bundle_archive_message and the key-bound verifier
verify_smp_bundle_archive that re-checks every bundle before the outer
signature.
"""
...
# SMPBundle values authenticated by one threshold Schnorr signature. The
# signed message commits to the verifying public key and to the digest of a
# canonical framing C over the existing per-bundle transport encodings, so
# deleting, inserting, reordering or substituting a bundle — or presenting
# the archive under another key — invalidates the signature. The library
# keeps no state: the archive is a plain value and every bundle is re-checked
# on verification before the outer signature is examined.
# ---------------------------------------------------------------------------

SMP_BUNDLE_ARCHIVE_MESSAGE_TAG = b"ts/smpba/m1"


@dataclass(frozen=True)
class SMPBundleArchive:
    """A non-empty ordered batch of SMP bundles with one outer signature.
    ...
    compared by value; the items keep their order and the archive
    carries no network, storage or hidden state. Neither field is
    checked at construction time — :func:`smp_bundle_archive_message`
    requires structurally legal items and
    :func:`verify_smp_bundle_archive` is the way to test an archive
    against a key afterwards.
    """
...
def smp_bundle_archive_message(
    ...
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). ``C`` is, in order,
    the 4-byte unsigned big-endian item count ``n`` followed by, for
    every bundle in tuple order, the 4-byte unsigned big-endian byte
    length and the exact bytes ``E_i = encode_smp_bundle(items[i])``;
    every U32 is a 4-byte unsigned big-endian integer and ``E_i`` is the
    existing canonical encoding of the bundle. The message carries no
    signature and keeps no state.

    ``items`` must be a non-empty tuple of structurally legal
    :class:`SMPBundle` values exactly as :func:`encode_smp_bundle`
    requires and ``public_key`` a non-boolean non-negative integer.
    Wrong field or nested field types raise TypeError; an empty items
    tuple, an illegal nested bundle, a negative public key or an
    over-long encoding raises ValueError.
    """
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    for item in items:
    ...
    for encoded in frames:
        ...
        framing += encoded
    return (
        SMP_BUNDLE_ARCHIVE_MESSAGE_TAG
        + hashlib.sha256(bytes(framing)).digest()
        + _encode_varint(public_key)
    )
...
def verify_smp_bundle_archive(
    ...
    order, so its disclosed seals and its own root signature must all
    verify under ``key``; the canonical
    :func:`smp_bundle_archive_message` of the items and
    ``key.public_key`` is finally checked as the outer signature's
    threshold Schnorr message via :func:`verify_signature` with the
    ...
    or presenting the archive under another key — returns ``False``
    rather than raising. The function is stateless.
    """
    ...
    # exactly as the single-bundle verifier validates its bundle.
