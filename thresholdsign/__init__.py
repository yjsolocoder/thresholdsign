...
the full history, plus the compact multi-seal proofs
SealHistoryMultiProof / make_history_multi_proof /
check_history_multi_proof that certify several ordered seal positions
with the one root signature. Publicly
...
# ---------------------------------------------------------------------------
# Compact multi-seal inclusion proofs: one root signature certifies that
# several disclosed seals belong to the same ordered, indexed seal history.
# The tree is exactly make_history_proof's tree; a single
# SealHistoryMultiProof carries only the siblings that are not themselves
# disclosed seals (or odd-tail self-pairs), so the seals can be located in
# the sealed history from one signature without the history itself and with
# no hidden state.
# ---------------------------------------------------------------------------
...
class SealHistoryMultiProof:
    """A compact Merkle inclusion proof for several seals at once.
    ...
    empty when no companion digests are needed). The dataclass is frozen,
    ...
    checked at construction time — :func:`check_history_multi_proof` is the
    way to test a proof afterwards.
    """
...
def make_history_multi_proof(
    ...
    tree. Returns ``(message, proof)`` where ``message`` is
    ``b"sh/r" || U64(total) || root`` — the bytes to be threshold-signed —
    and ``proof`` is the :class:`SealHistoryMultiProof` whose ``seals`` pair
    up one to one with ``indices``. Its ``siblings`` are collected level by
    level, in ascending level order (leaves upward), one entry per needed
    companion: at a level the proven positions are paired left to right;
    when the companion position is itself a proven node carried in the
    proof, or the node is the last member of an odd-width level (which the
    tree pairs with itself), no sibling is appended; otherwise the
    companion digest is, exactly once. Proven nodes from lower levels feed
    the level above exactly as in the tree, so no digest is sent twice.

    Wrong argument types raise TypeError: a non-:class:`SealHistory`
    history, a non-tuple index sequence, or a non-integer (including
    boolean) index. An empty history, a structurally illegal item seal, a
    count that does not fit ``0 < total < 2**64``, an empty index tuple,
...
def _required_history_multi_proof_siblings(
    ...
    """Number of 32-byte companion digests a multi-proof for ``total`` leaves must carry.
...
def _validate_history_multi_proof_structure(
    ...
    if not isinstance(proof, SealHistoryMultiProof):
        raise TypeError("proof must be a SealHistoryMultiProof instance")
    indices = proof.indices
    total = proof.total
    seals = proof.seals
    siblings = proof.siblings
...
def check_history_multi_proof(
    ...
    Every disclosed seal's leaf digest is recomputed from its position and
    the seal exactly as in :func:`make_history_proof`. The root is then
    rebuilt level by level while consuming ``proof.siblings`` in ascending
    level order: the current level holds the digests of the proven nodes at
    their current positions, paired left to right; when a companion is
    itself a current-level node its digest is used directly, when the node
    is the last member of an odd-width level it is paired with itself, and
    otherwise the next 32-byte entry of ``siblings`` is consumed. The
    current width contracts as ``(width + 1) // 2``. Every embedded seal is
    re-checked with :func:`verify_seal` against ``key`` (in index order),
    and the statement ``b"sh/r" || U64(total) || root`` is checked as
    ``signature``'s threshold Schnorr message via :func:`verify_signature`.
    Returns ``True`` only when the rebuild consumes every sibling exactly
    and reaches the single signed root, every seal verifies for the key,
    and the signature verifies; a well-formed proof whose seals, indices,
    siblings, root or signature was tampered with — seals swapped or
    altered, a missing, extra or misplaced sibling — or which is presented
    under another key, returns ``False``.
    ...
    non-:class:`ReportSeal` seal, a non-bytes sibling) raises TypeError; a
    ...
    structurally illegal seal, signature or key raises ValueError, exactly
    as :func:`verify_seal` and :func:`verify_signature` would.
    """
    ...
    while width > 1:
        ...
        for position in sorted(nodes):
            ...
            if width % 2 == 1 and position == width - 1:
                # Odd tail with no companion: pair the node with itself.
                next_nodes[parent] = _history_proof_node(nodes[position], nodes[position])
            elif (position ^ 1) in nodes:
                left = position if position % 2 == 0 else position ^ 1
                next_nodes[parent] = _history_proof_node(nodes[left], nodes[left ^ 1])
            else:
                try:
                    sibling = next(pending)
                except StopIteration:
                    raise ValueError("proof.siblings is missing entries") from None
                if position % 2 == 0:
                    next_nodes[parent] = _history_proof_node(nodes[position], sibling)
                else:
                    next_nodes[parent] = _history_proof_node(sibling, nodes[position])
        nodes = next_nodes
        width = (width + 1) // 2
    ...
    for seal in seals:
        if not verify_seal(seal, key):
            return False
    signed_message = SEAL_HISTORY_PROOF_ROOT_TAG + _history_proof_u64(total) + nodes[0]
    return verify_signature(
        signed_message,
        signature,
        public_key,
