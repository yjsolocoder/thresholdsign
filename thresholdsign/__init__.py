...
def decode_hbapb(blob: bytes) -> HBAPB:
    ...
    for position in range(bundle_count):
        if offset + 4 > len(blob):
            raise ValueError(
                "truncated HAMPB archive proof bundle "
                f"{position + 1} length"
            )
        bundle_length = int.from_bytes(blob[offset:offset + 4], "big")
        offset += 4
        if bundle_length == 0:
            raise ValueError(
                f"HAMPB archive proof bundle {position + 1} must be "
                "non-empty"
            )
        if offset + bundle_length > len(blob):
            raise ValueError(
                f"truncated HAMPB archive proof bundle {position + 1}"
            )
        encoded_bundle = bytes(blob[offset:offset + bundle_length])
...
def decode_hbapba(blob: bytes) -> HBAPBArchive:
    ...
    for index in range(item_count):
        if offset + 4 > len(blob):
            raise ValueError(
                f"truncated HBAPB archive item {index + 1} length"
            )
        frame_length = int.from_bytes(blob[offset:offset + 4], "big")
        offset += 4
        if frame_length == 0:
            raise ValueError(
                f"HBAPB archive item {index + 1} must be non-empty"
            )
        if offset + frame_length > len(blob):
            raise ValueError(
                f"truncated HBAPB archive item {index + 1}"
            )
        frame = bytes(blob[offset:offset + frame_length])
        offset += frame_length
