...
string, and the compact multi-bundle archive inclusion proofs
...
non-empty order-preserving archive HDSCArchiveProofBundleArchive of such
bundles, whose outer signature binds hdscapa_message over the existing
canonical bundle encodings and the verifying public key, with the
key-bound verifier verify_hdscapa that re-checks every bundle with
verify_hdsc_archive_proof_bundle before the outer verify_signature, and
the canonical archive transport encoding encode_hdscapa /
decode_hdscapa for transferring or persisting a whole archive as one
...
def _read_archive_block(
    stream: bytes, offset: int, *, what: str
) -> tuple[bytes, int]:
    """Read one 4-byte-length-prefixed frame body at ``offset``.

    The same wire rules as :func:`_read_audit_proof_block` with a
    non-empty body, but the error messages are phrased purely in the
    caller's ``what`` terms.
    """
    if offset + 4 > len(stream):
...
# ---------------------------------------------------------------------------
# Compact multi-bundle inclusion proofs over a non-empty order-preserving
# archive of whole HDSC proof bundles: one root signature certifies that
# several disclosed HDSCProofBundle values belong, at fixed positions, to
# the exact non-empty bundle tuple an HDSCProofBundleArchive carries,
# without presenting the whole archive. The tree is SHA256 with its own
# domain-separated leaf/node prefixes over the existing canonical bundle
# encodings, exactly isomorphic to the HDSC proof tree; the root
# statement to sign is ``b"ts/hdscba/r1" || U64(total) || root``. Only
# the companion digests that are not themselves disclosed are carried
# (and never the odd-tail self-pair), so arbitrary index subsets share
# one root signature. check_hdsc_archive_proof re-runs
# verify_hdsc_proof_bundle on every disclosed bundle (and, through it,
# every seal that bundle discloses) and verify_signature on the root
# signature afterwards. No transport encoding and no self-signed bundle
...
HDSC_ARCHIVE_PROOF_DIGEST_SIZE = 32  # SHA256 width; every tree node is this wide
...
def _hdsc_archive_proof_leaf(
    index: int, bundle: HDSCProofBundle
) -> bytes:
    """The index-bound leaf digest ``H(b"ts/hdscba/l1" || U64(i) || H(encode_hdsc_proof_bundle(bundle)))``."""
    return hashlib.sha256(
...
def _check_hdsc_archive_items(
    ...
    to ``items``: a tuple of :class:`HDSCProofBundle` instances,
...
class HDSCArchiveProof:
    ...
    (the proven :class:`HDSCProofBundle` values, one per entry of
    ``indices``, in the same order) and ``siblings`` (the 32-byte sibling
    digests consumed from the leaf level up to the root, in ascending
    level order and left to right within a level; the tuple is empty when
    no companion digests are needed). At a level whose width is odd, the
    ...
    constructible and compared by value, and carries no network, storage
    or hidden state. Field types and bounds are not checked at
    construction time — :func:`check_hdsc_archive_proof` is the way to
    test a proof afterwards.
    """
...
def make_hdsc_archive_proof(
    ...
    ``archive`` must be an :class:`HDSCProofBundleArchive` whose items
    are a non-empty tuple of structurally legal
    :class:`HDSCProofBundle` values exactly as
    :func:`encode_hdsc_proof_bundle` requires, and ``indices`` a
    ...
    in archive order with SHA256, exactly isomorphic to the HDSC proof
    tree: leaves are ``H(b"ts/hdscba/l1" || U64(i) ||
    H(encode_hdsc_proof_bundle(bundle)))`` and internal nodes
    ``H(b"ts/hdscba/n1" || left || right)``; a level with an odd tail
    width duplicates its last node for pairing, and positions and counts
    are encoded as 8-byte unsigned big-endian integers. Returns
    ``(message, proof)`` where ``message`` is
    ...
    ``proof`` is the :class:`HDSCArchiveProof` whose ``bundles`` pair one
    to one with ``indices``. Its ``siblings`` are collected level by
    level, in ascending level order (leaves upward) and left to right
    within a level, one entry per needed companion: when the companion
    position is itself a proven node carried in the proof, or the node is
    the last member of an odd-width level (paired with itself), no
    sibling is appended; otherwise the companion digest is. Proven nodes
    from lower levels feed the level above exactly as in the tree, so no
    digest is sent twice. One threshold signature over the statement
    covers every index subset of the same archive.

    Wrong argument types raise TypeError: a
    non-:class:`HDSCProofBundleArchive` archive, a non-tuple index
    sequence or a non-integer (including boolean) index. An empty archive
    or index tuple, a structurally illegal nested bundle, a count at or
    above ``2**64``, indices that are not strictly increasing and unique,
    or an index outside ``0 <= i < total`` raises ValueError.
    """
    if not isinstance(archive, HDSCProofBundleArchive):
        raise TypeError(
            "archive must be an HDSCProofBundleArchive instance"
    ...
    if not isinstance(indices, tuple):
        raise TypeError("indices must be a tuple")
    items = _check_hdsc_archive_items(archive.items)
    total = len(items)
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("too many HDSC proof bundle archive items")
    if len(indices) == 0:
    ...
    levels = _hdsc_archive_proof_levels(items)
...
def _validate_hdsc_archive_proof_structure(
    ...
    siblings tuple whose entries are exactly 32 bytes. Wrong field types
    raise TypeError; illegal bounds or shapes raise ValueError. The two
    count couplings — bundles pairing one-to-one with indices and the
    sibling count the compact walk derives from ``total`` and
    ``indices`` — are not judged here: :func:`check_hdsc_archive_proof`
    reports either mismatch as ``False``.
    """
...
def check_hdsc_archive_proof(
    ...
    each bundle's inner proof and every seal it discloses must verify,
    and the statement ``b"ts/hdscba/r1" || U64(total) || root`` is
    finally checked as ``signature``'s threshold Schnorr message via
    :func:`verify_signature` with the key's group parameters. Returns
    ``True`` only when the rebuild consumes every sibling exactly and
    ...
    signed_message = (
        HDSC_ARCHIVE_PROOF_ROOT_TAG
        + _hdsc_archive_proof_u64(total)
        + nodes[0]
    )
    return verify_signature(
        signed_message,
...
# ---------------------------------------------------------------------------
# Non-empty order-preserving archives of whole HDSC archive proof bundles,
# authenticated by one outer threshold-Schnorr signature. The signed message
# commits to the verifying public key and to the SHA256 digest of the exact
# canonical framing of the bundle sequence, so deleting, inserting,
# reordering or substituting a bundle — or presenting the archive under
# another key — all invalidate the outer signature. The library keeps no
# state: the archive is a plain frozen value, every bundle is re-checked on
# verification, and encoding restores structure only.
# ---------------------------------------------------------------------------

HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_TAG = b"ts/hdscapa/m1"
HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_WIRE_TAG = b"ts/hdscapa/w1"


@dataclass(frozen=True)
class HDSCArchiveProofBundleArchive:
    """A non-empty ordered archive of HDSC archive proof bundles with one
    outer signature.

    The fields, in order, are ``items`` — a non-empty tuple of
    :class:`HDSCArchiveProofBundle` values in archive order — and
    ``signature`` — the threshold Schnorr :class:`AggregateSignature` on
    :func:`hdscapa_message` of the items and the verifying public key.
    The dataclass is frozen, positionally constructible and compared by
    value; neither field has a default, the items keep their order and
    the archive carries no network, storage or hidden state. Neither
    field is checked at construction time —
    :func:`hdscapa_message` requires structurally legal items and
    :func:`verify_hdscapa` is the way to test an archive against a key
    afterwards.
    """

    items: tuple[HDSCArchiveProofBundle, ...]
    signature: AggregateSignature


def hdscapa_message(
    items: tuple[HDSCArchiveProofBundle, ...], public_key: int
) -> bytes:
    """Encode the canonical message the threshold key signs for an archive.

    The message is, in order, the tag ``b"ts/hdscapa/m1"``, the 32-byte
    ``SHA256`` digest of the canonical framing ``C`` and
    ``V(public_key)`` — a 4-byte unsigned big-endian length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). With
    ``E_i = encode_hdsc_archive_proof_bundle(items[i])`` and ``n`` the
    item count, ``C`` is built in tuple order as
    ``U32(n) || Σ(U32(len(E_i)) || E_i)``; every ``U32`` is a 4-byte
    unsigned big-endian integer and ``E_i`` is the existing canonical
    transport encoding of the bundle. The message carries no signature
    and keeps no state.
    ...
    :func:`encode_hdsc_archive_proof_bundle` requires and
    ``public_key`` a non-boolean non-negative integer. Wrong field or
    nested field types raise TypeError — a non-tuple items sequence, an
    item that is not an :class:`HDSCArchiveProofBundle`, or a
    non-integer (including boolean) public key; an empty items tuple,
    an illegal nested bundle, a negative public key or an over-long
    encoding raises ValueError.
    """
    ...
    for item in items:
        if not isinstance(item, HDSCArchiveProofBundle):
            raise TypeError(
                "each archive item must be an "
                "HDSCArchiveProofBundle instance"
            )
    if not isinstance(public_key, int) or isinstance(public_key, bool):
        raise TypeError("public_key must be an integer")
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HDSC archive proof bundle archive item "
                f"{index + 1} encoding too long"
            )
        frames.append(encoded)
    ...
    for encoded in frames:
        ...
        framing += encoded
    return (
        HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_TAG
        + hashlib.sha256(bytes(framing)).digest()
        + _encode_varint(public_key)
    )
...
def verify_hdscapa(
    archive: HDSCArchiveProofBundleArchive, key: SigningDKGResult
) -> bool:
    """Verify an archive of HDSC archive proof bundles against the key.

    ``archive`` must be a structurally legal
    :class:`HDSCArchiveProofBundleArchive` and ``key`` a legal
    :class:`SigningDKGResult`. The outer signature and the key structure
    are checked first, as is the structure of every item, so an illegal
    outer signature or key is never masked by a bad nested bundle: a
    non-:class:`HDSCArchiveProofBundleArchive` argument, a non-tuple
    ``items`` field, a non-:class:`HDSCArchiveProofBundle` element or a
    non-:class:`SigningDKGResult` ``key`` raises TypeError, and an empty
    items tuple, an illegal outer signature structure, an illegal key
    structure or any structurally illegal nested bundle raises
    ValueError.

    Every bundle is then passed to
    :func:`verify_hdsc_archive_proof_bundle` in archive order, so each
    disclosed bundle and its root signature must verify under ``key``;
    the canonical :func:`hdscapa_message` of the items and
    ``key.public_key`` is finally checked as the outer signature's
    threshold Schnorr message via :func:`verify_signature` with the
    key's group parameters. Returns ``True`` only when every
    per-bundle check and the outer signature check pass; a structurally
    legal archive whose nested bundle or outer signature does not match
    — including deleting, inserting, reordering or substituting a
    bundle, tampering with the outer signature, or presenting the
    archive under another key — returns ``False`` rather than raising.
    The function is stateless.
    """
    ...
    if not all(
        verify_hdsc_archive_proof_bundle(item, key) for item in items
    ):
        return False
    message = hdscapa_message(items, public_key)
    return verify_signature(
        message,
        archive.signature,
        public_key,
...
def encode_hdscapa(archive: HDSCArchiveProofBundleArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HDSC archive
    proof bundles as one self-delimiting object for transport or
    persistence.

    The encoding is, in order, the tag ``b"ts/hdscapa/w1"``, the
    4-byte unsigned big-endian item count (never zero), then one frame
    per bundle in tuple order — each the 4-byte unsigned big-endian
    byte length (never zero) followed by the exact bytes of
    :func:`encode_hdsc_archive_proof_bundle` for that item, with no
    further separators — and finally the outer signature frame an
    :class:`AuditProofBundle` carries: ``VARINT(R)``, ``VARINT(z)``,
    the 4-byte unsigned big-endian signer count ``k`` and one
    ``VARINT(id)`` per ascending signer id. Every U32 is a 4-byte
    unsigned big-endian integer; a ``VARINT`` is a 4-byte unsigned
    big-endian body length followed by the shortest unsigned
    big-endian value (zero is the single byte ``00``, positive values
    carry no leading zero); ``R`` must be positive, ``z`` may be zero
    and the signer ids are a non-empty strictly increasing tuple of
    positive integers.

    Only the archive container, each individual bundle and the outer
    signature are checked structurally — the items are not sorted or
    otherwise reordered, the outer signature is not matched against
    the bundles, no inner or outer signature is checked and no state
    is read: :func:`verify_hdscapa` stays the way to test an archive
    against a key afterwards. A non-:class:`HDSCArchiveProofBundleArchive`
    argument, a non-tuple ``items`` field or a
    non-:class:`HDSCArchiveProofBundle` element raises TypeError, as do
    wrong field types nested inside a bundle and a
    non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame,
    any structural error a bundle would raise on its own, or an
    illegal outer signature structure raises ValueError. The output
    for a given archive is unique and the encoding carries no
    network, storage or hidden state.
    """
    if not isinstance(archive, HDSCArchiveProofBundleArchive):
        raise TypeError(
    ...
    buffer = bytearray(HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_WIRE_TAG)
    buffer += item_count.to_bytes(4, "big", signed=False)
    for encoded in frames:
        buffer += len(encoded).to_bytes(4, "big", signed=False)
        buffer += encoded
...
def decode_hdscapa(blob: bytes) -> HDSCArchiveProofBundleArchive:
    """Decode the canonical encoding produced by :func:`encode_hdscapa`.

    Accepts only the single canonical form: the tag
    ``b"ts/hdscapa/w1"``, a 4-byte unsigned big-endian non-zero item
    count ``n``, then exactly ``n`` frames, each a 4-byte unsigned
    big-endian non-zero byte length followed by the exact canonical
    encoding of one :class:`HDSCArchiveProofBundle` that
    :func:`decode_hdsc_archive_proof_bundle` accepts, and finally the
    outer signature frame: the length-prefixed integers ``R`` and
    ``z`` with ``R`` positive, the 4-byte non-zero signer count ``k``
    and then exactly ``k`` strictly increasing positive signer ids.
    The bundles are restored in their original order as a tuple; they
    are neither sorted nor matched against one another or against the
    outer signature, and no signature is verified —
    :func:`verify_hdscapa` judges the archive afterwards. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero item
    count, a zero or over-long frame length, a declared count that
    does not match the number of frames, a nested bundle encoding
    that is illegal or non-canonical (including a frame that would
    not re-encode byte for byte), a zero ``R``, a non-canonical
    integer (leading zero or over-long length), a zero signer count,
    a non-positive or non-increasing signer id, truncation, or
    trailing bytes raises ValueError. A successfully decoded archive
    re-encodes to exactly the input bytes.

    Decoding only restores structure: every nested bundle is decoded
    with :func:`decode_hdsc_archive_proof_bundle` (which verifies no
    seal and checks no signature) and the outer signature is not
    checked. A structurally legal archive whose bundles do not verify
    or whose outer signature does not sign the bundles is returned
    normally, and :func:`verify_hdscapa` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_WIRE_TAG):
        raise ValueError("bad HDSC archive proof bundle archive tag")
    offset = len(HDSC_ARCHIVE_PROOF_BUNDLE_ARCHIVE_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated HDSC archive proof bundle archive item count"
        )
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError(
            "HDSC archive proof bundle archive items must be non-empty"
        )
    ...
    for index in range(item_count):
        frame, offset = _read_audit_proof_block(
            ...
            offset,
            what=(
                f"HDSC archive proof bundle archive item {index + 1}"
            ),
        )
        items.append(decode_hdsc_archive_proof_bundle(frame))

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HDSC archive proof bundle archive"
    )
    if offset != len(blob):
        raise ValueError(
            "trailing bytes after HDSC archive proof bundle archive"
        )

    archive = HDSCArchiveProofBundleArchive(
        items=tuple(items), signature=signature
    )
    if encode_hdscapa(archive) != blob:
        raise ValueError(
...
# ---------------------------------------------------------------------------
# Non-empty order-preserving archives of whole HAMPB transport bundles,
# authenticated by one outer threshold-Schnorr signature. The signed message
# commits to the verifying public key and to the SHA256 digest of the exact
# canonical framing of the bundle sequence, so deleting, inserting,
# reordering or substituting a bundle — or presenting the archive under
# another key — all invalidate the outer signature. The library keeps no
# state: the archive is a plain frozen value, every bundle is re-checked
# with verify_hampb on verification, and encoding restores structure only.
# ---------------------------------------------------------------------------

HAMPBA_MESSAGE_TAG = b"ts/hampba/m1"
HAMPBA_WIRE_TAG = b"ts/hampba/w1"


@dataclass(frozen=True)
class HAMPBArchive:
    """A non-empty ordered archive of HAMPB bundles with one outer signature.

    The fields, in order, are ``items`` — a non-empty tuple of
    :class:`HAMPB` values in archive order — and ``signature`` — the
    threshold Schnorr :class:`AggregateSignature` on
    :func:`hampba_message` of the items and the verifying public key.
    The dataclass is frozen, positionally constructible and compared by
    value; neither field has a default, the items keep their order and
    the archive carries no network, storage or hidden state. Neither
    field is checked at construction time — :func:`hampba_message`
    requires structurally legal items and :func:`verify_hampba` is the
    way to test an archive against a key afterwards.
    """
...
def hampba_message(
    items: tuple[HAMPB, ...], public_key: int
) -> bytes:
    """Encode the canonical message the threshold key signs for an archive.

    The message is, in order, the tag ``b"ts/hampba/m1"``, the 32-byte
    ``SHA256`` digest of the canonical framing ``C`` and
    ``V(public_key)`` — a 4-byte unsigned big-endian length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). With
    ``E_i = encode_hampb(items[i])`` and ``n`` the item count, ``C`` is
    built in tuple order as ``U32(n) || Σ(U32(len(E_i)) || E_i)``;
    every ``U32`` is a 4-byte unsigned big-endian integer and ``E_i`` is
    the existing canonical transport encoding of the bundle. The
    message carries no signature and keeps no state, and can be taken
    through the two-round threshold signing flow as is.

    ``items`` must be a non-empty tuple of structurally legal
    :class:`HAMPB` values exactly as :func:`encode_hampb` requires and
    ``public_key`` a non-boolean non-negative integer. Wrong field or
    nested field types raise TypeError — a non-tuple items sequence, an
    item that is not an :class:`HAMPB`, or a non-integer (including
    boolean) public key; an empty items tuple, an illegal nested
    bundle, a negative public key or an over-long encoding raises
    ValueError.
    """
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    for item in items:
        if not isinstance(item, HAMPB):
            raise TypeError("each archive item must be an HAMPB instance")
    if not isinstance(public_key, int) or isinstance(public_key, bool):
        raise TypeError("public_key must be an integer")
    ...
    if item_count == 0:
        raise ValueError("archive items must be a non-empty tuple")
    if item_count > 0xFFFFFFFF:
        raise ValueError("too many HAMPB archive items")
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HAMPB archive item {index + 1} encoding too long"
            )
        frames.append(encoded)
...
def verify_hampba(
    archive: HAMPBArchive, key: SigningDKGResult
) -> bool:
    """Verify an archive of HAMPB bundles against the threshold key.

    ``archive`` must be a structurally legal :class:`HAMPBArchive` and
    ``key`` a legal :class:`SigningDKGResult`. The outer signature and
    the key structure are checked first, as is the structure of every
    item, so an illegal outer signature or key is never masked by a bad
    nested bundle: a non-:class:`HAMPBArchive` argument, a non-tuple
    ``items`` field, a non-:class:`HAMPB` element or a
    non-:class:`SigningDKGResult` ``key`` raises TypeError, and an
    empty items tuple, an illegal outer signature structure, an illegal
    key structure or any structurally illegal nested bundle raises
    ValueError.

    Every bundle is then passed to :func:`verify_hampb` in archive
    order, so each disclosed HAMP proof and its root signature must
    verify under ``key``; the canonical :func:`hampba_message` of the
    items and ``key.public_key`` is finally checked as the outer
    signature's threshold Schnorr message via :func:`verify_signature`
    with the key's group parameters. Returns ``True`` only when every
    per-bundle check and the outer signature check pass; a structurally
    legal archive whose nested bundle or outer signature does not match
    ...
    The function is stateless.
    """
    if not isinstance(archive, HAMPBArchive):
        raise TypeError(
            "archive must be an HAMPBArchive instance"
        )
    items = archive.items
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    ...
    if not all(verify_hampb(item, key) for item in items):
        return False
    message = hampba_message(items, public_key)
    return verify_signature(
        message,
        archive.signature,
        public_key,
...
def encode_hampba(archive: HAMPBArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HAMPB bundles as
    one self-delimiting object for transport or persistence.

    The encoding is, in order, the tag ``b"ts/hampba/w1"``, the 4-byte
    unsigned big-endian item count (never zero), then one frame per
    bundle in tuple order — each the 4-byte unsigned big-endian byte
    length (never zero) followed by the exact bytes of
    :func:`encode_hampb` for that item, with no further separators —
    and finally the outer signature frame an :class:`AuditProofBundle`
    carries: ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned
    big-endian signer count ``k`` and one ``VARINT(id)`` per ascending
    signer id. Every U32 is a 4-byte unsigned big-endian integer; a
    ``VARINT`` is a 4-byte unsigned big-endian body length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero); ``R`` must be
    positive, ``z`` may be zero and the signer ids are a non-empty
    strictly increasing tuple of positive integers.

    Only the archive container, each individual bundle and the outer
    signature are checked structurally — the items are not sorted or
    otherwise reordered, the outer signature is not matched against the
    bundles, no inner or outer signature is checked and no state is
    read: :func:`verify_hampba` stays the way to test an archive
    against a key afterwards. A non-:class:`HAMPBArchive` argument, a
    non-tuple ``items`` field or a non-:class:`HAMPB` element raises
    TypeError, as do wrong field types nested inside a bundle and a
    non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame,
    any structural error a bundle would raise on its own, or an
    illegal outer signature structure raises ValueError. The output
    ...
    buffer = bytearray(HAMPBA_WIRE_TAG)
    buffer += item_count.to_bytes(4, "big", signed=False)
    for encoded in frames:
        buffer += len(encoded).to_bytes(4, "big", signed=False)
        buffer += encoded
...
def decode_hampba(blob: bytes) -> HAMPBArchive:
    """Decode the canonical encoding produced by :func:`encode_hampba`.

    Accepts only the single canonical form: the tag
    ``b"ts/hampba/w1"``, a 4-byte unsigned big-endian non-zero item
    count ``n``, then exactly ``n`` frames, each a 4-byte unsigned
    big-endian non-zero byte length followed by the exact canonical
    encoding of one :class:`HAMPB` that :func:`decode_hampb` accepts,
    and finally the outer signature frame: the length-prefixed integers
    ``R`` and ``z`` with ``R`` positive, the 4-byte non-zero signer
    count ``k`` and then exactly ``k`` strictly increasing positive
    signer ids. The bundles are restored in their original order as a
    tuple; they are neither sorted nor matched against one another or
    against the outer signature, and no signature is verified —
    :func:`verify_hampba` judges the archive afterwards. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero item
    count, a zero or over-long frame length, a declared count that does
    not match the number of frames, a nested bundle encoding that is
    illegal or non-canonical (including a frame that would not
    re-encode byte for byte), a zero ``R``, a non-canonical integer
    (leading zero or over-long length), a zero signer count, a
    non-positive or non-increasing signer id, truncation, or trailing
    bytes raises ValueError. A successfully decoded archive re-encodes
    to exactly the input bytes.

    Decoding only restores structure: every nested bundle is decoded
    with :func:`decode_hampb` (which verifies no bundle and checks no
    signature) and the outer signature is not checked. A structurally
    legal archive whose bundles do not verify or whose outer signature
    does not sign the bundles is returned normally, and
    :func:`verify_hampba` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HAMPBA_WIRE_TAG):
        raise ValueError("bad HAMPB archive tag")
    offset = len(HAMPBA_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError("truncated HAMPB archive item count")
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError("HAMPB archive items must be non-empty")
    ...
    for index in range(item_count):
        frame, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"HAMPB archive item {index + 1}",
        )
        items.append(decode_hampb(frame))

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HAMPB archive"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HAMPB archive")

    archive = HAMPBArchive(items=tuple(items), signature=signature)
    if encode_hampba(archive) != blob:
        raise ValueError("non-canonical HAMPB archive encoding")
    return archive


# ---------------------------------------------------------------------------
# Compact multi-bundle membership proofs over the HAMPB archive: one root
# signature covers any subset of the archive's HAMPB transport bundles. The
# tree is isomorphic to the HAMP membership proof tree — leaves bind the
# position to the bundle's canonical encode_hampb bytes, internal nodes are
# ordered pairs and an odd-width level pairs its last node with itself — with
# its own domain tags b"hbap/l", b"hbap/n" and b"hbap/r". The proof and the
# proof-plus-signature transport bundle are plain frozen values, decoding
# restores structure only and no state is kept: check_hbap and verify_hbapb
# remain the sole verifiers.
# ---------------------------------------------------------------------------

HBAP_LEAF_TAG = b"hbap/l"
HBAP_NODE_TAG = b"hbap/n"
HBAP_ROOT_TAG = b"hbap/r"

HBAP_DIGEST_SIZE = 32  # SHA256 width; every tree node is this wide


def _hbap_u64(value: int) -> bytes:
    """8-byte unsigned big-endian encoding of a non-negative integer < 2**64."""
    return value.to_bytes(8, "big", signed=False)


def _hbap_leaf(index: int, bundle: HAMPB) -> bytes:
    """The index-bound leaf digest ``H(b"hbap/l" || U64(i) || H(encode_hampb(bundle)))``."""
    return hashlib.sha256(
        HBAP_LEAF_TAG
        + _hbap_u64(index)
        + hashlib.sha256(encode_hampb(bundle)).digest()
    ).digest()


def _hbap_node(left: bytes, right: bytes) -> bytes:
    """The ordered internal digest ``H(b"hbap/n" || left || right)``."""
    return hashlib.sha256(HBAP_NODE_TAG + left + right).digest()
...
def _hbap_levels(
    ...
    while len(current) > 1:
        if len(current) % 2 == 1:
            current = current + current[-1:]
        current = tuple(
            _hbap_node(current[index], current[index + 1])
            for index in range(0, len(current), 2)
        )
        levels.append(current)
    return levels
...
def _check_hbap_archive_items(
    items: object,
) -> tuple[HAMPB, ...]:
    """Type- and structure-check a non-empty ordered tuple of HAMPB bundles.

    Mirrors the container and per-item checks the archive codec applies
    to ``items``: a tuple of :class:`HAMPB` instances, non-empty, each
    ...
    for item in items:
        if not isinstance(item, HAMPB):
            raise TypeError(
                "each archive item must be an HAMPB instance"
            )
    if len(items) == 0:
        raise ValueError("archive items must be a non-empty tuple")
    # Encode every bundle first — each call raises TypeError/ValueError
...
@dataclass(frozen=True)
class HBAP:
    """A compact Merkle membership proof for several bundles of an HAMPB archive.

    The fields, in order, are ``indices`` (the proven leaf positions as a
    non-empty tuple of strictly increasing, unique non-negative integers),
    ``total`` (the total, non-empty archive bundle count), ``bundles``
    (the proven :class:`HAMPB` values, one per entry of ``indices``, in
    ...
    The dataclass is frozen, positionally constructible and compared by
    value, and carries no network, storage or hidden state. Field types
    and bounds are not checked at construction time — :func:`check_hbap`
...
def make_hbap(
    archive: HAMPBArchive, indices: tuple[int, ...]
) -> tuple[bytes, HBAP]:
    """Build the root statement and a compact multi-bundle membership proof.

    ``archive`` must be an :class:`HAMPBArchive` whose items are a
    non-empty tuple of structurally legal :class:`HAMPB` values exactly
    as :func:`encode_hampb` requires, and ``indices`` a non-empty tuple
    ...
    with SHA256, exactly isomorphic to the HAMP membership proof tree:
    leaves are ``H(b"hbap/l" || U64(i) || H(encode_hampb(bundle)))`` and
    internal nodes ``H(b"hbap/n" || left || right)``; a level with an odd
    ...
    ``(message, proof)`` where ``message`` is
    ``b"hbap/r" || U64(total) || root`` (the 8-byte unsigned big-endian
    bundle count followed by the 32-byte root) — the bytes to be
    threshold-signed through the caller's two-round signing flow — and
    ``proof`` is the :class:`HBAP` whose ``bundles`` take the archive
    items at ``indices`` in archive order, one to one. Its ``siblings``
    are collected level by level, in ascending level order (leaves
    upward) and left to right within a level, one entry per needed
    companion: when the companion position is itself a proven node
    ...
    Wrong argument types raise TypeError: a non-:class:`HAMPBArchive`
    archive, a non-tuple ``archive.items`` field, an item that is not an
    :class:`HAMPB`, a non-tuple index sequence or a non-integer
    ...
    if not isinstance(indices, tuple):
        raise TypeError("indices must be a tuple")
    items = _check_hbap_archive_items(archive.items)
    total = len(items)
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("too many HAMPB archive items")
    if len(indices) == 0:
    ...
    return HBAP_ROOT_TAG + _hbap_u64(total) + root, proof


def _validate_hbap_structure(
    proof: object,
) -> tuple[
    tuple[int, ...],
    int,
    ...
    """Type- and structure-check an HBAP, returning its fields.

    Only the container structure is checked: the bundles themselves are
    neither encoded nor verified here (encoding happens in the leaf
    recomputation and verification is :func:`verify_hampb`'s job). The
    bounds are ``0 < total < 2**64``, a non-empty ``indices`` tuple of
    strictly increasing indices in ``0 <= i < total`` and a siblings
    tuple whose entries are exactly 32 bytes. Wrong field types raise
    TypeError; illegal bounds or shapes raise ValueError. The two count
    ...
    count the compact walk derives from ``total`` and ``indices`` — are
    not judged here: :func:`check_hbap` reports either mismatch as
    ``False``.
    """
    if not isinstance(proof, HBAP):
        raise TypeError("proof must be an HBAP instance")
    indices = proof.indices
    total = proof.total
    bundles = proof.bundles
    siblings = proof.siblings
    ...
    for bundle in bundles:
        if not isinstance(bundle, HAMPB):
            raise TypeError(
                "proof.bundles entries must be HAMPB instances"
            )
    ...
    for sibling in siblings:
        if not isinstance(sibling, bytes):
            raise TypeError("proof.siblings entries must be bytes")
    for sibling in siblings:
        if len(sibling) != HBAP_DIGEST_SIZE:
            raise ValueError(
                "proof.siblings entries must be exactly 32 bytes"
            )
    return indices, total, bundles, siblings
...
def check_hbap(
    ...
    The root ``signature`` and ``key`` structures are checked first.
    Each disclosed bundle's leaf digest is then recomputed from its
    position and the bundle exactly as in :func:`make_hbap`, and the
    root is rebuilt level by level while consuming ``proof.siblings`` in
    ascending level order and left to right within a level: the current
    level holds the digests of the proven nodes at their current
    positions, paired left to right; when a companion is itself a
    ...
    current width contracts as ``(width + 1) // 2`` and the rebuild must
    consume every sibling exactly. Every disclosed bundle is then
    re-checked with :func:`verify_hampb` against ``key`` in index order,
    so each bundle's inner HAMP proof and every archive proof bundle it
    discloses must verify, and the statement
    ``b"hbap/r" || U64(total) || root`` is finally checked as
    ``signature``'s threshold Schnorr message via
    :func:`verify_signature` with the key's group parameters. Returns
    ``True`` only when the rebuild consumes every sibling exactly and
    reaches the single signed root, every bundle verifies against the
    ...
    A non-:class:`HBAP` or non-:class:`AggregateSignature` argument or
    any wrong field type (non-tuple indices/bundles/siblings, a
    non-integer ``total`` or index including booleans, a non-:class:`HAMPB`
    ...
    exactly as :func:`verify_hampb` and :func:`verify_signature` would.
    """
    indices, total, bundles, siblings = _validate_hbap_structure(proof)
    if not isinstance(signature, AggregateSignature):
        raise TypeError("signature must be an AggregateSignature instance")
    _check_history_proof_bundle_signature(signature)
    ...
    for bundle in bundles:
        if not verify_hampb(bundle, key):
            return False
...
# ---------------------------------------------------------------------------
# Canonical HBAPB transport: an HBAP carried together with the aggregate
# threshold-Schnorr signature over its root statement — the same
# b"hbap/r" || U64(total) || root message make_hbap returns — as one
# self-delimiting, byte-for-byte reproducible byte string, mirroring the
# proof-plus-signature bundles. Decoding restores structure only — the nested
# bundles go through decode_hampb, no bundle or signature is checked and no
# state is kept, so verify_hbapb remains the sole verifier afterwards.
# ---------------------------------------------------------------------------

HBAPB_WIRE_TAG = b"ts/hbapb/v1"
...
class HBAPB:
    ...
    The fields, in order, are ``proof`` (an :class:`HBAP`) and
    ``signature`` (the :class:`AggregateSignature` on the proof's
    ``b"hbap/r" || U64(total) || root`` statement, the exact message
    :func:`make_hbap` returns). The dataclass is frozen, positionally
    constructible and compared by value, and carries no network, storage
    or hidden state. Field types and bounds are not checked at
    construction time — :func:`encode_hbapb` checks the structure and
    :func:`verify_hbapb` is the way to test a bundle afterwards.
    """
...
def encode_hbapb(bundle: HBAPB) -> bytes:
    """Canonically encode an hbap bundle for transport or persistence.

    The encoding is the direct concatenation, in order, of the tag
    ``b"ts/hbapb/v1"``, ``VARINT(total)``, the index count as a 4-byte
    unsigned big-endian integer, one ``VARINT(index)`` per proven leaf in
    the proof's strictly increasing order, the bundle count as a 4-byte
    unsigned big-endian integer, one frame per bundle — the 4-byte
    unsigned big-endian length ``len(E)`` followed by the raw bytes
    ``E = encode_hampb(bundle_value)`` (never empty) — the sibling count
    as a 4-byte unsigned big-endian integer followed by the raw 32-byte
    sibling digests in their original leaf-to-root, left-to-right order,
    and finally the signature frame: ``VARINT(R)``, ``VARINT(z)``, the
    4-byte unsigned big-endian signer count and one ``VARINT(id)`` per
    ascending signer id. A ``VARINT`` is a 4-byte unsigned big-endian
    body length followed by the shortest unsigned big-endian value (zero
    is the single byte ``00``, positive values carry no leading zero);
    ``R`` must be positive and ``z`` may be zero. The indices must be
    non-empty, strictly increasing, unique and within ``0 <= i < total``,
    the bundle count must equal the index count, and the sibling count
    must be the unique one derived from ``total`` and the indices by the
    compact multi-proof walk.

    Only a structurally legal :class:`HBAPB` is accepted — a non-bundle
    or non-:class:`HBAP` proof argument, or wrong proof/signature field
    types, raise TypeError and illegal bounds, an empty index tuple, a
    bundle tuple that does not pair one-to-one with the indices, a
    sibling that is not exactly 32 bytes, a sibling count other than the
    one ``total`` and the indices determine, a structurally illegal
    ...
    an empty or non-strictly-increasing signer id tuple) or an over-long
    frame raises ValueError — but the bundles are not verified and the
    signature is not checked against the root: :func:`verify_hbapb` stays
    the way to verify a bundle afterwards. The output for a given bundle
    is unique and the encoding carries no network, storage or hidden
    state.
    """
    if not isinstance(bundle, HBAPB):
        raise TypeError("bundle must be an HBAPB instance")
    if not isinstance(bundle.proof, HBAP):
        raise TypeError("bundle.proof must be an HBAP instance")
    indices, total, bundles, siblings = _validate_hbap_structure(
        bundle.proof
    )
    if len(bundles) != len(indices):
        raise ValueError(
    ...
    encoded_bundles = []
    for position, bundle_value in enumerate(bundles):
        encoded_bundle = encode_hampb(bundle_value)
        if len(encoded_bundle) > 0xFFFFFFFF:
            raise ValueError(
                f"hbap bundle {position + 1} encoding too long"
            )
        encoded_bundles.append(encoded_bundle)

    buffer = bytearray(HBAPB_WIRE_TAG)
    buffer += _encode_varint(total)
    buffer += len(indices).to_bytes(4, "big", signed=False)
    for index in indices:
        buffer += _encode_varint(index)
...
def decode_hbapb(blob: bytes) -> HBAPB:
    """Decode the canonical encoding produced by :func:`encode_hbapb`.

    Accepts only the single canonical form: the tag ``b"ts/hbapb/v1"``,
    the length-prefixed integer ``total`` (a 4-byte unsigned big-endian
    length followed by its shortest unsigned big-endian value, zero
    encoded as the single byte ``00``), the 4-byte non-zero index count
    followed by one canonical ``VARINT`` per strictly increasing index,
    the 4-byte bundle count (which must equal the index count) followed
    by that many non-empty bundle frames (a 4-byte non-zero length
    followed by bytes that :func:`decode_hampb` accepts), the 4-byte
    sibling count followed by exactly that many raw 32-byte sibling
    digests, where the count must be the unique one derived from
    ``total`` and the disclosed indices, and finally the signature frame:
    the length-prefixed integers ``R`` and ``z`` with ``R`` positive, the
    ...
    :func:`verify_hbapb` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HBAPB_WIRE_TAG):
        raise ValueError("bad HAMPB archive proof bundle tag")
    offset = len(HBAPB_WIRE_TAG)

    total, offset = _read_varint(blob, offset, what="HAMPB archive proof total")
    if total <= 0:
        raise ValueError("HAMPB archive proof total must be positive")
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("HAMPB archive proof total too large")

    if offset + 4 > len(blob):
        raise ValueError("truncated HAMPB archive proof index count")
    index_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if index_count == 0:
        raise ValueError("HAMPB archive proof indices must be non-empty")
    indices = []
    for _ in range(index_count):
        index, offset = _read_varint(
            blob, offset, what="HAMPB archive proof index"
        )
        indices.append(index)

    if offset + 4 > len(blob):
        raise ValueError("truncated HAMPB archive proof bundle count")
    bundle_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if bundle_count != index_count:
        raise ValueError(
            "HAMPB archive proof bundle count must match the index count"
        )
    bundles = []
    for position in range(bundle_count):
        encoded_bundle, offset = _read_archive_block(
            blob,
            offset,
            what=f"HAMPB archive proof bundle {position + 1}",
        )
        bundles.append(decode_hampb(encoded_bundle))

    if offset + 4 > len(blob):
        raise ValueError("truncated HAMPB archive proof sibling count")
    sibling_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    required_siblings = _required_history_multi_proof_siblings(
        total, tuple(indices)
    ...
    if sibling_count != required_siblings:
        raise ValueError(
            "HAMPB archive proof sibling count is not the one determined "
            "by total and indices"
        )
    siblings = []
    for _ in range(sibling_count):
        if offset + HBAP_DIGEST_SIZE > len(blob):
            raise ValueError("truncated HAMPB archive proof sibling")
        siblings.append(
            bytes(blob[offset:offset + HBAP_DIGEST_SIZE])
        )
        offset += HBAP_DIGEST_SIZE

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HAMPB archive proof"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HAMPB archive proof bundle")
    ...
    _validate_hbap_structure(proof)
    bundle = HBAPB(proof=proof, signature=signature)
    if encode_hbapb(bundle) != blob:
        raise ValueError("non-canonical HAMPB archive proof bundle encoding")
    return bundle


def verify_hbapb(bundle: HBAPB, key: SigningDKGResult) -> bool:
    """Verify a bundle exactly as :func:`check_hbap` would.

    This is a convenience wrapper over
    ``check_hbap(bundle.proof, bundle.signature, key)``: the Merkle root
    is rebuilt from the proof, every disclosed bundle is re-checked with
    :func:`verify_hampb` against ``key`` and the signature verified on
    the exact statement :func:`make_hbap` returns,
    ``b"hbap/r" || U64(total) || root``. Returns ``True`` only when all
    of them hold; a well-formed bundle with a tampered bundle, index,
    sibling or signature, a bundle swapped or altered, a missing or extra
    sibling, or one presented under another key, returns ``False``.

    A non-:class:`HBAPB` argument raises TypeError; illegal nested proof,
    bundle, signature or key structure raises TypeError/ValueError,
    exactly as :func:`check_hbap` does.
    """
    if not isinstance(bundle, HBAPB):
        raise TypeError("bundle must be an HBAPB instance")
    return check_hbap(bundle.proof, bundle.signature, key)


# ---------------------------------------------------------------------------
# Non-empty order-preserving archives of whole HBAPB transport bundles,
# authenticated by one outer threshold-Schnorr signature. The signed message
# commits to the verifying public key and to the SHA256 digest of the exact
# canonical framing of the bundle sequence, so deleting, inserting,
# reordering or substituting a bundle — or presenting the archive under
# another key — all invalidate the outer signature. The library keeps no
# state: the archive is a plain frozen value, every bundle is re-checked
# with verify_hbapb on verification, and encoding restores structure only.
# ---------------------------------------------------------------------------

HBAPBA_MESSAGE_TAG = b"ts/hbapba/m1"
HBAPBA_WIRE_TAG = b"ts/hbapba/w1"


@dataclass(frozen=True)
class HBAPBArchive:
    """A non-empty ordered archive of HBAPB bundles with one outer signature.

    The fields, in order, are ``items`` — a non-empty tuple of
    :class:`HBAPB` values in archive order — and ``signature`` — the
    threshold Schnorr :class:`AggregateSignature` on
    :func:`hbapba_message` of the items and the verifying public key.
    The dataclass is frozen, positionally constructible and compared by
    value; neither field has a default, the items keep their order and
    the archive carries no network, storage or hidden state. Neither
    field is checked at construction time — :func:`hbapba_message`
    requires structurally legal items and :func:`verify_hbapba` is the
    way to test an archive against a key afterwards.
    """
...
def hbapba_message(
    items: tuple[HBAPB, ...], public_key: int
) -> bytes:
    """Encode the canonical message the threshold key signs for an archive.

    The message is, in order, the tag ``b"ts/hbapba/m1"``, the 32-byte
    ``SHA256`` digest of the canonical framing ``C`` and
    ``V(public_key)`` — a 4-byte unsigned big-endian length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). With
    ``E_i = encode_hbapb(items[i])`` and ``n`` the item count, ``C`` is
    built in tuple order as ``U32(n) || Σ(U32(len(E_i)) || E_i)``;
    every ``U32`` is a 4-byte unsigned big-endian integer and ``E_i`` is
    the existing canonical transport encoding of the bundle. The
    message carries no signature and keeps no state, and can be taken
    ...
    ``items`` must be a non-empty tuple of structurally legal
    :class:`HBAPB` values exactly as :func:`encode_hbapb` requires and
    ``public_key`` a non-boolean non-negative integer. Wrong field or
    nested field types raise TypeError — a non-tuple items sequence, an
    item that is not an :class:`HBAPB`, or a non-integer (including
    boolean) public key; an empty items tuple, an illegal nested
    bundle, a negative public key or an over-long encoding raises
    ValueError.
    """
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    for item in items:
        if not isinstance(item, HBAPB):
            raise TypeError("each archive item must be an HBAPB instance")
    if not isinstance(public_key, int) or isinstance(public_key, bool):
        raise TypeError("public_key must be an integer")
    ...
    if item_count == 0:
        raise ValueError("archive items must be a non-empty tuple")
    if item_count > 0xFFFFFFFF:
        raise ValueError("too many HBAPB archive items")
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HBAPB archive item {index + 1} encoding too long"
            )
        frames.append(encoded)
...
def verify_hbapba(
    archive: HBAPBArchive, key: SigningDKGResult
) -> bool:
    """Verify an archive of HBAPB bundles against the threshold key.

    ``archive`` must be a structurally legal :class:`HBAPBArchive` and
    ``key`` a legal :class:`SigningDKGResult`. The outer signature and
    the key structure are checked first, as is the structure of every
    item, so an illegal outer signature or key is never masked by a bad
    nested bundle: a non-:class:`HBAPBArchive` argument, a non-tuple
    ``items`` field, a non-:class:`HBAPB` element or a
    non-:class:`SigningDKGResult` ``key`` raises TypeError, and an
    empty items tuple, an illegal outer signature structure, an illegal
    key structure or any structurally illegal nested bundle raises
    ValueError.

    Every bundle is then passed to :func:`verify_hbapb` in archive
    order, so each disclosed HBAP proof and its root signature must
    verify under ``key``; the canonical :func:`hbapba_message` of the
    items and ``key.public_key`` is finally checked as the outer
    signature's threshold Schnorr message via :func:`verify_signature`
    with the key's group parameters. Returns ``True`` only when every
    per-bundle check and the outer signature check pass; a structurally
    ...
    The function is stateless.
    """
    if not isinstance(archive, HBAPBArchive):
        raise TypeError(
            "archive must be an HBAPBArchive instance"
        )
    items = archive.items
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    ...
    if not all(verify_hbapb(item, key) for item in items):
        return False
    message = hbapba_message(items, public_key)
    return verify_signature(
        message,
        archive.signature,
        public_key,
...
def encode_hbapba(archive: HBAPBArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HBAPB bundles as
    one self-delimiting object for transport or persistence.

    The encoding is, in order, the tag ``b"ts/hbapba/w1"``, the 4-byte
    unsigned big-endian item count (never zero), then one frame per
    bundle in tuple order — each the 4-byte unsigned big-endian byte
    length (never zero) followed by the exact bytes of
    :func:`encode_hbapb` for that item, with no further separators —
    and finally the outer signature frame an :class:`AuditProofBundle`
    carries: ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned
    big-endian signer count ``k`` and one ``VARINT(id)`` per ascending
    signer id. Every U32 is a 4-byte unsigned big-endian integer; a
    ...
    otherwise reordered, the outer signature is not matched against the
    bundles, no inner or outer signature is checked and no state is
    read: :func:`verify_hbapba` stays the way to test an archive
    against a key afterwards. A non-:class:`HBAPBArchive` argument, a
    non-tuple ``items`` field or a non-:class:`HBAPB` element raises
    TypeError, as do wrong field types nested inside a bundle and a
    non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame,
    any structural error a bundle would raise on its own, or an
    ...
    buffer = bytearray(HBAPBA_WIRE_TAG)
    buffer += item_count.to_bytes(4, "big", signed=False)
    for encoded in frames:
        buffer += len(encoded).to_bytes(4, "big", signed=False)
        buffer += encoded
...
def decode_hbapba(blob: bytes) -> HBAPBArchive:
    """Decode the canonical encoding produced by :func:`encode_hbapba`.

    Accepts only the single canonical form: the tag
    ``b"ts/hbapba/w1"``, a 4-byte unsigned big-endian non-zero item
    count ``n``, then exactly ``n`` frames, each a 4-byte unsigned
    big-endian non-zero byte length followed by the exact canonical
    encoding of one :class:`HBAPB` that :func:`decode_hbapb` accepts,
    and finally the outer signature frame: the length-prefixed integers
    ``R`` and ``z`` with ``R`` positive, the 4-byte non-zero signer
    count ``k`` and then exactly ``k`` strictly increasing positive
    signer ids. The bundles are restored in their original order as a
    tuple; they are neither sorted nor matched against one another or
    against the outer signature, and no signature is verified —
    :func:`verify_hbapba` judges the archive afterwards. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero item
    count, a zero or over-long frame length, a declared count that does
    not match the number of frames, a nested bundle encoding that is
    illegal or non-canonical (including a frame that would not
    ...
    Decoding only restores structure: every nested bundle is decoded
    with :func:`decode_hbapb` (which verifies no bundle and checks no
    signature) and the outer signature is not checked. A structurally
    legal archive whose bundles do not verify or whose outer signature
    does not sign the bundles is returned normally, and
    :func:`verify_hbapba` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HBAPBA_WIRE_TAG):
        raise ValueError("bad HBAPB archive tag")
    offset = len(HBAPBA_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError("truncated HBAPB archive item count")
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError("HBAPB archive items must be non-empty")

    items = []
    for index in range(item_count):
        frame, offset = _read_archive_block(
            blob,
            offset,
            what=f"HBAPB archive item {index + 1}",
        )
        items.append(decode_hbapb(frame))

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HBAPB archive"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HBAPB archive")

    archive = HBAPBArchive(items=tuple(items), signature=signature)
    if encode_hbapba(archive) != blob:
        raise ValueError("non-canonical HBAPB archive encoding")
    return archive


# ---------------------------------------------------------------------------
# Compact multi-bundle membership proofs over the HBAPB archive: one root
# signature covers any subset of the archive's HBAPB transport bundles. The
# tree is isomorphic to the HBAP membership proof tree — leaves bind the
# position to the bundle's canonical encode_hbapb bytes, internal nodes are
# ordered pairs and an odd-width level pairs its last node with itself — with
# its own domain tags b"hbp/l", b"hbp/n" and b"hbp/r". The proof and the
# proof-plus-signature transport bundle are plain frozen values, decoding
# restores structure only and no state is kept: check_hbp and verify_hbpb
# remain the sole verifiers.
# ---------------------------------------------------------------------------

HBP_LEAF_TAG = b"hbp/l"
HBP_NODE_TAG = b"hbp/n"
HBP_ROOT_TAG = b"hbp/r"

HBP_DIGEST_SIZE = 32  # SHA256 width; every tree node is this wide


def _hbp_u64(value: int) -> bytes:
    """8-byte unsigned big-endian encoding of a non-negative integer < 2**64."""
    return value.to_bytes(8, "big", signed=False)


def _hbp_leaf(index: int, bundle: HBAPB) -> bytes:
    """The index-bound leaf digest ``H(b"hbp/l" || U64(i) || H(encode_hbapb(bundle)))``."""
    return hashlib.sha256(
        HBP_LEAF_TAG
        + _hbp_u64(index)
        + hashlib.sha256(encode_hbapb(bundle)).digest()
    ).digest()


def _hbp_node(left: bytes, right: bytes) -> bytes:
    """The ordered internal digest ``H(b"hbp/n" || left || right)``."""
    return hashlib.sha256(HBP_NODE_TAG + left + right).digest()
...
def _hbp_levels(
    ...
    while len(current) > 1:
        if len(current) % 2 == 1:
            current = current + current[-1:]
        current = tuple(
            _hbp_node(current[index], current[index + 1])
            for index in range(0, len(current), 2)
        )
        levels.append(current)
    return levels
...
def _check_hbp_archive_items(
    items: object,
) -> tuple[HBAPB, ...]:
    """Type- and structure-check a non-empty ordered tuple of HBAPB bundles.

    Mirrors the container and per-item checks the archive codec applies
    to ``items``: a tuple of :class:`HBAPB` instances, non-empty, each
    structurally legal through its own canonical encoder. The archive's
    outer signature is deliberately outside the scope — the proof is
    built from the bundles alone. Nothing here verifies any signature.
    """
    ...
    for item in items:
        if not isinstance(item, HBAPB):
            raise TypeError(
                "each archive item must be an HBAPB instance"
            )
    try:
        count = len(items)
    except OverflowError:
        raise ValueError("too many HBAPB archive items") from None
    if count == 0:
        raise ValueError("archive items must be a non-empty tuple")
    # Encode every bundle first — each call raises TypeError/ValueError
    # for an illegal bundle exactly as the bundle codec does — so a
...
@dataclass(frozen=True)
class HBP:
    """A compact Merkle membership proof for several bundles of an HBAPB archive.

    The fields, in order, are ``indices`` (the proven leaf positions as a
    non-empty tuple of strictly increasing, unique non-negative integers),
    ``total`` (the total, non-empty archive bundle count), ``bundles``
    (the proven :class:`HBAPB` values, one per entry of ``indices``, in
    the same order) and ``siblings`` (the 32-byte sibling digests consumed
    from the leaf level up to the root, in ascending level order and left
    to right within a level; the tuple is empty when no sibling digests
    are needed). At a level whose width is odd, the last node pairs with
    ...
    The dataclass is frozen, positionally constructible and compared by
    value, and carries no network, storage or hidden state. Field types
    and bounds are not checked at construction time — :func:`check_hbp`
    is the way to test a proof afterwards.
    """
...
def make_hbp(
    archive: HBAPBArchive, indices: tuple[int, ...]
) -> tuple[bytes, HBP]:
    """Build the root statement and a compact multi-bundle membership proof.

    ``archive`` must be an :class:`HBAPBArchive` whose items are a
    non-empty tuple of structurally legal :class:`HBAPB` values exactly
    as :func:`encode_hbapb` requires, and ``indices`` a non-empty tuple
    of leaf positions to prove. Only the archive's structure and its
    bundles are consulted: the archive's own outer signature is never
    examined and no state is kept. The tree is built in archive order
    with SHA256, exactly isomorphic to the HBAP membership proof tree:
    leaves are ``H(b"hbp/l" || U64(i) || H(encode_hbapb(bundle)))`` and
    internal nodes ``H(b"hbp/n" || left || right)``; a level with an odd
    tail width duplicates its last node for pairing, and positions and
    counts are encoded as 8-byte unsigned big-endian integers. Returns
    ``(message, proof)`` where ``message`` is
    ``b"hbp/r" || U64(total) || root`` (the 8-byte unsigned big-endian
    bundle count followed by the 32-byte root) — the bytes to be
    threshold-signed through the caller's two-round signing flow — and
    ``proof`` is the :class:`HBP` whose ``bundles`` take the archive
    items at ``indices`` in archive order, one to one. Its ``siblings``
    are collected level by level, in ascending level order (leaves
    upward) and left to right within a level, one entry per needed
    companion: when the companion position is itself a proven node
    ...
    Wrong argument types raise TypeError: a non-:class:`HBAPBArchive`
    archive, a non-tuple ``archive.items`` field, an item that is not an
    :class:`HBAPB`, a non-tuple index sequence or a non-integer
    (including boolean) index. An empty archive or index tuple, a
    structurally illegal nested bundle, a count at or above ``2**64``,
    indices that are not strictly increasing and unique, or an index
    outside ``0 <= i < total`` raises ValueError.
    ...
    if not isinstance(indices, tuple):
        raise TypeError("indices must be a tuple")
    items = _check_hbp_archive_items(archive.items)
    try:
        total = len(items)
    except OverflowError:
        raise ValueError("too many HBAPB archive items") from None
    ...
    return HBP_ROOT_TAG + _hbp_u64(total) + root, proof


def _validate_hbp_structure(
    proof: object,
) -> tuple[
    tuple[int, ...],
    int,
    ...
    """Type- and structure-check an HBP, returning its fields.

    Only the container structure is checked: the bundles themselves are
    neither encoded nor verified here (encoding happens in the leaf
    recomputation and verification is :func:`verify_hbapb`'s job). The
    bounds are ``0 < total < 2**64``, a non-empty ``indices`` tuple of
    strictly increasing indices in ``0 <= i < total`` and a siblings
    tuple whose entries are exactly 32 bytes. Wrong field types raise
    TypeError; illegal bounds or shapes raise ValueError. The two count
    couplings — bundles pairing one-to-one with indices and the sibling
    count the compact walk derives from ``total`` and ``indices`` — are
    not judged here: :func:`check_hbp` reports either mismatch as
    ``False``.
    """
    if not isinstance(proof, HBP):
        raise TypeError("proof must be an HBP instance")
    indices = proof.indices
    total = proof.total
    bundles = proof.bundles
    siblings = proof.siblings
    ...
    for bundle in bundles:
        if not isinstance(bundle, HBAPB):
            raise TypeError(
                "proof.bundles entries must be HBAPB instances"
            )
    ...
    for sibling in siblings:
        if len(sibling) != HBP_DIGEST_SIZE:
            raise ValueError(
                "proof.siblings entries must be exactly 32 bytes"
            )
    return indices, total, bundles, siblings
...
def check_hbp(
    ...
    The root ``signature`` and ``key`` structures are checked first.
    Each disclosed bundle's leaf digest is then recomputed from its
    position and the bundle exactly as in :func:`make_hbp`, and the
    root is rebuilt level by level while consuming ``proof.siblings`` in
    ascending level order and left to right within a level: the current
    level holds the digests of the proven nodes at their current
    positions, paired left to right; when a companion is itself a
    ...
    current width contracts as ``(width + 1) // 2`` and the rebuild must
    consume every sibling exactly. Every disclosed bundle is then
    re-checked with :func:`verify_hbapb` against ``key`` in index order,
    so each bundle's nested HBAP proof and every archive proof bundle it
    in turn discloses must verify, and the statement
    ``b"hbp/r" || U64(total) || root`` is finally checked as
    ``signature``'s threshold Schnorr message via
    :func:`verify_signature` with the key's group parameters. Returns
    ``True`` only when the rebuild consumes every sibling exactly and
    reaches the single signed root, every bundle verifies against the
    ...
    A non-:class:`HBP` or non-:class:`AggregateSignature` argument or
    any wrong field type (non-tuple indices/bundles/siblings, a
    non-integer ``total`` or index including booleans, a non-:class:`HBAPB`
    bundle or non-bytes sibling) raises TypeError; a non-positive or
    over-64-bit ``total``, empty indices, an index out of range or not
    strictly increasing, a sibling entry that is not exactly 32 bytes, or
    a structurally illegal bundle, signature or ``key`` raises ValueError,
    exactly as :func:`verify_hbapb` and :func:`verify_signature` would.
    """
    indices, total, bundles, siblings = _validate_hbp_structure(proof)
    if not isinstance(signature, AggregateSignature):
        raise TypeError("signature must be an AggregateSignature instance")
    _check_history_proof_bundle_signature(signature)
    ...
    try:
        ...
        raise ValueError("too many HBAPB archive proof bundles") from None

    nodes = {
        index: _hbp_leaf(index, bundle)
        for index, bundle in zip(indices, bundles)
    }
    pending = iter(siblings)
    width = total
    ...
    for bundle in bundles:
        if not verify_hbapb(bundle, key):
            return False
...
# ---------------------------------------------------------------------------
# Canonical HBPB transport: an HBP carried together with the aggregate
# threshold-Schnorr signature over its root statement — the same
# b"hbp/r" || U64(total) || root message make_hbp returns — as one
# self-delimiting, byte-for-byte reproducible byte string, mirroring the
# existing proof-plus-signature bundles. Decoding restores structure only —
# the nested bundles go through decode_hbapb, no bundle or signature is
# checked and no state is kept, so verify_hbpb remains the sole verifier
# afterwards.
# ---------------------------------------------------------------------------

HBPB_WIRE_TAG = b"ts/hbpb/v1"
...
class HBPB:
    ...
    The fields, in order, are ``proof`` (an :class:`HBP`) and
    ``signature`` (the :class:`AggregateSignature` on the proof's
    ``b"hbp/r" || U64(total) || root`` statement, the exact message
    :func:`make_hbp` returns). The dataclass is frozen, positionally
    constructible and compared by value, and carries no network, storage
    or hidden state. Field types and bounds are not checked at
    construction time — :func:`encode_hbpb` checks the structure and
    :func:`verify_hbpb` is the way to test a bundle afterwards.
    """
...
def encode_hbpb(bundle: HBPB) -> bytes:
    """Canonically encode an hbp bundle for transport or persistence.

    The encoding is the direct concatenation, in order, of the tag
    ``b"ts/hbpb/v1"``, ``VARINT(total)``, the index count as a 4-byte
    unsigned big-endian integer, one ``VARINT(index)`` per proven leaf in
    the proof's strictly increasing order, the bundle count as a 4-byte
    unsigned big-endian integer, one frame per bundle — the 4-byte
    unsigned big-endian length ``len(E)`` followed by the raw bytes
    ``E = encode_hbapb(bundle_value)`` (never empty) — the sibling count
    as a 4-byte unsigned big-endian integer followed by the raw 32-byte
    sibling digests in their original leaf-to-root, left-to-right order,
    and finally the signature frame: ``VARINT(R)``, ``VARINT(z)``, the
    4-byte unsigned big-endian signer count and one ``VARINT(id)`` per
    ...
    body length followed by the shortest unsigned big-endian value (zero
    is the single byte ``00``, positive values carry no leading zero);
    ``R`` must be positive and ``z`` may be zero. The integer and frame
    conventions are exactly those of the existing HBAPB transport
    bundle. The indices must be non-empty, strictly increasing, unique
    and within ``0 <= i < total``, the bundle count must equal the index
    count, and the sibling count must be the unique one derived from
    ``total`` and the indices by the compact multi-proof walk.

    Only a structurally legal :class:`HBPB` is accepted — a non-bundle
    or non-:class:`HBP` proof argument, or wrong proof/signature field
    types, raise TypeError and illegal bounds, an empty index tuple, a
    bundle tuple that does not pair one-to-one with the indices, a
    sibling that is not exactly 32 bytes, a sibling count other than the
    one ``total`` and the indices determine, a structurally illegal
    ...
    an empty or non-strictly-increasing signer id tuple) or an over-long
    frame raises ValueError — but the bundles are not verified and the
    signature is not checked against the root: :func:`verify_hbpb` stays
    the way to verify a bundle afterwards. The output for a given bundle
    is unique and the encoding carries no network, storage or hidden
    state.
    """
    if not isinstance(bundle, HBPB):
        raise TypeError("bundle must be an HBPB instance")
    if not isinstance(bundle.proof, HBP):
        raise TypeError("bundle.proof must be an HBP instance")
    indices, total, bundles, siblings = _validate_hbp_structure(
        bundle.proof
    )
    try:
        if len(bundles) != len(indices):
    ...
    encoded_bundles = []
    for position, bundle_value in enumerate(bundles):
        encoded_bundle = encode_hbapb(bundle_value)
        if len(encoded_bundle) > 0xFFFFFFFF:
            raise ValueError(
                f"hbp bundle {position + 1} encoding too long"
            )
        encoded_bundles.append(encoded_bundle)

    buffer = bytearray(HBPB_WIRE_TAG)
    buffer += _encode_varint(total)
    buffer += len(indices).to_bytes(4, "big", signed=False)
    for index in indices:
        buffer += _encode_varint(index)
...
def decode_hbpb(blob: bytes) -> HBPB:
    """Decode the canonical encoding produced by :func:`encode_hbpb`.

    Accepts only the single canonical form: the tag ``b"ts/hbpb/v1"``,
    the length-prefixed integer ``total`` (a 4-byte unsigned big-endian
    length followed by its shortest unsigned big-endian value, zero
    encoded as the single byte ``00``), the 4-byte non-zero index count
    followed by one canonical ``VARINT`` per strictly increasing index,
    the 4-byte bundle count (which must equal the index count) followed
    by that many non-empty bundle frames (a 4-byte non-zero length
    followed by bytes that :func:`decode_hbapb` accepts), the 4-byte
    sibling count followed by exactly that many raw 32-byte sibling
    digests, where the count must be the unique one derived from
    ``total`` and the disclosed indices, and finally the signature frame:
    the length-prefixed integers ``R`` and ``z`` with ``R`` positive, the
    ...
    legal bundle whose nested bundles do not verify or whose signature
    does not sign the root is returned normally, and
    :func:`verify_hbpb` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HBPB_WIRE_TAG):
        raise ValueError("bad HBAPB archive proof bundle tag")
    offset = len(HBPB_WIRE_TAG)

    total, offset = _read_varint(blob, offset, what="HBAPB archive proof total")
    if total <= 0:
        raise ValueError("HBAPB archive proof total must be positive")
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("HBAPB archive proof total too large")

    if offset + 4 > len(blob):
        raise ValueError("truncated HBAPB archive proof index count")
    index_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if index_count == 0:
        raise ValueError("HBAPB archive proof indices must be non-empty")
    indices = []
    for _ in range(index_count):
        index, offset = _read_varint(
            blob, offset, what="HBAPB archive proof index"
        )
        indices.append(index)

    if offset + 4 > len(blob):
        raise ValueError("truncated HBAPB archive proof bundle count")
    bundle_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if bundle_count != index_count:
        raise ValueError(
            "HBAPB archive proof bundle count must match the index count"
        )
    bundles = []
    for position in range(bundle_count):
        encoded_bundle, offset = _read_archive_block(
            blob,
            offset,
            what=f"HBAPB archive proof bundle {position + 1}",
        )
        bundles.append(decode_hbapb(encoded_bundle))

    if offset + 4 > len(blob):
        raise ValueError("truncated HBAPB archive proof sibling count")
    sibling_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    required_siblings = _required_history_multi_proof_siblings(
        total, tuple(indices)
    ...
    if sibling_count != required_siblings:
        raise ValueError(
            "HBAPB archive proof sibling count is not the one determined "
            "by total and indices"
        )
    siblings = []
    for _ in range(sibling_count):
        if offset + HBP_DIGEST_SIZE > len(blob):
            raise ValueError("truncated HBAPB archive proof sibling")
        siblings.append(
            bytes(blob[offset:offset + HBP_DIGEST_SIZE])
        )
        offset += HBP_DIGEST_SIZE

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HBAPB archive proof"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HBAPB archive proof bundle")
    ...
    _validate_hbp_structure(proof)
    bundle = HBPB(proof=proof, signature=signature)
    if encode_hbpb(bundle) != blob:
        raise ValueError("non-canonical HBAPB archive proof bundle encoding")
    return bundle


def verify_hbpb(bundle: HBPB, key: SigningDKGResult) -> bool:
    """Verify a bundle exactly as :func:`check_hbp` would.

    This is a convenience wrapper over
    ``check_hbp(bundle.proof, bundle.signature, key)``: the Merkle root
    is rebuilt from the proof, every disclosed bundle is re-checked with
    :func:`verify_hbapb` against ``key`` and the signature verified on
    the exact statement :func:`make_hbp` returns,
    ``b"hbp/r" || U64(total) || root``. Returns ``True`` only when all
    of them hold; a well-formed bundle with a tampered bundle, index,
    sibling or signature, a bundle swapped or altered, a missing or extra
    sibling, or one presented under another key, returns ``False``.

    A non-:class:`HBPB` argument raises TypeError; illegal nested proof,
    bundle, signature or key structure raises TypeError/ValueError,
    exactly as :func:`check_hbp` does.
    """
    if not isinstance(bundle, HBPB):
        raise TypeError("bundle must be an HBPB instance")
    return check_hbp(bundle.proof, bundle.signature, key)


# ---------------------------------------------------------------------------
# Canonical HBPB archive: several whole HBPB membership-proof transport
# bundles kept in their original order as one non-empty tuple, with one
# aggregate threshold-Schnorr signature authenticating both the bytes and
# their order over the archive's canonical framing. Mirrors the existing
# HBAPB archive layer one level up: the value is frozen and stateless, the
# message can be taken through the two-round threshold signing flow as is,
# and decoding restores structure only — every nested bundle goes through
# decode_hbpb, no bundle or signature is checked and no state is kept, so
# verify_hbpba remains the sole verifier afterwards.
# ---------------------------------------------------------------------------

HBPBA_MESSAGE_TAG = b"ts/hbpba/m1"
HBPBA_WIRE_TAG = b"ts/hbpba/w1"


@dataclass(frozen=True)
class HBPBArchive:
    """A non-empty ordered archive of HBPB bundles with one outer signature.

    The fields, in order, are ``items`` — a non-empty tuple of
    :class:`HBPB` values in archive order — and ``signature`` — the
    threshold Schnorr :class:`AggregateSignature` on
    :func:`hbpba_message` of the items and the verifying public key.
    The dataclass is frozen, positionally constructible and compared by
    value; neither field has a default, the items keep their order and
    the archive carries no network, storage or hidden state. Neither
    field is checked at construction time — :func:`hbpba_message`
    requires structurally legal items and :func:`verify_hbpba` is the
    way to test an archive against a key afterwards.
    """
...
def hbpba_message(
    items: tuple[HBPB, ...], public_key: int
) -> bytes:
    """Encode the canonical message the threshold key signs for an archive.

    The message is, in order, the tag ``b"ts/hbpba/m1"``, the 32-byte
    ``SHA256`` digest of the canonical framing ``C`` and
    ``V(public_key)`` — a 4-byte unsigned big-endian length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). With
    ``E_i = encode_hbpb(items[i])`` and ``n`` the item count, ``C`` is
    built in tuple order as ``U32(n) || Σ(U32(len(E_i)) || E_i)``;
    every ``U32`` is a 4-byte unsigned big-endian integer, the frames
    carry no separators and ``E_i`` is the existing canonical transport
    encoding of the bundle. The message carries no signature and keeps
    no state, and can be taken through the two-round threshold signing
    flow as is.

    ``items`` must be a non-empty tuple of structurally legal
    :class:`HBPB` values exactly as :func:`encode_hbpb` requires and
    ``public_key`` a non-boolean non-negative integer. Wrong field or
    nested field types raise TypeError — a non-tuple items sequence, an
    item that is not an :class:`HBPB`, or a non-integer (including
    boolean) public key; an empty items tuple, an illegal nested
    bundle, a negative public key or an over-long encoding raises
    ValueError.
    """
    ...
    for item in items:
        if not isinstance(item, HBPB):
            raise TypeError("each archive item must be an HBPB instance")
    if not isinstance(public_key, int) or isinstance(public_key, bool):
        raise TypeError("public_key must be an integer")
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HBPB archive item {index + 1} encoding too long"
            )
        frames.append(encoded)
...
def verify_hbpba(
    archive: HBPBArchive, key: SigningDKGResult
) -> bool:
    """Verify an archive of HBPB bundles against the threshold key.

    ``archive`` must be a structurally legal :class:`HBPBArchive` and
    ``key`` a legal :class:`SigningDKGResult`. The outer signature and
    the key structure are checked first, as is the structure of every
    item, so an illegal outer signature or key is never masked by a bad
    nested bundle: a non-:class:`HBPBArchive` argument, a non-tuple
    ``items`` field, a non-:class:`HBPB` element or a
    non-:class:`SigningDKGResult` ``key`` raises TypeError, and an
    empty items tuple, an illegal outer signature structure, an illegal
    key structure or any structurally illegal nested bundle raises
    ValueError.

    Every bundle is then passed to :func:`verify_hbpb` in archive
    order, so each disclosed HBP proof, the HBAPB bundles and nested
    proofs it in turn carries, and its root signature must verify under
    ``key``; the canonical :func:`hbpba_message` of the items and
    ``key.public_key`` is finally checked as the outer signature's
    threshold Schnorr message via :func:`verify_signature` with the
    key's group parameters. Returns ``True`` only when every
    per-bundle check and the outer signature check pass; a structurally
    ...
    if not all(verify_hbpb(item, key) for item in items):
        return False
    message = hbpba_message(items, public_key)
    return verify_signature(
        message,
        archive.signature,
        public_key,
...
def encode_hbpba(archive: HBPBArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HBPB bundles as
    one self-delimiting object for transport or persistence.

    The encoding is, in order, the tag ``b"ts/hbpba/w1"``, the 4-byte
    unsigned big-endian item count (never zero), then one frame per
    bundle in tuple order — each the 4-byte unsigned big-endian byte
    length (never zero) followed by the exact bytes of
    :func:`encode_hbpb` for that item, with no further separators —
    and finally the outer signature frame an :class:`AuditProofBundle`
    carries: ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned
    big-endian signer count ``k`` and one ``VARINT(id)`` per ascending
    signer id. Every U32 is a 4-byte unsigned big-endian integer; a
    ...
    otherwise reordered, the outer signature is not matched against the
    bundles, no inner or outer signature is checked and no state is
    read: :func:`verify_hbpba` stays the way to test an archive
    against a key afterwards. A non-:class:`HBPBArchive` argument, a
    non-tuple ``items`` field or a non-:class:`HBPB` element raises
    TypeError, as do wrong field types nested inside a bundle and a
    non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame,
    any structural error a bundle would raise on its own, or an
    ...
    buffer = bytearray(HBPBA_WIRE_TAG)
    buffer += item_count.to_bytes(4, "big", signed=False)
    for encoded in frames:
        buffer += len(encoded).to_bytes(4, "big", signed=False)
        buffer += encoded
...
def decode_hbpba(blob: bytes) -> HBPBArchive:
    """Decode the canonical encoding produced by :func:`encode_hbpba`.

    Accepts only the single canonical form: the tag
    ``b"ts/hbpba/w1"``, a 4-byte unsigned big-endian non-zero item
    count ``n``, then exactly ``n`` frames, each a 4-byte unsigned
    big-endian non-zero byte length followed by the exact canonical
    encoding of one :class:`HBPB` that :func:`decode_hbpb` accepts,
    and finally the outer signature frame: the length-prefixed integers
    ``R`` and ``z`` with ``R`` positive, the 4-byte non-zero signer
    count ``k`` and then exactly ``k`` strictly increasing positive
    signer ids. The bundles are restored in their original order as a
    tuple; they are neither sorted nor matched against one another or
    against the outer signature, and no signature is verified —
    :func:`verify_hbpba` judges the archive afterwards. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero item
    count, a zero or over-long frame length, a declared count that does
    not match the number of frames, a nested bundle encoding that is
    illegal or non-canonical (including a frame that would not
    ...
    legal archive whose bundles do not verify or whose outer signature
    does not sign the bundles is returned normally, and
    :func:`verify_hbpba` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HBPBA_WIRE_TAG):
        raise ValueError("bad HBPB archive tag")
    offset = len(HBPBA_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError("truncated HBPB archive item count")
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError("HBPB archive items must be non-empty")
    ...
    for index in range(item_count):
        frame, offset = _read_archive_block(
            blob,
            offset,
            what=f"HBPB archive item {index + 1}",
        )
        items.append(decode_hbpb(frame))

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HBPB archive"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HBPB archive")

    archive = HBPBArchive(items=tuple(items), signature=signature)
...
# ---------------------------------------------------------------------------
# Canonical HPB archive: several whole HPB membership-proof transport
# bundles kept in their original order as one non-empty tuple, with one
# aggregate threshold-Schnorr signature authenticating both the bytes and
# their order over the archive's canonical framing. Mirrors the existing
# HBPB archive layer one level down: the value is frozen and stateless, the
# message can be taken through the two-round threshold signing flow as is,
# and decoding restores structure only — every nested bundle goes through
# decode_hpb, no bundle or signature is checked and no state is kept, so
# verify_hpba remains the sole verifier afterwards.
# ---------------------------------------------------------------------------

HPBA_MESSAGE_TAG = b"ts/hpba/m1"
HPBA_WIRE_TAG = b"ts/hpba/w1"


@dataclass(frozen=True)
class HPBArchive:
    """A non-empty ordered archive of HPB bundles with one outer signature.

    The fields, in order, are ``items`` — a non-empty tuple of
    :class:`HPB` values in archive order — and ``signature`` — the
    threshold Schnorr :class:`AggregateSignature` on
    :func:`hpba_message` of the items and the verifying public key.
    The dataclass is frozen, positionally constructible and compared by
    value; neither field has a default, the items keep their order and
    the archive carries no network, storage or hidden state. Neither
    field is checked at construction time — :func:`hpba_message`
    requires structurally legal items and :func:`verify_hpba` is the
    way to test an archive against a key afterwards.
    """
...
def hpba_message(
    items: tuple[HPB, ...], public_key: int
) -> bytes:
    """Encode the canonical message the threshold key signs for an archive.

    The message is, in order, the tag ``b"ts/hpba/m1"``, the 32-byte
    ``SHA256`` digest of the canonical framing ``C`` and
    ``V(public_key)`` — a 4-byte unsigned big-endian length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero). With
    ``E_i = encode_hpb(items[i])`` and ``n`` the item count, ``C`` is
    built in tuple order as ``U32(n) || Σ(U32(len(E_i)) || E_i)``;
    every ``U32`` is a 4-byte unsigned big-endian integer, the frames
    carry no separators and ``E_i`` is the existing canonical transport
    encoding of the bundle. The message carries no signature and keeps
    ...
    ``items`` must be a non-empty tuple of structurally legal
    :class:`HPB` values exactly as :func:`encode_hpb` requires and
    ``public_key`` a non-boolean non-negative integer. Wrong field or
    nested field types raise TypeError — a non-tuple items sequence, an
    item that is not an :class:`HPB`, or a non-integer (including
    boolean) public key; an empty items tuple, an illegal nested
    bundle, a negative public key or an over-long encoding raises
    ValueError.
    """
    ...
    for item in items:
        if not isinstance(item, HPB):
            raise TypeError("each archive item must be an HPB instance")
    if not isinstance(public_key, int) or isinstance(public_key, bool):
        raise TypeError("public_key must be an integer")
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HPB archive item {index + 1} encoding too long"
            )
        frames.append(encoded)
...
def verify_hpba(
    archive: HPBArchive, key: SigningDKGResult
) -> bool:
    """Verify an archive of HPB bundles against the threshold key.

    ``archive`` must be a structurally legal :class:`HPBArchive` and
    ``key`` a legal :class:`SigningDKGResult`. The outer signature and
    the key structure are checked first, as is the structure of every
    item, so an illegal outer signature or key is never masked by a bad
    nested bundle: a non-:class:`HPBArchive` argument, a non-tuple
    ``items`` field, a non-:class:`HPB` element or a
    non-:class:`SigningDKGResult` ``key`` raises TypeError, and an
    empty items tuple, an illegal outer signature structure, an illegal
    key structure or any structurally illegal nested bundle raises
    ValueError.

    Every bundle is then passed to :func:`verify_hpb` in archive
    order, so each disclosed HP membership proof, the HBPB bundles it
    carries and its root signature must verify under ``key``; the
    canonical :func:`hpba_message` of the items and
    ``key.public_key`` is finally checked as the outer signature's
    threshold Schnorr message via :func:`verify_signature` with the
    key's group parameters. Returns ``True`` only when every
    per-bundle check and the outer signature check pass; a structurally
    ...
    if not all(verify_hpb(item, key) for item in items):
        return False
    message = hpba_message(items, public_key)
    return verify_signature(
        message,
        archive.signature,
        public_key,
...
def encode_hpba(archive: HPBArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HPB bundles as
    one self-delimiting object for transport or persistence.

    The encoding is, in order, the tag ``b"ts/hpba/w1"``, the 4-byte
    unsigned big-endian item count (never zero), then one frame per
    bundle in tuple order — each the 4-byte unsigned big-endian byte
    length (never zero) followed by the exact bytes of
    :func:`encode_hpb` for that item, with no further separators —
    and finally the outer signature frame an :class:`AuditProofBundle`
    carries: ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned
    big-endian signer count ``k`` and one ``VARINT(id)`` per ascending
    signer id. Every U32 is a 4-byte unsigned big-endian integer; a
    ...
    otherwise reordered, the outer signature is not matched against the
    bundles, no inner or outer signature is checked and no state is
    read: :func:`verify_hpba` stays the way to test an archive
    against a key afterwards. A non-:class:`HPBArchive` argument, a
    non-tuple ``items`` field or a non-:class:`HPB` element raises
    TypeError, as do wrong field types nested inside a bundle and a
    non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame,
    any structural error a bundle would raise on its own, or an
    ...
    buffer = bytearray(HPBA_WIRE_TAG)
    buffer += item_count.to_bytes(4, "big", signed=False)
    for encoded in frames:
        buffer += len(encoded).to_bytes(4, "big", signed=False)
        buffer += encoded
...
def decode_hpba(blob: bytes) -> HPBArchive:
    """Decode the canonical encoding produced by :func:`encode_hpba`.

    Accepts only the single canonical form: the tag
    ``b"ts/hpba/w1"``, a 4-byte unsigned big-endian non-zero item
    count ``n``, then exactly ``n`` frames, each a 4-byte unsigned
    big-endian non-zero byte length followed by the exact canonical
    encoding of one :class:`HPB` that :func:`decode_hpb` accepts,
    and finally the outer signature frame: the length-prefixed integers
    ``R`` and ``z`` with ``R`` positive, the 4-byte non-zero signer
    count ``k`` and then exactly ``k`` strictly increasing positive
    signer ids. The bundles are restored in their original order as a
    tuple; they are neither sorted nor matched against one another or
    against the outer signature, and no signature is verified —
    :func:`verify_hpba` judges the archive afterwards. A non-bytes
    argument raises TypeError; a wrong or missing tag, a zero item
    count, a zero or over-long frame length, a declared count that does
    not match the number of frames, a nested bundle encoding that is
    illegal or non-canonical (including a frame that would not
    ...
    Decoding only restores structure: every nested bundle is decoded
    with :func:`decode_hpb` (which verifies no proof and checks no
    signature) and the outer signature is not checked. A structurally
    legal archive whose bundles do not verify or whose outer signature
    does not sign the bundles is returned normally, and
    :func:`verify_hpba` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HPBA_WIRE_TAG):
        raise ValueError("bad HPB archive tag")
    offset = len(HPBA_WIRE_TAG)

    if offset + 4 > len(blob):
        raise ValueError("truncated HPB archive item count")
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError("HPB archive items must be non-empty")
    ...
    for index in range(item_count):
        frame, offset = _read_archive_block(
            blob,
            offset,
            what=f"HPB archive item {index + 1}",
        )
        items.append(decode_hpb(frame))

    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="HPB archive"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after HPB archive")

    archive = HPBArchive(items=tuple(items), signature=signature)
