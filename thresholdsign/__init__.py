...
the canonical archive transport encoding encode_hdscapa / decode_hdscapa
for transferring or persisting a whole archive as one self-delimiting
byte string, and the
...
# ---------------------------------------------------------------------------
# Archives of HDSC archive proof bundles: a non-empty, order-preserving
# batch of whole HDSCArchiveProofBundle values authenticated by one more
# threshold Schnorr signature. The outer signature binds the digest of a
# canonical framing C over the existing per-bundle transport encodings and
# the verifying public key, so deleting, inserting, reordering or
# substituting a bundle — or presenting the archive under another key —
# invalidates it. The archive carries no state of its own: every bundle is
# re-checked with verify_hdsc_archive_proof_bundle (and, through it, every
# disclosed HDSC proof bundle and seal) on verification before the outer
# signature is examined via verify_signature.
# ---------------------------------------------------------------------------
...
class HDSCArchiveProofBundleArchive:
    """A non-empty ordered archive of HDSC archive proof bundles with one outer signature.
    ...
    The dataclass is frozen, positionally constructible and compared by
    value; the items keep their order and the archive carries no network,
    storage or hidden state. Neither field is checked at construction
    time — :func:`hdscapa_message` requires structurally legal items and
...
def hdscapa_message(
    ...
    :func:`encode_hdsc_archive_proof_bundle` requires and ``public_key``
    a non-boolean non-negative integer. Wrong field or nested field types
    raise TypeError — a non-tuple items sequence, an item that is not an
    :class:`HDSCArchiveProofBundle`, or a non-integer (including boolean)
    public key; an empty items tuple, an illegal nested bundle, a
    negative public key or an over-long encoding raises ValueError.
    """
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    for item in items:
    ...
    for index, item in enumerate(items):
        ...
        if len(encoded) > 0xFFFFFFFF:
            raise ValueError(
                f"HDSC archive proof bundle archive item {index + 1} encoding too long"
            )
        frames.append(encoded)
...
def verify_hdscapa(
    ...
    """Verify an archive of HDSC archive proof bundles against the threshold key.
    ...
    Every bundle is then passed to :func:`verify_hdsc_archive_proof_bundle`
    in archive order, so each disclosed HDSC proof bundle, each disclosed
    seal and each root signature must verify under ``key``; the canonical
    :func:`hdscapa_message` of the items and ``key.public_key`` is
    finally checked as the outer signature's threshold Schnorr message
    via :func:`verify_signature` with the key's group parameters. Returns
    ``True`` only when every per-bundle check and the outer signature
    check pass; a structurally legal archive whose nested bundle or outer
    signature does not match — including deleting, inserting, reordering
    or substituting a bundle, or presenting the archive under another key
    — returns ``False`` rather than raising. The function is stateless.
    """
    ...
    if not all(verify_hdsc_archive_proof_bundle(item, key) for item in items):
        return False
...
def encode_hdscapa(archive: HDSCArchiveProofBundleArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of HDSC archive proof
    bundles as one self-delimiting object for transport or persistence.

    The encoding is, in order, the tag ``b"ts/hdscapa/w1"``, the 4-byte
    unsigned big-endian item count (never zero), then one frame per
    bundle in tuple order — each the 4-byte unsigned big-endian byte
    length followed by the exact bytes of
    ...
    otherwise reordered, the outer signature is not matched against the
    bundles, no inner or outer signature is checked and no state is
    read: :func:`verify_hdscapa` stays the way to test an archive
    against a key afterwards. A
    non-:class:`HDSCArchiveProofBundleArchive` argument, a non-tuple
    ``items`` field or a non-:class:`HDSCArchiveProofBundle` element
    raises TypeError, as do wrong field types nested inside a bundle and
    a non-:class:`AggregateSignature` outer signature or one with wrong
    field types. An empty items tuple, an over-long count or frame, any
    structural error a bundle would raise on its own, or an illegal
    outer signature structure raises ValueError. The output for a given
    archive is unique and the encoding carries no network, storage or
...
def decode_hdscapa(blob: bytes) -> HDSCArchiveProofBundleArchive:
    ...
    outer signature frame: the length-prefixed integers ``R`` and ``z``
    with ``R`` positive, the 4-byte non-zero signer count ``k`` and then
    exactly ``k`` strictly increasing positive signer ids. The bundles
    are restored in their original order as a tuple; they are neither
    sorted nor matched against one another or against the outer
    signature, and no signature is verified — :func:`verify_hdscapa`
    judges the archive afterwards. A non-bytes argument raises
    TypeError; a wrong or missing tag, a zero item count, a zero or
    over-long frame length, a count mismatch, a nested bundle encoding
    that is illegal or non-canonical (including a frame that would not
    ...
    checked. A structurally legal archive whose bundles do not verify or
    whose outer signature does not sign the bundles is returned
    ...
    if offset + 4 > len(blob):
        raise ValueError("truncated HDSC archive proof bundle archive item count")
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count == 0:
        raise ValueError("HDSC archive proof bundle archive items must be non-empty")
    ...
    for index in range(item_count):
        frame, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"HDSC archive proof bundle archive item {index + 1}",
        )
    ...
    if offset != len(blob):
        raise ValueError("trailing bytes after HDSC archive proof bundle archive")

    archive = HDSCArchiveProofBundleArchive(items=tuple(items), signature=signature)
    if encode_hdscapa(archive) != blob:
        raise ValueError("non-canonical HDSC archive proof bundle archive encoding")
    return archive
