...
decode_audit_proof_bundle / verify_audit_proof_bundle, and the
multi-proof-plus-signature bundle AuditMultiProofBundle /
encode_audit_multi_proof_bundle / decode_audit_multi_proof_bundle /
verify_audit_multi_proof_bundle. Append-only consistency proofs over the same tree:
...
# are not parsed and no signature is checked, so
# verify_audit_multi_proof_bundle remains the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
class AuditMultiProofBundle:
    """A compact multi-record audit proof together with its root signature.
    ...
    positionally constructible and compared by value, and carries no
    network, storage or hidden state. Field types and bounds are not
    checked at construction time — :func:`encode_audit_multi_proof_bundle`
    checks the structure and :func:`verify_audit_multi_proof_bundle` is
...
def encode_audit_multi_proof_bundle(bundle: AuditMultiProofBundle) -> bytes:
    ...
    unsigned big-endian length ``len(P)`` followed by
    ``P = encode_audit_multi_proof(bundle.proof)`` (never empty), and
    ...
    Only a structurally legal :class:`AuditMultiProofBundle` is
    accepted — a non-bundle or non-:class:`AuditMultiProof` proof
    ...
    if not isinstance(bundle, AuditMultiProofBundle):
        raise TypeError(
            "bundle must be an AuditMultiProofBundle instance"
        )
...
def decode_audit_multi_proof_bundle(
    blob: bytes,
) -> AuditMultiProofBundle:
    ...
    if R == 0:
        raise ValueError(
            "audit multi-proof bundle signature R must be positive"
        )

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated audit multi-proof bundle signer count"
        )
    signer_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if signer_count == 0:
        raise ValueError(
            "audit multi-proof bundle must name at least one signer"
        )
    ...
    if offset != len(blob):
        raise ValueError(
            "trailing bytes after audit multi-proof bundle"
        )
    ...
    if encode_audit_multi_proof_bundle(bundle) != blob:
        raise ValueError(
            "non-canonical audit multi-proof bundle encoding"
        )
    return bundle
...
def verify_audit_multi_proof_bundle(
    ...
    This is a convenience wrapper over
    ``check_multi_proof(bundle.proof, bundle.signature, key)``: every
    disclosed record is re-checked with :func:`check_audit`, the Merkle
    root is rebuilt from the proof and the signature verified on
    ``b"am/r" || U64(n) || root``. Returns ``True`` only when all of
    them hold; a well-formed bundle with tampered records, indices,
    siblings or signature, or one presented under another key, returns
    ``False``.

    A non-:class:`AuditMultiProofBundle` argument raises
    TypeError; illegal nested proof, signature or key structure raises
    TypeError/ValueError, exactly as
    :func:`check_multi_proof` does.
    """
    if not isinstance(bundle, AuditMultiProofBundle):
        raise TypeError(
            "bundle must be an AuditMultiProofBundle instance"
