...
decode_hdsc_proof_bundle / verify_hdsc_proof_bundle, and the non-empty
order-preserving archive HDSCProofBundleArchive of such bundles, whose
outer signature binds hdsc_proof_bundle_archive_message over the
existing canonical bundle encodings and the verifying public key, with
the key-bound verifier verify_hdsc_proof_bundle_archive that re-checks
every bundle with verify_hdsc_proof_bundle before the outer
...
class HDSCProofBundleArchive:
    ...
    field is checked at construction time —
    :func:`hdsc_proof_bundle_archive_message` requires structurally
    legal items and :func:`verify_hdsc_proof_bundle_archive` is the way
    to test an archive against a key afterwards.
    """
...
def hdsc_proof_bundle_archive_message(
    ...
    :class:`HDSCProofBundle`, or a non-integer (including boolean)
...
def verify_hdsc_proof_bundle_archive(
    ...
    field, a non-:class:`HDSCProofBundle` element or
    a non-:class:`SigningDKGResult` ``key`` raises TypeError, and an
...
def encode_hdsc_proof_bundle_archive(archive: HDSCProofBundleArchive) -> bytes:
    """Canonically encode a non-empty ordered archive of hdsc proof
    ...
    :class:`HDSCProofBundle` carries: ``VARINT(R)``, ``VARINT(z)``, the
    4-byte unsigned big-endian signer count ``k`` and one ``VARINT(id)``
    per ascending signer id. Every U32 is a 4-byte unsigned big-endian
    integer; a ``VARINT`` is a 4-byte unsigned big-endian body length
    followed by the shortest unsigned big-endian value (zero is the
    single byte ``00``, positive values carry no leading zero); ``R``
    must be positive, ``z`` may be zero and the signer ids are a
    non-empty strictly increasing tuple of positive integers.
