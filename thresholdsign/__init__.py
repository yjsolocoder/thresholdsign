...
# ---------------------------------------------------------------------------
# Compact multi-bundle membership proofs over an SMP bundle archive: one
# root signature certifies that several disclosed SMPBundle values belong,
# at fixed positions, to the exact non-empty bundle sequence an
# SMPBundleArchive carries. The tree is SHA256 with its own
# domain-separated leaf/node tags over the existing canonical bundle
# transport encodings; the root statement to sign is
# ``b"hamp/r" || U64(total) || root``. An HAMP carries only the companion
# digests that are not themselves disclosed (and never the odd-tail
# self-pair), so the whole set can be audited from one signature without
# hidden state, and check_hamp re-runs verify_smp_bundle on every
# disclosed bundle and verify_signature on the root signature afterwards.
# The archive's own outer signature is never consulted by make_hamp.
# ---------------------------------------------------------------------------
...
HAMP_DIGEST_SIZE = 32  # SHA256 output width; every tree node is this wide
...
def _hamp_leaf(index: int, encoded_bundle: bytes) -> bytes:
    """The index-bound leaf digest ``H(b"hamp/l" || U64(j) || H(E_j))``."""
    return hashlib.sha256(
        HAMP_LEAF_TAG
        + _hamp_u64(index)
        + hashlib.sha256(encoded_bundle).digest()
    ).digest()
...
def _hamp_levels(encoded_bundles: tuple[bytes, ...]) -> list[tuple[bytes, ...]]:
    """Build the leaf level and every internal level up to the single root.
    ...
    levels: list[tuple[bytes, ...]] = [
        tuple(
            _hamp_leaf(index, encoded_bundle)
            for index, encoded_bundle in enumerate(encoded_bundles)
        )
    ]
    current = levels[0]
    while len(current) > 1:
...
def _check_hamp_archive_items(
    ...
    to ``items``: a tuple of :class:`SMPBundle` instances, non-empty,
    each structurally legal through its own canonical encoder. The
    archive's outer signature is deliberately outside the scope — the
    proof is built from the bundles alone and make_hamp never examines
    any signature outside the proof. Nothing here verifies a signature.
    """
    if not isinstance(items, tuple):
        raise TypeError("items must be a tuple")
    for item in items:
        if not isinstance(item, SMPBundle):
            raise TypeError(
                "each archive item must be an SMPBundle instance"
            )
    if len(items) == 0:
        raise ValueError("archive items must be a non-empty tuple")
    # Encode every bundle first — each call raises TypeError/ValueError
...
class HAMP:
    ...
    (the proven :class:`SMPBundle` values, one per entry of ``indices``,
    ...
    and left to right within a level; the tuple is empty when no
    companion digests are needed). At a level whose width is odd, the
    ...
    indices: tuple[int, ...]
    total: int
    bundles: tuple[SMPBundle, ...]
    siblings: tuple[bytes, ...]


def make_hamp(
    archive: SMPBundleArchive, indices: tuple[int, ...]
) -> tuple[bytes, HAMP]:
    """Build the root statement and a compact multi-bundle membership proof.

    ``archive`` must be an :class:`SMPBundleArchive` whose items are a
    non-empty tuple of structurally legal :class:`SMPBundle` values
    exactly as :func:`encode_smp_bundle` requires, and ``indices`` a
    ...
    in archive order with SHA256, exactly isomorphic to the existing
    attribution proof trees: leaves are
    ``H(b"hamp/l" || U64(i) || H(encode_smp_bundle(bundle)))`` and
    internal nodes ``H(b"hamp/n" || left || right)``; a level with an
    ...
    ``proof`` is the :class:`HAMP` whose ``bundles`` pair one to one
    with ``indices``. Its ``siblings`` are collected level by level, in
    ascending level order (leaves upward) and left to right within a
    level, one entry per needed companion: when the companion position
    is itself a proven node carried in the proof, or the node is the
    last member of an odd-width level (paired with itself), no sibling
    is appended; otherwise the companion digest is. Proven nodes from
    lower levels feed the level above exactly as in the tree, so no
    digest is sent twice. One threshold signature over the statement
    covers every index subset of the same archive.

    Wrong argument types raise TypeError: a
    non-:class:`SMPBundleArchive` archive, a non-tuple index sequence or
    a non-integer (including boolean) index. An empty archive or index
    tuple, a structurally illegal nested bundle, a count at or above
    ``2**64``, indices that are not strictly increasing and unique, or
    an index outside ``0 <= i < total`` raises ValueError.
    """
    if not isinstance(archive, SMPBundleArchive):
        raise TypeError(
            "archive must be an SMPBundleArchive instance"
    ...
    encoded_bundles = tuple(encode_smp_bundle(item) for item in items)
    levels = _hamp_levels(encoded_bundles)
    ...
    proof = HAMP(
        indices=tuple(indices),
        total=total,
        bundles=tuple(items[index] for index in indices),
        siblings=tuple(siblings),
    )
...
def _validate_hamp_structure(
    ...
    """Type- and structure-check a :class:`HAMP`, returning its fields.
    ...
    recomputation and verification is :func:`verify_smp_bundle`'s job).
    The bounds are ``0 < total < 2**64``, a non-empty ``indices`` tuple
    of strictly increasing indices in ``0 <= i < total`` and a
    ``siblings`` tuple whose entries are exactly 32 bytes. Wrong field
    types raise TypeError; illegal bounds or shapes raise ValueError.
    The two count couplings — bundles pairing one-to-one with indices
    and the sibling count the compact walk derives from ``total`` and
    ``indices`` — are not judged here: :func:`check_hamp` reports either
    mismatch as ``False``.
    """
    if not isinstance(proof, HAMP):
        raise TypeError("proof must be a HAMP instance")
    indices = proof.indices
    total = proof.total
    ...
    for bundle in bundles:
        if not isinstance(bundle, SMPBundle):
            raise TypeError(
                "proof.bundles entries must be SMPBundle instances"
            )
...
def check_hamp(
    ...
    root is rebuilt level by level while consuming ``proof.siblings``
    in ascending level order and left to right within a level: the
    current level holds the digests of the proven nodes at their
    current positions, paired left to right; when a companion is itself
    a current-level node its digest is used directly, when the node is
    the last member of an odd-width level it is paired with itself, and
    otherwise the next 32-byte entry of ``siblings`` is consumed. The
    current width contracts as ``(width + 1) // 2`` and the rebuild
    must consume every sibling exactly. Every disclosed bundle is then
    re-checked with :func:`verify_smp_bundle` against ``key`` in index
    order, so each bundle's disclosed seals and its root signature must
    verify, and the statement ``b"hamp/r" || U64(total) || root`` is
    ...
    misplaced sibling, bundle/index count mismatches — or which is
    presented under another key, returns ``False`` rather than raising.
    ...
    non-:class:`SMPBundle` bundle or non-bytes sibling) raises
    ...
    :func:`verify_smp_bundle` and :func:`verify_signature` would.
    """
    ...
    nodes = {
        index: _hamp_leaf(index, encode_smp_bundle(bundle))
    ...
    for bundle in bundles:
        if not verify_smp_bundle(bundle, key):
            return False

    signed_message = (
        HAMP_ROOT_TAG + _hamp_u64(total) + nodes[0]
    )
    return verify_signature(
        signed_message,
        signature,
...
# ---------------------------------------------------------------------------
# Canonical HAMPB transport: a HAMP carried together with the aggregate
...
# existing attribution proof bundle transport encodings. Decoding
# restores structure only — the nested bundles go through
# decode_smp_bundle, no bundle or signature is checked and no state is
# kept, so check_hamp remains the sole verifier afterwards.
# ---------------------------------------------------------------------------

HAMP_BUNDLE_WIRE_TAG = b"ts/hampb/v1"
...
class HAMPB:
    ...
    The fields, in order, are ``proof`` (a :class:`HAMP`) and
    ``signature`` (the :class:`AggregateSignature` on the proof's
    ``b"hamp/r" || U64(total) || root`` statement, the exact message
    :func:`make_hamp` returns). The dataclass is frozen, positionally
    constructible and compared by value, and carries no network,
    storage or hidden state. Field types and bounds are not checked at
    construction time — :func:`encode_hampb` checks the structure and
    :func:`check_hamp` is the way to test a bundle afterwards.
    """
...
def encode_hampb(bundle: HAMPB) -> bytes:
    """Canonically encode a HAMP bundle for transport/storage.

    The segment order and framing follow the existing attribution proof
    bundle transport encodings exactly: the tag ``b"ts/hampb/v1"``,
    ``VARINT(total)``, the index count as a 4-byte unsigned big-endian
    integer, one ``VARINT(index)`` per proven leaf in the proof's
    strictly increasing order, the bundle count as a 4-byte unsigned
    big-endian integer, one frame per bundle — the 4-byte unsigned
    big-endian length ``len(E)`` followed by the raw bytes
    ``E = encode_smp_bundle(bundle_value)`` (never empty), i.e. each
    nested bundle's existing canonical transport encoding — the
    sibling count as a 4-byte unsigned big-endian integer followed by
    the raw 32-byte sibling digests in their original leaf-to-root,
    left-to-right order, and finally the signature frame:
    ``VARINT(R)``, ``VARINT(z)``, the 4-byte unsigned big-endian
    signer count and one ``VARINT(id)`` per ascending signer id. A
    ``VARINT`` is a 4-byte unsigned big-endian body length followed by
    the shortest unsigned big-endian value (zero is the single byte
    ``00``, positive values carry no leading zero); ``R`` must be
    positive and ``z`` may be zero. The indices must be non-empty,
    strictly increasing, unique and within ``0 <= i < total``, the
    bundle count must equal the index count, and the sibling count
    must be the unique one derived from ``total`` and the indices by
    the compact multi-proof walk.
    ...
    sibling that is not exactly 32 bytes, a sibling count other than
    the one ``total`` and the indices determine, a structurally
    illegal nested bundle or signature (a non-positive ``R``, a
    negative ``z``, an empty or non-strictly-increasing signer id
    tuple) or an over-long frame raises ValueError — but the bundles
    are not verified and the signature is not checked against the
    root: :func:`check_hamp` stays the way to verify a bundle
    afterwards. The output for a given bundle is unique and the
    encoding is stateless.
    """
    if not isinstance(bundle, HAMPB):
        raise TypeError("bundle must be a HAMPB instance")
    if not isinstance(bundle.proof, HAMP):
        raise TypeError("bundle.proof must be a HAMP instance")
    ...
    for position, bundle_value in enumerate(bundles):
        encoded_bundle = encode_smp_bundle(bundle_value)
    ...
    buffer = bytearray(HAMP_BUNDLE_WIRE_TAG)
...
def decode_hampb(blob: bytes) -> HAMPB:
    ...
    Accepts only the single canonical form: the tag
    ``b"ts/hampb/v1"``, the length-prefixed integer ``total`` (a
    ...
    strictly increasing index, the 4-byte bundle count (which must
    equal the index count) followed by that many non-empty bundle
    frames (a 4-byte non-zero length followed by bytes that
    :func:`decode_smp_bundle` accepts — each bundle's existing
    canonical transport encoding), the 4-byte sibling count followed
    by exactly that many raw 32-byte sibling digests, where the count
    must be the unique one derived from ``total`` and the disclosed
    ...
    signer ids. A non-bytes argument raises TypeError; a wrong or
    missing tag, an empty index list, a non-positive or over-64-bit
    ``total``, an index outside ``0 <= i < total``, indices that are
    not strictly increasing and unique, a bundle count that does not
    match the index count, an empty, truncated or non-canonical bundle
    frame, a missing or extra sibling relative to the count ``total``
    and the indices require, a sibling that is not exactly 32 bytes, a
    ...
    Decoding only restores the structure: each nested bundle is
    decoded with :func:`decode_smp_bundle` (which verifies no seal and
    ...
    whose signature does not sign the root is returned normally and
    :func:`check_hamp` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
    if not blob.startswith(HAMP_BUNDLE_WIRE_TAG):
        raise ValueError("bad hamp tag")
    offset = len(HAMP_BUNDLE_WIRE_TAG)

    total, offset = _read_varint(blob, offset, what="hamp total")
    if total <= 0:
        raise ValueError("hamp total must be positive")
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("hamp total too large")

    if offset + 4 > len(blob):
        raise ValueError("truncated hamp index count")
    index_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if index_count == 0:
        raise ValueError("hamp indices must be non-empty")
    indices = []
    for _ in range(index_count):
        index, offset = _read_varint(blob, offset, what="hamp index")
        if indices and index <= indices[-1]:
            raise ValueError(
                "hamp indices must be strictly increasing and unique"
            )
        if index < 0 or index >= total:
            raise ValueError("hamp index out of range")
        indices.append(index)

    if offset + 4 > len(blob):
        raise ValueError("truncated hamp bundle count")
    bundle_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if bundle_count != index_count:
        raise ValueError(
            "hamp bundle count must match the index count"
        )
    bundles = []
    for position in range(bundle_count):
        encoded_bundle, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"hamp bundle {position + 1}",
        )
        bundles.append(decode_smp_bundle(encoded_bundle))

    if offset + 4 > len(blob):
        raise ValueError("truncated hamp sibling count")
    ...
    if sibling_count != required_siblings:
        raise ValueError(
            "hamp sibling count is not the one determined by total "
            "and indices"
        )
    siblings = []
    for _ in range(sibling_count):
        if offset + HAMP_DIGEST_SIZE > len(blob):
            raise ValueError("truncated hamp sibling")
        siblings.append(
    ...
    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="hamp"
    )
    if offset != len(blob):
        raise ValueError("trailing bytes after hamp")
    ...
    if encode_hampb(bundle) != blob:
        raise ValueError("non-canonical hamp encoding")
    return bundle
