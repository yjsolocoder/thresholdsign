...
# ---------------------------------------------------------------------------
# Compact multi-bundle membership proofs over the non-empty order-preserving
# HAMPB archive: one threshold-Schnorr root signature attests any disclosed
# subset of the archive's bundles. The tree is isomorphic to the HAMP
# membership proof tree — leaves bind the position and the bundle's existing
# canonical encode_hampb encoding, an odd tail pairs with itself — and the
# HBAPB transport carries the proof together with the aggregate signature
# over its root statement as one self-delimiting, byte-for-byte reproducible
# byte string. Decoding restores structure only and no state is kept, so
# check_hbap / verify_hbapb remain the sole verifiers.
# ---------------------------------------------------------------------------
...
class HBAP:
    ...
    the same order) and ``siblings`` (the 32-byte sibling digests
    consumed from the leaf level up to the root, in ascending level order
    and left to right within a level; the tuple is empty when no sibling
    digests are needed). At a level whose width is odd, the last node
    pairs with itself and no sibling is carried for it, and a companion
    that is itself a disclosed node is never carried either; the number
    and order of siblings are therefore fixed uniquely by ``total`` and
    ``indices``. The dataclass is frozen, positionally constructible and
    compared by value, and carries no network, storage or hidden state.
    Field types and bounds are not checked at construction time —
    :func:`check_hbap` is the way to test a proof afterwards.
    """
...
def make_hbap(
    ...
    leaves are
    ``H(b"hbap/l" || U64(i) || H(encode_hampb(bundle)))`` and internal
    nodes ``H(b"hbap/n" || left || right)``; a level with an odd tail
    width duplicates its last node for pairing, and positions and counts
    are encoded as 8-byte unsigned big-endian integers. Returns
    ``(message, proof)`` where ``message`` is
...
def check_hbap(
    ...
    so each bundle's inner HAMP proof and its root signature must
    verify, and the statement ``b"hbap/r" || U64(total) || root`` is
    finally checked as ``signature``'s threshold Schnorr message via
    :func:`verify_signature` with the key's group parameters. Returns
    ``True`` only when the rebuild consumes every sibling exactly and
    reaches the single signed root, every bundle verifies against the
    ...
    a structurally illegal bundle, signature or ``key`` raises
    ValueError, exactly as :func:`verify_hampb` and
    :func:`verify_signature` would.
    """
...
def encode_hbapb(bundle: HBAPB) -> bytes:
    ...
    non-empty, strictly increasing, unique and within
    ``0 <= i < total``, the bundle count must equal the index count, and
    the sibling count must be the unique one derived from ``total`` and
    the indices by the compact multi-proof walk.
...
def decode_hbapb(blob: bytes) -> HBAPB:
    ...
    sibling count followed by exactly that many raw 32-byte sibling
    digests, where the count must be the unique one derived from
    ``total`` and the disclosed indices, and finally the signature
    frame: the length-prefixed integers ``R`` and ``z`` with ``R``
    ...
    if not blob.startswith(HBAPB_WIRE_TAG):
        raise ValueError("bad hbap bundle tag")
    ...
    if offset != len(blob):
        raise ValueError("trailing bytes after hbap bundle")
    ...
    if encode_hbapb(bundle) != blob:
        raise ValueError("non-canonical hbap bundle encoding")
    return bundle
