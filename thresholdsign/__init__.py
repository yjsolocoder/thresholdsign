...
# canonical proof transport codec (encode_hp/decode_hp) and the
# proof-plus-root-signature transport bundle (HPB with encode_hpb/decode_hpb
# and verify_hpb) follow below.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Canonical HP transport: a self-delimiting, byte-for-byte reproducible
# encoding of an HP membership proof for cross-implementation exchange and
# persistence, plus the HPB bundle that pairs the proof with the aggregate
# root signature so a proof and its root signature travel together. Decoding
# restores structure only — each nested bundle goes through decode_hbpb, no
# bundle or signature is checked and no state is kept, so check_hp (via
# verify_hpb) remains the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
def encode_hp(proof: HP) -> bytes:
    ...
    unsigned big-endian integer, one frame per bundle — the 4-byte
    unsigned big-endian length ``len(E)`` followed by the raw bytes
    ``E = encode_hbpb(bundle)`` (never empty) — and finally the sibling
    count as a 4-byte unsigned big-endian integer followed by the raw
    32-byte sibling digests in their original leaf-to-root, left-to-right
    order. A ``VARINT`` is a 4-byte unsigned big-endian body length
    followed by the shortest unsigned big-endian value (zero is the
    single byte ``00``, positive values carry no leading zero) — exactly
    the integer and frame conventions of the existing HBPB transport
    bundle. The indices must be non-empty, strictly increasing, unique
    and within ``0 <= i < total``, the bundle count must equal the index
    count, and the sibling count must be the unique one derived from
    ``total`` and the indices by the compact multi-proof walk.

    Only a structurally legal :class:`HP` is accepted — a non-proof
    argument or wrong field/entry types (including boolean indices) raise
    TypeError and illegal bounds, an empty index tuple, a bundle tuple
    that does not pair one-to-one with the indices, a sibling that is not
    exactly 32 bytes, a sibling count other than the one ``total`` and
    the indices determine, a structurally illegal nested bundle or an
    over-long frame raise ValueError — but the bundles are not verified
    and no signature is checked: :func:`check_hp` stays the way to verify
    a proof afterwards. The output for a given proof is unique and the
    encoding carries no network, storage or hidden state.
    """
    if not isinstance(proof, HP):
        raise TypeError("proof must be an HP instance")
...
def decode_hp(blob: bytes) -> HP:
    ...
    the 4-byte bundle count (which must equal the index count) followed
    by that many non-empty bundle frames (a 4-byte non-zero length
    followed by bytes that :func:`decode_hbpb` accepts), and finally the
    4-byte sibling count followed by exactly that many raw 32-byte
    sibling digests, where the count must be the unique one derived from
    ``total`` and the disclosed indices. A non-bytes argument raises
    TypeError; a wrong or missing tag, an empty index list, a
    non-positive or over-64-bit ``total``, an index outside
    ``0 <= i < total``, indices that are not strictly increasing and
    ...
    with :func:`decode_hbpb` (which verifies no bundle and checks no
    signature) and nothing else is verified. A structurally legal proof
    whose bundles do not verify or whose root is not signed is returned
    normally, and :func:`check_hp` reports it as ``False``.
    """
    if not isinstance(blob, bytes):
        raise TypeError("blob must be bytes")
...
def encode_hpb(bundle: HPB) -> bytes:
    """Canonically encode an hp bundle for transport or persistence.

    The encoding is the direct concatenation, in order, of the tag
    ``b"ts/hpb/v1"``, the 4-byte unsigned big-endian length ``len(P)``
    followed by ``P = encode_hp(bundle.proof)`` (never empty), and
    finally the signature frame: ``VARINT(R)``, ``VARINT(z)``, the 4-byte
    unsigned big-endian signer count and one ``VARINT(id)`` per ascending
    signer id. A ``VARINT`` is a 4-byte unsigned big-endian body length
    followed by the shortest unsigned big-endian value (zero is the
    single byte ``00``, positive values carry no leading zero); ``R``
    must be positive and ``z`` may be zero. The integer and frame
    conventions are exactly those of the existing HBPB transport bundle.
    ...
    ``R``, a negative ``z``, an empty or non-strictly-increasing signer
    id tuple) or an over-long frame raises ValueError — but the proof's
    bundles are not verified and the signature is not checked against the
    root: :func:`verify_hpb` stays the way to verify a bundle afterwards.
    The output for a given bundle is unique and the encoding carries no
    network, storage or hidden state.
    """
    if not isinstance(bundle, HPB):
        raise TypeError("bundle must be an HPB instance")
    if not isinstance(bundle.proof, HP):
        raise TypeError("bundle.proof must be an HP instance")
...
def verify_hpb(bundle: HPB, key: SigningDKGResult) -> bool:
    ...
    ``b"hp/r" || U64(total) || root``. Returns ``True`` only when all of
    them hold; a well-formed bundle with a tampered bundle, index,
    sibling or signature, a bundle swapped or altered, a missing or extra
    sibling, or one presented under another key, returns ``False``.
