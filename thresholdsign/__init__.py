...
decode_hdsc_archive_proof_bundle / verify_hdsc_archive_proof_bundle,
and the
...
# statement as one self-delimiting, byte-for-byte reproducible byte string,
# mirroring the single-proof bundles. Decoding restores structure only —
# the nested bundles go through decode_hdsc_proof_bundle, their seals are
# not parsed for verification and no signature is checked, so
# verify_hdsc_archive_proof_bundle remains the sole verifier afterwards.
# ---------------------------------------------------------------------------
...
class HDSCArchiveProofBundle:
    """A compact multi-bundle archive inclusion proof with its root signature.

    The fields, in order, are ``proof`` (an :class:`HDSCArchiveProof`)
    and ``signature`` (the :class:`AggregateSignature` on the proof's
    ``b"ts/hdscba/r1" || U64(total) || root`` statement — exactly the
    message :func:`make_hdsc_archive_proof` returns for signing). The
    dataclass is frozen, positionally constructible and compared by
    value, and carries no network, storage or hidden state. Field types
    and bounds are not checked at construction time —
...
def encode_hdsc_archive_proof_bundle(
    ...
    unsigned big-endian integer, one frame per disclosed bundle — the
    4-byte unsigned big-endian length ``len(E)`` followed by
    ``E = encode_hdsc_proof_bundle(item)`` (never empty) — the sibling
    count as a 4-byte unsigned big-endian integer followed by the raw
    32-byte sibling digests in their original leaf-to-root, left-to-right
    order, and finally the signature frame: ``VARINT(R)``, ``VARINT(z)``,
    the 4-byte unsigned big-endian signer count ``k`` and one
    ``VARINT(id)`` per ascending signer id. A ``VARINT`` is a 4-byte
    unsigned big-endian body length followed by the shortest unsigned
    big-endian value (zero is the single byte ``00`` and positive values
    carry no leading zero); ``R`` must be positive and ``z`` may be zero.
    The indices must be non-empty, strictly increasing, unique and within
    ...
    wrong proof/signature field types, raise TypeError and illegal proof
    or signature structure (the exact bounds of
    :func:`check_hdsc_archive_proof`'s structural checks, a bundle tuple
    that does not pair one-to-one with the indices, a sibling count other
    than the one ``total`` and the indices determine, a structurally
    illegal nested bundle, a non-positive ``R``, a negative ``z``, an
    empty or non-strictly-increasing signer id tuple) or an over-long
    frame raises ValueError — but the nested bundles are not verified
    ...
    for position, item in enumerate(bundles):
        encoded_item = encode_hdsc_proof_bundle(item)
        if len(encoded_item) > 0xFFFFFFFF:
            raise ValueError(
                f"hdsc archive proof bundle item {position + 1} "
                "encoding too long"
            )
        encoded_bundles.append(encoded_item)
    signer_count = _check_history_proof_bundle_signature(bundle.signature)
    if signer_count > 0xFFFFFFFF:
        raise ValueError("too many signer ids")
    ...
    for encoded_item in encoded_bundles:
        buffer += len(encoded_item).to_bytes(4, "big", signed=False)
        buffer += encoded_item
    buffer += len(siblings).to_bytes(4, "big", signed=False)
    for sibling in siblings:
...
def decode_hdsc_archive_proof_bundle(
    ...
    """Decode the canonical encoding of :func:`encode_hdsc_archive_proof_bundle`.

    Accepts only the single canonical form: the tag
    ``b"ts/hdscapb/v1"``, the length-prefixed integer ``total`` (a 4-byte
    unsigned big-endian length followed by its shortest unsigned
    big-endian value, zero encoded as the single byte ``00``), the 4-byte
    non-zero index count followed by one canonical ``VARINT`` per
    ...
    integers ``R`` and ``z``, the 4-byte non-zero signer count ``k`` and
    exactly ``k`` strictly increasing positive signer ids. A non-bytes
    argument raises TypeError; a wrong or missing tag, an empty index
    list, a non-positive or over-64-bit ``total``, an index outside
    ``0 <= i < total``, indices that are not strictly increasing and
    ...
    with :func:`decode_hdsc_proof_bundle` (which verifies no seal and
    judges no signature) and the root signature is not checked. A
    structurally legal bundle whose disclosed bundles do not verify or
    whose signature does not sign the root is returned normally, and
    ...
    total, offset = _read_varint(
        blob, offset, what="hdsc archive proof bundle total"
    )
    if total <= 0:
        raise ValueError("hdsc archive proof bundle total must be positive")
    if total > 0xFFFFFFFFFFFFFFFF:
        raise ValueError("hdsc archive proof bundle total too large")

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated hdsc archive proof bundle index count"
        )
    index_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if index_count == 0:
        raise ValueError(
            "hdsc archive proof bundle indices must be non-empty"
        )
    indices = []
    for _ in range(index_count):
        index, offset = _read_varint(
            blob, offset, what="hdsc archive proof bundle index"
        )
        indices.append(index)

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated hdsc archive proof bundle item count"
        )
    item_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    if item_count != index_count:
        raise ValueError(
            "hdsc archive proof bundle item count must match the "
            "index count"
        )
    bundles = []
    for position in range(item_count):
        encoded_item, offset = _read_audit_proof_block(
            blob,
            offset,
            what=f"hdsc archive proof bundle item {position + 1}",
        )
        bundles.append(decode_hdsc_proof_bundle(encoded_item))

    if offset + 4 > len(blob):
        raise ValueError(
            "truncated hdsc archive proof bundle sibling count"
        )
    sibling_count = int.from_bytes(blob[offset:offset + 4], "big")
    offset += 4
    siblings = []
    for _ in range(sibling_count):
        if offset + HDSC_ARCHIVE_PROOF_DIGEST_SIZE > len(blob):
            raise ValueError(
                "truncated hdsc archive proof bundle sibling"
            )
        siblings.append(
            bytes(
                blob[
    ...
    signature, offset = _read_bundle_signature_frame(
        blob, offset, what="hdsc archive proof bundle"
    )
    if offset != len(blob):
    ...
    if encode_hdsc_archive_proof_bundle(bundle) != blob:
        raise ValueError(
            "non-canonical hdsc archive proof bundle encoding"
        )
    return bundle
...
def verify_hdsc_archive_proof_bundle(
    ...
    This is a convenience wrapper over
    ``check_hdsc_archive_proof(bundle.proof, bundle.signature, key)``:
    the Merkle root is rebuilt from the proof, every disclosed bundle
    re-checked with :func:`verify_hdsc_proof_bundle` against ``key`` and
    the signature verified on
    ``b"ts/hdscba/r1" || U64(total) || root``. Returns ``True`` only
    when all of them hold; a well-formed bundle with a tampered or
    swapped disclosed bundle, index, sibling or signature, or one
    presented under another key, returns ``False``.
    ...
    return check_hdsc_archive_proof(bundle.proof, bundle.signature, key)
