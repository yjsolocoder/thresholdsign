"""Tests for the canonical SealHistoryExtensionDeltaChain transport encoding:
encode_history_delta / decode_history_delta / verify_history_delta."""
...
from test_seal_history_extension_delta_chain import linked_chain, make_delta
...
def build_wire(delta: SealHistoryExtensionDeltaChain) -> bytes:
    """Independently build the delta chain wire format from the spec."""
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_honest_chains_round_trip_at_every_length(self):
        ...
        for m in range(2, 9):
            ...
            delta = make_delta(self.key, self.history, splits, seed=seed)
            seed += 100
    ...
    def test_wider_splits_round_trip_and_verify(self):
        splits = ((2, 4), (4, 5), (5, 8))
        delta = make_delta(self.key, self.history, splits, seed=31000)
        wire = encode_history_delta(delta)
    ...
    def test_zero_batches_single_hop_round_trips(self):
        delta = make_delta(self.key, self.history, [(2, 4)], seed=31100)
    ...
    def test_encoding_matches_independent_builder(self):
        splits = ((2, 4), (4, 5), (5, 7))
        delta = make_delta(self.key, self.history, splits, seed=32000)
        wire = encode_history_delta(delta)
        ...
        self.assertEqual(
            wire[offset:offset + len(encoded_first)], encoded_first
        )
    ...
    def test_layout_is_first_frame_then_batches_then_bare_sig_frames(self):
        splits = ((1, 3), (3, 6))
        delta = make_delta(self.key, self.history, splits, seed=32100)
        wire = encode_history_delta(delta)
        offset = len(DELTA_TAG)
        encoded_first = encode_history_extension(delta.first)
        self.assertEqual(wire[offset:offset + 4], u32(len(encoded_first)))
        ...
        offset += 32 * len(batch)
        # The remaining b+2 signature frames are bare canonical frames.
        for signature in delta.signatures:
...
class StructureOnlyTest(unittest.TestCase):
    ...
    def test_honest_chain_wrong_key_decodes_but_fails_verification(self):
        other = make_other_key()
        delta = make_delta(
            self.key, self.history, ((1, 3), (3, 5)), seed=33000
        )
        self.assertTrue(encode_history_delta(delta))
        self.assertFalse(verify_history_delta(delta, other))

    def test_tampered_batch_leaves_decode_but_fail_verification(self):
        # The batches need not reproduce signed roots for the decoder: a
        # flipped appended digest is restored verbatim and fails to verify.
        delta = make_delta(self.key, self.history, [(2, 4), (4, 6)])
...
class DecodeValidationTest(unittest.TestCase):
    ...
    def test_round_trip_restores_structure_byte_for_byte(self):
        decoded = decode_history_delta(self.wire)
        self.assertEqual(encode_history_delta(decoded), self.wire)
