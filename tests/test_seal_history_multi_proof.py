...
def seal_multi(history, indices, key, *, seed=800):
...
class SealHistoryMultiProofDataTest(unittest.TestCase):
    ...
    def test_frozen_value_equality_and_hash(self):
        ...
        for changed in (
            ...
            SealHistoryMultiProof((1, 3), 4, (other, seal), siblings),
    ...
    def test_construction_does_not_validate(self):
        # Out-of-range indices, non-tuple containers and non-seal values
        # all construct; the maker and checker reject them.
        seal = ReportSeal(
            NonceLeakReport((synthetic(1, 10),)),
            AggregateSignature(R=5, z=7, signer_ids=(1, 3)),
        )
        ...
        SealHistoryMultiProof((-1, 3), 3, (seal, seal), (b"",))
...
class MakeHistoryMultiProofTest(unittest.TestCase):
    ...
    def test_single_index_omits_odd_tail_companions(self):
        # The single-seal SealHistoryProof always carries a (redundant)
        # self sibling on odd-tail levels; the compact proof omits them.
        for n in range(1, 9):
            history = self._history_of_size(n)
            levels = independent_tree(history.items)
    ...
    def test_root_is_order_and_content_bound(self):
        ...
        shortened = self._history_of_size(4)
        self.assertNotEqual(
...
class CheckHistoryMultiProofTest(unittest.TestCase):
    ...
    def test_single_item_history_verifies_with_empty_siblings(self):
        ...
        proof, signature = seal_multi(history, (0,), self.key)
        self.assertEqual(proof.siblings, ())
        self.assertTrue(check_history_multi_proof(proof, signature, self.key))

    def test_tampered_seal_returns_false(self):
        history = self._history_of_size(5)
        proof, signature = seal_multi(history, (1, 2), self.key)
        _, other = make_history_multi_proof(history, (3,))
        bad = dataclasses.replace(
            proof, seals=(other.seals[0], proof.seals[1])
        )
        self.assertFalse(check_history_multi_proof(bad, signature, self.key))
    ...
    def test_changed_index_returns_false_when_shape_still_fits(self):
        # n=5 -> n=6 keeps the same sibling count for indices (1, 3), so
        # the proof stays structurally legal but rebuilds a wrong root.
        history = self._history_of_size(6)
        proof, signature = seal_multi(history, (1, 3), self.key)
        bad = dataclasses.replace(proof, total=5)
        self.assertEqual(len(bad.siblings), 3)
        self.assertFalse(check_history_multi_proof(bad, signature, self.key))

    def test_flipped_sibling_returns_false(self):
        history = self._history_of_size(5)
        proof, signature = seal_multi(history, (0, 2), self.key)
    ...
    def test_missing_sibling_raises_value_error(self):
        history = self._history_of_size(5)
        proof, signature = seal_multi(history, (0, 2, 4), self.key)
    ...
    def test_bad_signature_returns_false(self):
        ...
        self.assertFalse(check_history_multi_proof(proof, bad_sig, self.key))

    def test_signature_from_another_tree_returns_false(self):
        history = self._history_of_size(5)
        _message, proof = make_history_multi_proof(history, (0, 2))
        other = SealHistory(tuple(reversed(history.items)))
        other_message, _other = make_history_multi_proof(other, (0, 2))
    ...
    def test_cross_key_seal_returns_false(self):
        # A seal that verifies under another key fails verify_seal here.
        other_key = make_other_key()
        other_history = make_history(other_key, nonces=(301, 302, 303))
        _, foreign = make_history_multi_proof(other_history, (1,))
        history = self._history_of_size(3)
        proof, signature = seal_multi(history, (0, 1), self.key)
        mixed = dataclasses.replace(
            proof, seals=(proof.seals[0], foreign.seals[0])
        )
        self.assertFalse(check_history_multi_proof(mixed, signature, self.key))

    def test_proof_does_not_need_the_full_history(self):
        # The checker takes only the disclosed seals plus digests.
        history = self._history_of_size(5)
        proof, signature = seal_multi(history, (1, 3), self.key)
        self.assertTrue(check_history_multi_proof(proof, signature, self.key))
    ...
    def test_argument_type_errors(self):
        ...
        with self.assertRaises(TypeError):
            check_history_multi_proof("proof", signature, self.key)
        with self.assertRaises(TypeError):
            check_history_multi_proof(None, signature, self.key)
        with self.assertRaises(TypeError):
            check_history_multi_proof(proof, "signature", self.key)
        with self.assertRaises(TypeError):
            check_history_multi_proof(proof, None, self.key)
        with self.assertRaises(TypeError):
            check_history_multi_proof(proof, signature, "key")
        with self.assertRaises(TypeError):
            check_history_multi_proof(proof, signature, None)
    ...
    def test_field_type_errors(self):
        ...
        good = proof
        for bad in (
            dataclasses.replace(good, indices=[0, 1]),
            dataclasses.replace(good, indices=(True, 1)),
            dataclasses.replace(good, indices=("0", 1)),
            dataclasses.replace(good, total="3"),
            dataclasses.replace(good, total=True),
            dataclasses.replace(good, seals=list(good.seals)),
            SealHistoryMultiProof(
                good.indices, good.total, ("seal", good.seals[1]), ()
            ),
            dataclasses.replace(good, siblings=[b"s" * 32]),
            dataclasses.replace(good, siblings=(b"s" * 32, 33)),
        ):
    ...
    def test_structure_and_boundary_value_errors(self):
        ...
        seals = proof.seals
        for bad in (
            SealHistoryMultiProof((0, 1), 0, seals, ()),
            SealHistoryMultiProof((0, 1), -1, seals, ()),
            SealHistoryMultiProof((0, 1), 2 ** 64, seals, ()),
            SealHistoryMultiProof((), 3, (), ()),
            SealHistoryMultiProof((-1, 1), 3, seals, ()),
            SealHistoryMultiProof((0, 3), 3, seals, ()),
            SealHistoryMultiProof((1, 0), 3, seals, ()),
            SealHistoryMultiProof((0, 0), 3, seals, ()),
            SealHistoryMultiProof((0,), 3, (), ()),
            SealHistoryMultiProof((0, 1, 2), 3, seals, ()),
            SealHistoryMultiProof((0, 1), 3, seals, (b"x" * 31,)),
            SealHistoryMultiProof((0, 1), 3, seals, (b"x" * 33,)),
            dataclasses.replace(proof, siblings=proof.siblings[:-1]),
            dataclasses.replace(
                proof, siblings=proof.siblings + (b"s" * 32,)
            ),
        ):
    ...
    def test_illegal_signature_value_error(self):
        history = self._history_of_size(3)
        _message, proof = make_history_multi_proof(history, (0, 1))
        with self.assertRaises(ValueError):
            check_history_multi_proof(
                proof,
                AggregateSignature(R=0, z=7, signer_ids=(1, 3)),
