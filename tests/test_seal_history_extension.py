"""Tests for append-only consistency proofs over the seal-history tree:
...
def history_of_size(key, n):
    return make_history(key, tuple(range(700, 700 + n)))
...
class SealHistoryExtensionDataTest(unittest.TestCase):
    ...
    def test_construction_does_not_validate(self):
        # A plain value: out-of-range totals and non-bytes leaves are accepted
        # at construction time; check_history_extension is the verifier.
        proof = SealHistoryExtension(0, ())
        self.assertEqual((proof.old_total, proof.leaves), (0, ()))
...
class MakeHistoryExtensionTest(unittest.TestCase):
    ...
    def test_statements_match_make_history_proof(self):
        # Both statements are exactly the root statements make_history_proof
        # builds for the old prefix and for the full history.
        for old_total in range(1, len(self.history.items)):
            old_message, new_message, _proof = make_history_extension(
                self.history, old_total
            )
            prefix = SealHistory(items=self.history.items[:old_total])
            self.assertEqual(old_message, make_history_proof(prefix, 0)[0])
            self.assertEqual(new_message, make_history_proof(self.history, 0)[0])
    ...
    def test_empty_history_and_bad_old_total_raise_value_error(self):
        with self.assertRaises(ValueError):
            make_history_extension(SealHistory(items=()), 0)
        for bad in (-1, 0, 6, 7, 100):
            with self.assertRaises(ValueError, msg=f"old_total={bad}"):
                make_history_extension(self.history, bad)

    def test_history_count_overflow_raises_value_error(self):
        class HugeTuple(tuple):
            def __len__(self):
                return 2 ** 64

        history = SealHistory(items=HugeTuple((self.history.items[0],)))
        with self.assertRaises(ValueError):
...
class CheckHistoryExtensionTest(unittest.TestCase):
    ...
    def test_tampered_leaf_returns_false(self):
        ...
        self.assertFalse(check_history_extension(bad, old_sig, new_sig, self.key))
    ...
    def test_bad_signature_returns_false(self):
        ...
        self.assertFalse(check_history_extension(proof, old_sig, bad_new, self.key))
        bad_old = AggregateSignature(
            R=old_sig.R,
            z=(old_sig.z + 1) % FIELD_PRIME,
            signer_ids=old_sig.signer_ids,
        )
        self.assertFalse(check_history_extension(proof, bad_old, new_sig, self.key))

    def test_signature_from_another_statement_returns_false(self):
        # The new statement binds (total, root), so a signature over a
    ...
    def test_structurally_illegal_signature_raises_value_error(self):
        from test_nonce_reuse import GROUP_PRIME
