"""Tests for compact multi-package Merkle membership proofs over the
...
from test_hdsc_proof_bundle_archive import make_archive, make_signed_bundle
...
def bundles_of_size(n, key, seed_base=7100):
    """Build ``n`` distinct structurally legal bundles for ``key``."""
    combos = (
        (8, (0, 2)),
        (7, (1, 3)),
        (6, (3, 5)),
...
def archive_of_size(n, key, seed_base=7400):
    """Seal an archive of ``n`` distinct bundles with one outer signature."""
    return make_archive(
        bundles_of_size(n, key), key, seed=seed_base + n
    )


def signed_archive_proof(archive, indices, key, *, seed=7600):
...
class HDSCArchiveProofDataTest(unittest.TestCase):
    def test_field_order_and_positional_construction(self):
        ...
        key = make_key()
        bundles = bundles_of_size(2, key, seed_base=7010)
        siblings = (b"s" * 32,)
    ...
    def test_frozen_value_equality_and_hash(self):
        key = make_key()
        bundles = bundles_of_size(2, key, seed_base=7020)
    ...
    def test_construction_does_not_validate(self):
        ...
        key = make_key()
        bundle = bundles_of_size(1, key, seed_base=7030)[0]
        HDSCArchiveProof((9,), 1, (bundle,), ())
        HDSCArchiveProof((0,), 0, ("not-a-bundle",), b"x")
        HDSCArchiveProof((), 3, (bundle,), (b"",))


class MakeHDSCArchiveProofTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.archive = archive_of_size(8, self.key, seed_base=7700)
    ...
    def test_archive_outer_signature_is_not_consulted(self):
        ...
        # usable outer signature still builds the same statement.
    ...
    def test_non_tuple_and_non_bundle_items_raise_type_error(self):
        for bad in (list(self.archive.items), None, "items"):
            with self.assertRaises(TypeError, msg=repr(bad)):
                make_hdsc_archive_proof(
        ...
        for bad in ("x", None, 42, b"x", object()):
            with self.assertRaises(TypeError, msg=repr(bad)):
                make_hdsc_archive_proof(
                    HDSCProofBundleArchive(
                        (bad,) + self.archive.items[1:],
                        self.archive.signature,
                    ),
                    (0,),
                )
        # An HDSCProof is not an HDSCProofBundle.
        with self.assertRaises(TypeError):
            make_hdsc_archive_proof(
                HDSCProofBundleArchive(
                    (self.archive.items[0].proof,) + self.archive.items[1:],
                    self.archive.signature,
                ),
                (0,),
            )
...
class CheckHDSCArchiveProofTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.archive = archive_of_size(8, self.key, seed_base=7800)
    ...
    def test_honest_proofs_verify_for_every_size_and_subset(self):
        for n in range(1, 9):
            ...
            signature = sign_message(self.key, message, seed=8000 + n)
            for mask in range(1, 1 << n):
                indices = tuple(i for i in range(n) if mask & (1 << i))
    ...
    def test_single_bundle_archive_verifies_with_empty_siblings(self):
        ...
        proof, signature = signed_archive_proof(
            archive, (0,), self.key, seed=8100
        )
        self.assertEqual(proof.siblings, ())
        self.assertTrue(check_hdsc_archive_proof(proof, signature, self.key))
    ...
    def test_one_signature_covers_every_subset(self):
        ...
        signature = sign_message(self.key, message, seed=8200)
        for indices in (
            (0,),
            (4,),
            (0, 4),
    ...
    def test_rebuilt_root_matches_independent_tree(self):
        archive = self._archive_of_size(6)
        root = independent_tree(archive.items)[-1][0]
        message, _ = make_hdsc_archive_proof(archive, (0, 2, 5))
        self.assertEqual(message, ROOT_TAG + u64(6) + root)

    def test_replaced_bundle_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=8300
        )
        # Substitute a structurally legal bundle that genuinely verifies
        # under the same key but belongs at another archive position.
        bad = dataclasses.replace(
            proof, bundles=(self.archive.items[0],) + proof.bundles[1:]
        )
        self.assertFalse(check_hdsc_archive_proof(bad, signature, self.key))

    def test_swapped_bundles_return_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=8400
        )
    ...
    def test_changed_total_returns_false_when_shape_still_fits(self):
        ...
        signature = sign_message(self.key, _message, seed=8500)
    ...
    def test_flipped_sibling_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2), self.key, seed=8600
        )
    ...
    def test_sibling_order_matters(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2), self.key, seed=8700
        )
    ...
    def test_missing_sibling_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2, 4), self.key, seed=8800
        )
    ...
    def test_bundle_count_mismatch_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=9000
        )
        for bad_bundles in (
            (),
            proof.bundles[:1],
    ...
    def test_bad_signature_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2, 4), self.key, seed=9100
        )
        ...
        self.assertFalse(check_hdsc_archive_proof(proof, bad_sig, self.key))

    def test_tampered_signature_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2, 4), self.key, seed=9200
        )
        ...
        self.assertFalse(check_hdsc_archive_proof(proof, tampered, self.key))
    ...
    def test_signature_from_another_tree_returns_false(self):
        ...
        other_archive = HDSCProofBundleArchive(
            tuple(reversed(self.archive.items[:5])), self.archive.signature
        )
        other_message, _other = make_hdsc_archive_proof(
            other_archive, (0, 2)
        )
        other_signature = sign_message(self.key, other_message, seed=9300)
        self.assertFalse(
    ...
    def test_wrong_key_returns_false(self):
        proof, signature = signed_archive_proof(
            self.archive, (0, 2), self.key, seed=9400
        )
        other_key = make_other_key()
        self.assertNotEqual(other_key.public_key, self.key.public_key)
        self.assertFalse(check_hdsc_archive_proof(proof, signature, other_key))

    def test_cross_key_bundle_returns_false(self):
        # A well-formed proof carrying a real, self-verifying bundle of
        # another key: verify_hdsc_proof_bundle fails before the root
        # signature is consulted.
        other_key = make_other_key()
        foreign_archive = archive_of_size(2, other_key, seed_base=9500)
        _message, foreign = make_hdsc_archive_proof(foreign_archive, (0,))
        proof, signature = signed_archive_proof(
            self.archive, (0,), self.key, seed=9600
        )
        mixed = dataclasses.replace(proof, bundles=foreign.bundles)
        self.assertFalse(check_hdsc_archive_proof(mixed, signature, self.key))

    def test_inner_root_signature_tamper_returns_false(self):
        # The per-bundle root signature is re-checked through
        # verify_hdsc_proof_bundle: a structurally legal bundle carrying
        # the signature of another message fails, so the archive proof
        # fails even though the tree rebuild matches.
        proof, signature = signed_archive_proof(
            self.archive, (0, 2), self.key, seed=9700
        )
        good = proof.bundles[0]
        inner = good.signature
        tampered_inner = AggregateSignature(
            R=inner.R,
            z=(inner.z + 1) % FIELD_PRIME,
            signer_ids=inner.signer_ids,
        )
    ...
    def test_inner_seal_tamper_returns_false(self):
        # An inner proof with a swapped seal: the bundle stays
        # structurally legal but its canonical encoding changes, so the
        # archive leaf no longer matches and the proof reports False (the
        # same tamper makes check_hdsc_proof reject the inner bundle on
        # its own as well).
        proof, signature = signed_archive_proof(
            self.archive, (0,), self.key, seed=9750
        )
        bundle = proof.bundles[0]
        donor = self.archive.items[3].proof
        replacement = next(
            seal
            for seal in donor.seals
            if seal != bundle.proof.seals[0]
        )
        tampered_inner = dataclasses.replace(
            bundle.proof,
            seals=(replacement,) + bundle.proof.seals[1:],
        )
        bad_bundle = HDSCProofBundle(tampered_inner, bundle.signature)
        self.assertFalse(verify_hdsc_proof_bundle(bad_bundle, self.key))
        bad = dataclasses.replace(proof, bundles=(bad_bundle,))
        self.assertFalse(check_hdsc_archive_proof(bad, signature, self.key))

    def test_proof_does_not_need_the_full_archive(self):
        proof, signature = signed_archive_proof(
            self.archive, (2, 5), self.key, seed=9800
        )
        self.assertTrue(check_hdsc_archive_proof(proof, signature, self.key))

    def test_entry_type_errors(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=9900
        )
        with self.assertRaises(TypeError):
            check_hdsc_archive_proof("proof", signature, self.key)
        with self.assertRaises(TypeError):
    ...
    def test_structure_and_boundary_value_errors(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=10000
        )
        thirty_two = b"s" * 32
        for bad in (
    ...
    def test_structurally_illegal_bundle_raises_value_error(self):
        proof, signature = signed_archive_proof(
            self.archive, (1, 2), self.key, seed=10100
        )
        bad_bundle = HDSCProofBundle(
            HDSCProof(indices=(), total=0, seals=(), siblings=()),
            AggregateSignature(R=5, z=7, signer_ids=(1, 3)),
