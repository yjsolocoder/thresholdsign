...
def linked_chain(key, history, splits, *, seed=9000):
    ...
    for old_n, new_n in splits:
        ...
        items.append(signed_bundle(key, prefix, old_n, seed=seed))
        seed += 10
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_honest_chains_round_trip_at_every_length(self):
        ...
        for m in range(2, 9):
            ...
            chain = linked_chain(self.key, self.history, splits, seed=seed)
            seed += 100
    ...
    def test_single_item_chain_round_trips(self):
        prefix = SealHistory(self.history.items[:4])
        bundle = signed_bundle(self.key, prefix, 2, seed=11100)
...
class StructureOnlyTest(unittest.TestCase):
    ...
    def test_decode_does_not_check_signatures_or_linkage(self):
        # Two individually honest bundles over unrelated histories: every
        # signature verifies on its own, but the leaf sets do not link —
        # decode restores the chain unchanged and the verifier reports the
        # broken linkage as False.
        first = signed_bundle(
            self.key, SealHistory(self.history.items[:4]), 2, seed=13100
        )
...
class LinkageTest(unittest.TestCase):
    ...
    def test_same_count_but_different_leaves_returns_false(self):
        first = signed_bundle(
            self.key, SealHistory(self.history.items[:4]), 2, seed=14100
        )
        ...
        self.assertEqual(len(first.extension.leaves), second.extension.old_total)
        self.assertFalse(
            verify_history_extension_bundle_chain(chain, self.key)
        )
    ...
    def test_one_bad_bundle_returns_false_even_when_counts_line_up(self):
        other = make_other_key()
        first = signed_bundle(
            self.key, SealHistory(self.history.items[:4]), 2, seed=14200
        )
        prefix_ours = SealHistory(self.history.items[:6])
        second_ours = signed_bundle(self.key, prefix_ours, 4, seed=14210)
        second_foreign = signed_bundle(other, prefix_ours, 4, seed=14220)
    ...
    def test_tampered_middle_bundle_leaves_returns_false(self):
        chain = self._chain(((1, 3), (3, 5), (5, 8)))
        middle = chain.items[1]
        flipped = bytearray(middle.extension.leaves[-1])
        flipped[0] ^= 1
        bad_extension = SealHistoryExtension(
            old_total=middle.extension.old_total,
            leaves=middle.extension.leaves[:-1] + (bytes(flipped),),
        )
        bad_middle = SealHistoryExtensionBundle(
            bad_extension, middle.old_sig, middle.new_sig
        )
        bad_chain = SealHistoryExtensionBundleChain(
            (chain.items[0], bad_middle, chain.items[2])
        )
        self.assertFalse(
            verify_history_extension_bundle_chain(bad_chain, self.key)
        )
...
class EncodeValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.bundle = signed_bundle(
            self.key, SealHistory(self.history.items[:3]), 2, seed=15000
        )
    ...
    def test_illegal_nested_bundle_value_error(self):
        ...
        bad_bundle = SealHistoryExtensionBundle(
            bad_extension, self.bundle.old_sig, self.bundle.new_sig
        )
        with self.assertRaises(ValueError):
            encode_history_extension_bundle_chain(
...
class DecodeValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.chain = linked_chain(
            self.key, self.history, ((1, 3), (3, 5)), seed=16000
        )
...
class VerifyValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.history = history_of_size(self.key, 5)
        self.bundle = signed_bundle(
            self.key, SealHistory(self.history.items[:3]), 2, seed=17000
        )
