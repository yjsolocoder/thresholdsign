"""Tests for the non-empty order-preserving archive of whole HAMPB
transport bundles and its outer threshold-Schnorr signature:
HAMPBArchive / hampba_message / verify_hampba."""
...
def build_bundles(key):
    """Three distinct structurally legal, key-verifying HAMPB bundles."""
    return (
        make_signed_bundle(
            archive_of_size(4, key, seed_base=20100), (0, 2), key, seed=20200
        ),
        make_signed_bundle(
            archive_of_size(5, key, seed_base=20110), (1, 3), key, seed=20210
        ),
        make_signed_bundle(
            archive_of_size(7, key, seed_base=20120), (3, 6), key, seed=20220
        ),
    )
...
def make_archive(items, key, *, signer_ids=(1, 3), seed=20300):
    """Seal a tuple of HAMPB bundles with one outer signature of ``key``."""
...
class ArchiveShapeTest(unittest.TestCase):
    def test_fields_in_order(self):
        self.assertEqual(
            [field.name for field in dataclasses.fields(HAMPBArchive)],
            ["items", "signature"],
        )
    ...
    def test_frozen_positional_and_value_equal(self):
        ...
        archive = HAMPBArchive(items, signature)
        self.assertIs(archive.items, items)
        self.assertIs(archive.signature, signature)
        self.assertEqual(
            archive,
            HAMPBArchive(items=items, signature=signature),
        )
        self.assertEqual(
            hash(archive),
            hash(HAMPBArchive(items, signature)),
        )
        self.assertNotEqual(
            archive,
            HAMPBArchive(items[:1], signature),
        )
        self.assertNotEqual(
            archive,
            HAMPBArchive(
    ...
    def test_fields_have_no_defaults(self):
        with self.assertRaises(TypeError):
            HAMPBArchive()
        with self.assertRaises(TypeError):
            HAMPBArchive(build_bundles(make_key())[:1])

    def test_construction_does_not_validate(self):
        # A plain frozen value: non-tuple items and non-signature fields
        # construct; hampba_message and the verifier reject them.
        HAMPBArchive("not-a-tuple", "not-a-signature")
        HAMPBArchive(None, None)
...
class ArchiveMessageTest(unittest.TestCase):
    ...
    def test_matches_independent_spec_build(self):
        for items in (self.items_a, self.items_b):
            with self.subTest(n=len(items)):
                message = hampba_message(items, self.key.public_key)
                self.assertEqual(
    ...
    def test_layout_is_tag_digest_of_framing_then_key_varint(self):
        message = hampba_message(self.items_a, self.key.public_key)
    ...
    def test_frames_are_existing_hampb_transport_encodings(self):
        framing = build_framing(self.items_a)
        # Each framed body starts with the existing distinct bundle tag.
        self.assertIn(b"ts/hampb/v1", framing)
        for item in self.items_a:
            self.assertTrue(
                encode_hampb(item).startswith(b"ts/hampb/v1")
            )
    ...
    def test_commits_to_items_order_and_key(self):
        ...
        single = forward[:1]
        message = hampba_message(forward, self.key.public_key)
        self.assertNotEqual(
            message,
            hampba_message(reverse, self.key.public_key),
        )
        self.assertNotEqual(
            message,
            hampba_message(single, self.key.public_key),
        )
        self.assertNotEqual(
            message,
            hampba_message(forward, self.key.public_key + 1),
        )
        self.assertEqual(
            hampba_message(forward, self.key.public_key),
            message,
        )

    def test_zero_public_key_encodes_as_single_00(self):
        message = hampba_message(self.items_b, 0)
    ...
    def test_non_tuple_items_type_error(self):
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                hampba_message(bad, self.key.public_key)
    ...
    def test_non_bundle_element_type_error(self):
        ...
        # An HAMP proof is not an HAMPB bundle.
        with self.assertRaises(TypeError):
            hampba_message(
    ...
    def test_empty_items_value_error(self):
        with self.assertRaises(ValueError):
            hampba_message((), self.key.public_key)

    def test_illegal_nested_bundle_value_error(self):
        bad_bundle = HAMPB("not-a-proof", DUMMY_SIGNATURE)
        # A non-HAMP proof field is a TypeError from encode_hampb; use a
        # structurally illegal HAMP proof for the ValueError path.
        bad_value_bundle = HAMPB(
            self.items_a[0].proof.__class__(
                indices=(),
                total=0,
                bundles=(),
                siblings=(),
        ...
        with self.assertRaises(ValueError):
            hampba_message((bad_value_bundle,), self.key.public_key)
        with self.assertRaises(ValueError):
            hampba_message(
                (self.items_a[0], bad_value_bundle), self.key.public_key
            )
        with self.assertRaises(TypeError):
            hampba_message((bad_bundle,), self.key.public_key)
    ...
    def test_bad_public_key_type_or_value(self):
        for bad in ("x", None, b"x", 1.5, True, False):
            with self.assertRaises(TypeError, msg=repr(bad)):
                hampba_message(self.items_a, bad)
        with self.assertRaises(ValueError):
            hampba_message(self.items_a, -1)

    def test_deterministic_and_stateless(self):
        self.assertEqual(
            hampba_message(self.items_a, self.key.public_key),
            hampba_message(self.items_a, self.key.public_key),
        )
...
class VerifyArchiveShapeAndErrorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.archive = make_archive(self.items, self.key, seed=20400)
    ...
    def test_non_bundle_element_type_error(self):
        for bad in ("x", None, 42, b"x", object()):
            with self.assertRaises(TypeError, msg=repr(bad)):
                verify_hampba(
                    HAMPBArchive((bad,), self.archive.signature),
                    self.key,
                )
    ...
    def test_non_key_type_error(self):
        for bad in ("x", None, 42, b"x", object()):
            with self.assertRaises(TypeError, msg=repr(bad)):
                verify_hampba(self.archive, bad)
...
class VerifyArchiveVerdictTest(unittest.TestCase):
    def setUp(self):
        ...
        self.items = build_bundles(self.key)
        self.archive = make_archive(self.items, self.key, seed=20500)

    def test_verifying_archive_is_true(self):
        self.assertTrue(verify_hampba(self.archive, self.key))

    def test_single_item_archive_is_true(self):
        archive = make_archive(self.items[:1], self.key, seed=20510)
        self.assertTrue(verify_hampba(archive, self.key))

    def test_each_bundle_is_rechecked_individually(self):
        # Every item verifies standalone before the outer signature is
        # examined.
        for item in self.items:
            self.assertTrue(verify_hampb(item, self.key))

    def test_one_failing_nested_bundle_is_false(self):
        # A structurally legal bundle signed under another key encodes
        # fine but verify_hampb reports False under self.key, which
        # makes the whole archive False, outer signature or no.
        foreign_bundle = make_signed_bundle(
            archive_of_size(4, self.other, seed_base=20520),
            (0, 2),
            self.other,
            seed=20521,
        )
        self.assertFalse(verify_hampb(foreign_bundle, self.key))
        archive = make_archive(
            (self.items[0], foreign_bundle), self.key, seed=20522
        )
        self.assertFalse(verify_hampba(archive, self.key))
        # And the same with the failing bundle first.
        archive = make_archive(
            (foreign_bundle, self.items[0]), self.key, seed=20523
        )
        self.assertFalse(verify_hampba(archive, self.key))
    ...
    def test_tampered_inner_root_signature_is_false(self):
        ...
        bad_bundle = HAMPB(good.proof, tampered_inner)
        archive = make_archive(
            (bad_bundle,) + self.items[1:], self.key, seed=20530
        )
        self.assertFalse(verify_hampba(archive, self.key))
    ...
    def test_outer_signature_over_another_item_set_is_false(self):
        foreign = self._outer_signature_over(
            (self.items[2], self.items[0]), seed=20540
        )
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(self.items[:2], foreign),
                self.key,
            )
        )
    ...
    def test_deleting_an_item_is_false(self):
        ...
        foreign = self._outer_signature_over(self.items[:2], seed=20550)
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(self.items, foreign),
                self.key,
            )
        )
    ...
    def test_inserting_an_item_is_false(self):
        extra = make_signed_bundle(
            archive_of_size(6, self.key, seed_base=20560),
            (0, 2, 4),
            self.key,
            seed=20561,
        )
        foreign = self._outer_signature_over(
            self.items + (extra,), seed=20562
        )
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(self.items, foreign),
    ...
    def test_reordering_items_is_false(self):
        foreign = self._outer_signature_over(
            tuple(reversed(self.items)), seed=20570
        )
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(self.items, foreign),
    ...
    def test_replacing_an_item_is_false(self):
        replacement = make_signed_bundle(
            archive_of_size(3, self.key, seed_base=20580),
            (0, 1, 2),
            self.key,
            seed=20581,
        )
        ...
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(swapped, self.archive.signature),
                self.key,
            )
        )
    ...
    def test_tampered_outer_signature_is_false(self):
        ...
        self.assertFalse(
            verify_hampba(
                HAMPBArchive(self.items, tampered),
                self.key,
            )
        )
    ...
    def test_other_key_is_false(self):
        self.assertFalse(
            verify_hampba(self.archive, self.other)
        )
    ...
    def test_bad_bundle_does_not_mask_illegal_outer_signature(self):
        ...
        bad_bundle = make_signed_bundle(
            archive_of_size(4, self.other, seed_base=20590),
            (0, 2),
            self.other,
            seed=20591,
        )
        ...
        for bad_signature in (
            ...
            with self.assertRaises(ValueError, msg=repr(bad_signature)):
                verify_hampba(
                    HAMPBArchive(items, bad_signature),
                    self.key,
                )

    def test_bad_bundle_does_not_mask_illegal_key(self):
        bad_bundle = make_signed_bundle(
            archive_of_size(4, self.other, seed_base=20600),
            (0, 2),
            self.other,
            seed=20601,
        )
        archive = make_archive(
            (bad_bundle, self.items[0]), self.key, seed=20602
        )
        for bad_key in ("x", None, 42, b"x", object()):
            with self.assertRaises(TypeError, msg=repr(bad_key)):
                verify_hampba(archive, bad_key)
        ...
        with self.assertRaises(ValueError):
            verify_hampba(archive, broken_key)
