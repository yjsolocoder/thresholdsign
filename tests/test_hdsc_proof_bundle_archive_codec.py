"""Tests for the canonical HDSCProofBundleArchive transport encoding:
encode_hdsc_proof_bundle_archive / decode_hdsc_proof_bundle_archive."""
...
from test_hdsc_proof_bundle_archive import build_bundles, make_archive
...
def build_wire(archive: HDSCProofBundleArchive) -> bytes:
...
class EncodeDecodeArchiveTest(unittest.TestCase):
    def setUp(self):
        ...
        self.archives = (
            make_archive(self.items[:1], self.key, seed=7000),
            make_archive(self.items[:2], self.key, seed=7001),
            make_archive(self.items, self.key, seed=7002),
            make_archive(
                (self.items[2], self.items[0]), self.key, seed=7003
            ),
        )
    ...
    def test_wire_matches_independent_spec_build(self):
        for archive in self.archives:
            with self.subTest(n=len(archive.items)):
                blob = encode_hdsc_proof_bundle_archive(archive)
    ...
    def test_frames_are_existing_bundle_transport_encodings(self):
        ...
        for item in archive.items:
            ...
            self.assertEqual(frame, encode_hdsc_proof_bundle(item))
    ...
    def test_frames_decode_standalone(self):
        ...
        for item in archive.items:
            ...
            offset += 4
            frame = blob[offset:offset + length]
            self.assertEqual(decode_hdsc_proof_bundle(bytes(frame)), item)
            offset += length
    ...
    def test_outer_signature_frame_matches_audit_proof_bundle_frame(self):
        ...
        archive_blob = encode_hdsc_proof_bundle_archive(archive)
    ...
    def test_round_trip_byte_for_byte(self):
        for archive in self.archives:
            with self.subTest(n=len(archive.items)):
                blob = encode_hdsc_proof_bundle_archive(archive)
                restored = decode_hdsc_proof_bundle_archive(blob)
                self.assertIsInstance(restored, HDSCProofBundleArchive)
                self.assertEqual(restored, archive)
                self.assertEqual(
                    encode_hdsc_proof_bundle_archive(restored), blob
                )

    def test_order_is_preserved(self):
        forward = make_archive(
            (self.items[0], self.items[1]), self.key, seed=7010
        )
        reverse = make_archive(
            (self.items[1], self.items[0]), self.key, seed=7011
        )
        restored = decode_hdsc_proof_bundle_archive(
            encode_hdsc_proof_bundle_archive(forward)
        )
        self.assertEqual(restored.items, (self.items[0], self.items[1]))
        self.assertNotEqual(restored, reverse)
        self.assertEqual(
            decode_hdsc_proof_bundle_archive(
                encode_hdsc_proof_bundle_archive(reverse)
            ).items,
    ...
    def test_encoding_is_deterministic_and_unique(self):
        archive = make_archive(
            (self.items[0], self.items[1]), self.key, seed=7020
        )
        other = make_archive(
            (self.items[1], self.items[0]), self.key, seed=7021
        )
        single = make_archive(self.items[:1], self.key, seed=7022)
        self.assertEqual(
            encode_hdsc_proof_bundle_archive(archive),
            encode_hdsc_proof_bundle_archive(archive),
        )
        self.assertNotEqual(
            encode_hdsc_proof_bundle_archive(archive),
            encode_hdsc_proof_bundle_archive(other),
        )
        self.assertNotEqual(
            encode_hdsc_proof_bundle_archive(archive),
            encode_hdsc_proof_bundle_archive(single),
        )
    ...
    def test_z_zero_is_structurally_legal_and_round_trips(self):
        ...
        restored = decode_hdsc_proof_bundle_archive(blob)
        self.assertEqual(restored, archive)
        self.assertEqual(encode_hdsc_proof_bundle_archive(restored), blob)
    ...
    def test_mismatched_outer_signature_still_encodes_and_decodes(self):
        ...
        foreign = make_archive(
            (self.items[2], self.items[0]), self.key, seed=7030
        )
        mismatched = HDSCProofBundleArchive(
            self.items[:2], foreign.signature
        )
        blob = encode_hdsc_proof_bundle_archive(mismatched)
        restored = decode_hdsc_proof_bundle_archive(blob)
        self.assertEqual(restored, mismatched)
        self.assertEqual(encode_hdsc_proof_bundle_archive(restored), blob)
        self.assertFalse(
            verify_hdsc_proof_bundle_archive(restored, self.key)
        )
    ...
    def test_decoded_under_other_key_is_false(self):
        ...
        self.assertFalse(
            verify_hdsc_proof_bundle_archive(restored, make_other_key())
        )
...
class EncodeArchiveErrorTest(unittest.TestCase):
    ...
    def test_non_archive_type_error(self):
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                encode_hdsc_proof_bundle_archive(bad)
    ...
    def test_non_tuple_items_type_error(self):
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                encode_hdsc_proof_bundle_archive(
                    HDSCProofBundleArchive(bad, self.signature)
                )
    ...
    def test_empty_archive_value_error(self):
        with self.assertRaises(ValueError):
            encode_hdsc_proof_bundle_archive(
                HDSCProofBundleArchive((), self.signature)
            )
    ...
    def test_illegal_nested_bundle_value_error(self):
        ...
        with self.assertRaises(ValueError):
            encode_hdsc_proof_bundle_archive(
                HDSCProofBundleArchive((bad_bundle,), self.signature)
            )
        with self.assertRaises(ValueError):
            encode_hdsc_proof_bundle_archive(
                HDSCProofBundleArchive(
    ...
    def test_nested_bundle_field_type_error(self):
        bad_bundle = HDSCProofBundle("not-a-proof", self.signature)
        with self.assertRaises(TypeError):
            encode_hdsc_proof_bundle_archive(
                HDSCProofBundleArchive((bad_bundle,), self.signature)
            )
...
class DecodeArchiveErrorTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.items = build_bundles(self.key)
        self.archive = make_archive(self.items[:2], self.key, seed=7100)
        self.blob = encode_hdsc_proof_bundle_archive(self.archive)
        self.bundle_blob = encode_hdsc_proof_bundle(self.items[0])
    ...
    def test_non_bytes_type_error(self):
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                decode_hdsc_proof_bundle_archive(bad)
    ...
    def test_bad_tag(self):
        ...
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(b"")
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(self.blob[5:])
    ...
    def test_truncation(self):
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(TAG)
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(self.blob[:-1])
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(self.blob[:len(TAG) + 4 + 2])
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(self.blob[:self.sig_start + 2])

    def test_trailing_bytes(self):
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(self.blob + b"\x00")
    ...
    def test_short_garbage(self):
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(TAG + u32(1) + b"x")
    ...
    def test_zero_R_is_illegal(self):
        ...
        with self.assertRaises(ValueError):
            decode_hdsc_proof_bundle_archive(bytes(bad))
