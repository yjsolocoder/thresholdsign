"""Tests for the canonical SealHistoryExtensionBundle transport encoding:
...
def build_wire(bundle: SealHistoryExtensionBundle) -> bytes:
    ...
    out += frame(encode_history_extension(bundle.proof))
    out += signature_frame(bundle.old_signature)
    out += signature_frame(bundle.new_signature)
    return bytes(out)


def sealed_bundle(key, history, old_total, *, seed=800):
...
class BundleDataTest(unittest.TestCase):
    def test_field_order_and_positional_construction(self):
        ...
        bundle = SealHistoryExtensionBundle(proof, old_sig, new_sig)
    ...
    def test_frozen_value_equality_and_hash(self):
        ...
        new_sig = AggregateSignature(R=9, z=11, signer_ids=(2, 4))
        bundle = SealHistoryExtensionBundle(proof, old_sig, new_sig)
        same = SealHistoryExtensionBundle(proof, old_sig, new_sig)
        self.assertEqual(bundle, same)
        self.assertEqual(hash(bundle), hash(same))
        self.assertNotEqual(
            bundle,
            SealHistoryExtensionBundle(
                SealHistoryExtension(2, proof.leaves), old_sig, new_sig
            ),
        )
        self.assertNotEqual(
            bundle,
            SealHistoryExtensionBundle(proof, new_sig, old_sig),
        )
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_round_trip_for_every_size_and_split(self):
        ...
        for n in range(2, 10):
            ...
            for old_total in range(1, n):
                ...
                self.assertIsInstance(decoded.proof, SealHistoryExtension)
                self.assertIsInstance(decoded.proof.leaves, tuple)
    ...
    def test_prefix_is_tag_length_and_full_proof_encoding(self):
        history = history_of_size(self.key, 8)
        bundle = sealed_bundle(self.key, history, 4, seed=4000)
        wire = encode_history_extension_bundle(bundle)
        self.assertTrue(wire.startswith(BUNDLE_TAG))
        offset = len(BUNDLE_TAG)
        proof_bytes = encode_history_extension(bundle.proof)
        ...
        self.assertTrue(proof_bytes.startswith(EXTENSION_TAG))

    def test_suffix_is_old_then_new_signature_frame(self):
        history = history_of_size(self.key, 8)
        bundle = sealed_bundle(self.key, history, 4, seed=4100)
        wire = encode_history_extension_bundle(bundle)
        proof_bytes = encode_history_extension(bundle.proof)
    ...
    def test_zero_z_encodes_as_single_body_byte(self):
        history = history_of_size(self.key, 8)
        bundle = sealed_bundle(self.key, history, 3, seed=4200)
        bundle = SealHistoryExtensionBundle(
            bundle.proof,
            AggregateSignature(
                R=bundle.old_signature.R,
                z=0,
                signer_ids=bundle.old_signature.signer_ids,
            ),
        ...
        offset += len(varint(bundle.old_signature.R))
    ...
    def test_encoding_is_unique_and_stateless(self):
        ...
        bundle = sealed_bundle(self.key, history, 2, seed=4300)
        self.assertEqual(
            encode_history_extension_bundle(bundle),
            encode_history_extension_bundle(bundle),
        )

    def test_encoding_carries_no_seal_material(self):
        history = history_of_size(self.key, 6)
        bundle = sealed_bundle(self.key, history, 2, seed=4350)
        wire = encode_history_extension_bundle(bundle)
        # Only leaf digests appear; no encoded seal is present.
        from thresholdsign import encode_seal
...
class StructureOnlyTest(unittest.TestCase):
    ...
    def test_encoding_does_not_check_the_signatures_against_the_roots(self):
        ...
        bundle = sealed_bundle(key, history, 2, seed=4400)
        self.assertTrue(encode_history_extension_bundle(bundle))
        self.assertFalse(
            verify_history_extension_bundle(bundle, make_other_key())
        )
...
class EncodeValidationTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.history = history_of_size(self.key, 5)
        self.bundle = sealed_bundle(self.key, self.history, 2, seed=4500)
    ...
    def test_non_proof_field_type_error(self):
        ...
        with self.assertRaises(TypeError):
            encode_history_extension_bundle(
                SealHistoryExtensionBundle(
                    None, self.bundle.old_signature, self.bundle.new_signature
                )
            )
    ...
    def test_non_signature_field_type_error(self):
        for position in ("old", "new"):
            fields = {
                ...
                "old_signature": self.bundle.old_signature,
                "new_signature": self.bundle.new_signature,
            }
            for bad in ("no", None):
                fields[position + "_signature"] = bad
    ...
    def test_signature_field_type_errors(self):
        old_sig = self.bundle.old_signature
        for bad in (
            ...
            for fields in (
                (bad, self.bundle.new_signature),
                (self.bundle.old_signature, bad),
            ):
                bundle = SealHistoryExtensionBundle(self.bundle.proof, *fields)
                with self.assertRaises(TypeError, msg=repr(bad)):
                    encode_history_extension_bundle(bundle)

    def test_bad_nested_proof_structure_value_error(self):
        leaves = self.bundle.proof.leaves
        for bad_proof in (
            SealHistoryExtension(0, leaves),
            SealHistoryExtension(len(leaves), leaves),
            SealHistoryExtension(1, ()),
            SealHistoryExtension(2, (b"a" * 31, b"b" * 32, b"c" * 32)),
        ):
...
class DecodeValidationTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.history = history_of_size(self.key, 5)
        self.bundle = sealed_bundle(self.key, self.history, 2, seed=4600)
        self.wire = encode_history_extension_bundle(self.bundle)
        self.proof_bytes = encode_history_extension(self.bundle.proof)
    ...
    def test_zero_R_rejected_in_either_frame(self):
        for sig, offset in (
            (self.bundle.old_signature, self.suffix_offset),
            (
                self.bundle.new_signature,
...
class VerifyBundleTest(unittest.TestCase):
    ...
    def test_honest_bundles_verify_for_every_size_and_split(self):
        ...
        for n in range(2, 9):
            history = history_of_size(self.key, n)
            for old_total in range(1, n):
                bundle = sealed_bundle(self.key, history, old_total, seed=seed)
                seed += 10
                self.assertTrue(
    ...
    def test_wrong_key_returns_false(self):
        bundle = sealed_bundle(self.key, self.history, 3, seed=7100)
        self.assertFalse(
            verify_history_extension_bundle(bundle, self.other)
        )

    def test_swapped_signatures_return_false(self):
        bundle = sealed_bundle(self.key, self.history, 3, seed=7150)
    ...
    def test_bad_signature_returns_false(self):
        bundle = sealed_bundle(self.key, self.history, 3, seed=7160)
        bad_new = AggregateSignature(
            R=bundle.new_signature.R,
            z=(bundle.new_signature.z + 1) % FIELD_PRIME,
            signer_ids=bundle.new_signature.signer_ids,
        )
        self.assertFalse(
            verify_history_extension_bundle(
                SealHistoryExtensionBundle(
                    bundle.proof, bundle.old_signature, bad_new
                ),
                self.key,
            )
        )
    ...
    def test_signature_over_another_statement_returns_false(self):
        # The new statement binds (n, root), so a signature over a
        # different history's statement must not carry over.
        bundle = sealed_bundle(self.key, self.history, 3, seed=7300)
        ...
        self.assertFalse(
            verify_history_extension_bundle(
                SealHistoryExtensionBundle(
                    bundle.proof, bundle.old_signature, other_sig
                ),
                self.key,
            )
        )
    ...
    def test_tampered_leaf_returns_false(self):
        bundle = sealed_bundle(self.key, self.history, 3, seed=7450)
        flipped = bytearray(bundle.proof.leaves[1])
        flipped[0] ^= 1
        proof = SealHistoryExtension(
            old_total=bundle.proof.old_total,
            leaves=(
                bundle.proof.leaves[:1]
                + (bytes(flipped),)
                + bundle.proof.leaves[2:]
            ),
        )
    ...
    def test_changed_old_total_returns_false(self):
        bundle = sealed_bundle(self.key, self.history, 3, seed=7460)
        proof = SealHistoryExtension(2, bundle.proof.leaves)
        tampered = SealHistoryExtensionBundle(
            proof, bundle.old_signature, bundle.new_signature
        )
        self.assertFalse(
    ...
    def test_decoded_bundle_verifies_identically(self):
        bundle = sealed_bundle(self.key, self.history, 4, seed=7500)
        decoded = decode_history_extension_bundle(
            encode_history_extension_bundle(bundle)
        )
        self.assertEqual(
    ...
    def test_bad_nested_structure_value_error(self):
        bad_proof = SealHistoryExtension(0, (b"a" * 32, b"b" * 32))
        signature = AggregateSignature(R=5, z=7, signer_ids=(1, 3))
        bundle = SealHistoryExtensionBundle(bad_proof, signature, signature)
        with self.assertRaises(ValueError):
            verify_history_extension_bundle(bundle, self.key)
