...
def make_records(key, n):
    return tuple(make_record(key, b"msg-%d" % i, seed=100 + i) for i in range(n))


def sealed_bundle(key, records, index, *, seed=800):
    """Return a signed AuditProofBundle for the given leaf index."""
...
class BundleDataTest(unittest.TestCase):
    def test_field_order_and_positional_construction(self):
        ...
        proof = AuditProof(0, 1, b"m", SigningAudit(b"a"), ())
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_round_trip_for_every_size_and_index(self):
        for n in range(1, 10):
            ...
            for index in range(n):
                bundle = sealed_bundle(self.key, records, index, seed=2000 + n)
    ...
    def test_encoding_matches_independent_builder(self):
        for n in range(1, 10):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(self.key, records, index, seed=3000 + n)
                self.assertEqual(
    ...
    def test_prefix_is_tag_length_and_full_proof_encoding(self):
        bundle = sealed_bundle(self.key, self.records, 4, seed=4000)
        wire = encode_audit_proof_bundle(bundle)
    ...
    def test_suffix_is_r_z_count_and_ids(self):
        bundle = sealed_bundle(self.key, self.records, 4, seed=4100)
        wire = encode_audit_proof_bundle(bundle)
        proof_bytes = encode_audit_proof(bundle.proof)
    ...
    def test_decoded_bundle_still_verifies(self):
        for n in range(1, 10):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(self.key, records, index, seed=5000 + n)
    ...
    def test_encoding_is_unique_and_stateless(self):
        bundle = sealed_bundle(self.key, self.records, 5, seed=4300)
        self.assertEqual(
            encode_audit_proof_bundle(bundle),
            encode_audit_proof_bundle(bundle),
        )
...
class StructureOnlyTest(unittest.TestCase):
    def _synthetic_proof(self):
        ...
        return AuditProof(0, 2, b"m", SigningAudit(b"a"), (b"s" * 32,))
    ...
    def test_encoding_does_not_check_the_signature_against_the_root(self):
        ...
        key = make_key()
        records = make_records(key, 2)
        bundle = sealed_bundle(key, records, 0, seed=4400)
        self.assertTrue(encode_audit_proof_bundle(bundle))
        self.assertFalse(
            verify_audit_proof_bundle(bundle, make_other_key())
        )
...
class EncodeValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.bundle = sealed_bundle(self.key, self.records, 2, seed=4500)
    ...
    def test_bad_nested_proof_structure_value_error(self):
        bad_proof = AuditProof(0, 0, b"m", SigningAudit(b"a"), ())
...
class DecodeValidationTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.records = make_records(self.key, 5)
        self.bundle = sealed_bundle(self.key, self.records, 2, seed=4600)
...
class VerifyBundleTest(unittest.TestCase):
    ...
    def test_honest_bundles_verify_for_every_size_and_index(self):
        for n in range(1, 9):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(self.key, records, index, seed=6000 + n)
                self.assertTrue(
    ...
    def test_result_equals_check_proof(self):
        for n in range(1, 9):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(self.key, records, index, seed=7000 + n)
                self.assertEqual(
                    verify_audit_proof_bundle(bundle, self.key),
                    check_proof(
    ...
    def test_wrong_key_returns_false(self):
        bundle = sealed_bundle(self.key, self.records, 3, seed=7100)
        self.assertFalse(
            verify_audit_proof_bundle(bundle, self.other)
        )

    def test_tampered_signature_returns_false(self):
        bundle = sealed_bundle(self.key, self.records, 3, seed=7200)
    ...
    def test_tampered_sibling_returns_false(self):
        bundle = sealed_bundle(self.key, self.records, 2, seed=7450)
    ...
    def test_decoded_bundle_verifies_identically(self):
        bundle = sealed_bundle(self.key, self.records, 4, seed=7500)
        decoded = decode_audit_proof_bundle(
            encode_audit_proof_bundle(bundle)
        )
        self.assertEqual(
