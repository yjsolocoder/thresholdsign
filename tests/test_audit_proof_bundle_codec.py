"""Tests for the canonical AuditProofBundle transport encoding:
encode_audit_proof_bundle / decode_audit_proof_bundle /
...
def build_wire(bundle: AuditProofBundle) -> bytes:
    """Independently build the bundle wire format straight from the spec."""
    proof_bytes = encode_audit_proof(bundle.proof)
...
def make_records(key, n):
    return tuple(
        make_record(key, f"message-{i}".encode(), seed=100 + i)
        for i in range(n)
    )


def sealed_bundle(records, index, key, *, seed=800):
    """Return a signed AuditProofBundle for the given index."""
    message, proof = make_proof(records, index)
    signature = sign_message(key, message, seed=seed + index)
    return AuditProofBundle(proof, signature)


class BundleDataTest(unittest.TestCase):
    def test_field_order_and_positional_construction(self):
        self.assertEqual(
            [field.name for field in dataclasses.fields(AuditProofBundle)],
            ["proof", "signature"],
        )
        proof = AuditProof(0, 1, b"m", SigningAudit(payload=b"a"), ())
        signature = AggregateSignature(R=5, z=7, signer_ids=(1, 3))
        bundle = AuditProofBundle(proof, signature)
        self.assertIs(bundle.proof, proof)
        self.assertIs(bundle.signature, signature)
        self.assertEqual(
            AuditProofBundle(proof=proof, signature=signature),
            bundle,
        )
    ...
    def test_frozen_value_equality_and_hash(self):
        ...
        signature = AggregateSignature(R=5, z=7, signer_ids=(1, 3))
        bundle = AuditProofBundle(proof, signature)
        same = AuditProofBundle(proof, signature)
    ...
    def test_construction_does_not_validate(self):
        ...
        AuditProofBundle("not-a-proof", "not-a-signature")
        AuditProofBundle(None, None)


class RoundTripTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.records = make_records(self.key, 9)

    def test_round_trip_for_every_size_and_index(self):
        for n in range(1, 10):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(records, index, self.key, seed=2000 + n)
                wire = encode_audit_proof_bundle(bundle)
                decoded = decode_audit_proof_bundle(wire)
                self.assertEqual(decoded, bundle, msg=f"n={n} index={index}")
                self.assertEqual(
                    encode_audit_proof_bundle(decoded), wire
                )
    ...
    def test_encoding_matches_independent_builder(self):
        for n in range(1, 10):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(records, index, self.key, seed=3000 + n)
                self.assertEqual(
                    encode_audit_proof_bundle(bundle),
                    build_wire(bundle),
                    msg=f"n={n} index={index}",
                )

    def test_prefix_is_tag_length_and_full_proof_encoding(self):
        bundle = sealed_bundle(self.records, 4, self.key, seed=4000)
        wire = encode_audit_proof_bundle(bundle)
        ...
        self.assertTrue(
            proof_bytes.startswith(b"thresholdsign/audit-proof/v1")
        )

    def test_suffix_is_r_z_count_and_ids(self):
        bundle = sealed_bundle(self.records, 4, self.key, seed=4100)
        wire = encode_audit_proof_bundle(bundle)
        proof_bytes = encode_audit_proof(bundle.proof)
    ...
    def test_zero_z_encodes_as_single_body_byte(self):
        bundle = sealed_bundle(self.records, 0, self.key, seed=4200)
    ...
    def test_decoded_bundle_still_verifies(self):
        for n in range(1, 10):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(records, index, self.key, seed=5000 + n)
                decoded = decode_audit_proof_bundle(
                    encode_audit_proof_bundle(bundle)
                )
                self.assertTrue(
                    verify_audit_proof_bundle(decoded, self.key),
                    msg=f"n={n} index={index}",
                )

    def test_encoding_is_unique_and_stateless(self):
        bundle = sealed_bundle(self.records, 5, self.key, seed=4300)
        self.assertEqual(
            encode_audit_proof_bundle(bundle),
            encode_audit_proof_bundle(bundle),
        )
...
class StructureOnlyTest(unittest.TestCase):
    def _synthetic_proof(self):
        # A structurally legal synthetic receipt that never went through a
        # real key round-trip untouched; n=2 gives exactly one sibling.
        return AuditProof(
            0, 2, b"m", SigningAudit(payload=b"opaque"), (b"s" * 32,)
        )

    def test_decode_does_not_parse_receipt_or_check_signature(self):
    ...
    def test_encoding_does_not_check_the_signature_against_the_root(self):
        # A proof of one tree paired with a signature over another tree's
        # root encodes fine — structure alone is checked; verify is False.
        key = make_key()
        records = make_records(key, 4)
        bundle_a = sealed_bundle(records[:3], 0, key, seed=4400)
        bundle_b = sealed_bundle(records, 1, key, seed=4500)
        mixed = AuditProofBundle(bundle_a.proof, bundle_b.signature)
        self.assertTrue(encode_audit_proof_bundle(mixed))
        self.assertFalse(verify_audit_proof_bundle(mixed, key))
...
class EncodeValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.bundle = sealed_bundle(self.records, 2, self.key, seed=4600)

    def test_non_bundle_type_error(self):
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(("proof", "signature"))
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(None)

    def test_non_proof_field_type_error(self):
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(
                AuditProofBundle("not-a-proof", self.bundle.signature)
            )
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(
                AuditProofBundle(None, self.bundle.signature)
            )

    def test_non_signature_field_type_error(self):
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(
                AuditProofBundle(self.bundle.proof, "no")
            )
        with self.assertRaises(TypeError):
            encode_audit_proof_bundle(
                AuditProofBundle(self.bundle.proof, None)
            )
    ...
    def test_signature_field_type_errors(self):
        ...
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                encode_audit_proof_bundle(
                    AuditProofBundle(self.bundle.proof, bad)
                )

    def test_bad_nested_proof_structure_value_error(self):
        bad_proof = AuditProof(
            i=0, n=0, m=b"m", a=SigningAudit(payload=b"a"), p=()
        )
        with self.assertRaises(ValueError):
            encode_audit_proof_bundle(
                AuditProofBundle(bad_proof, self.bundle.signature)
            )
...
class DecodeValidationTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.records = make_records(self.key, 5)
        self.bundle = sealed_bundle(self.records, 2, self.key, seed=4700)
        self.wire = encode_audit_proof_bundle(self.bundle)
        self.proof_bytes = encode_audit_proof(self.bundle.proof)

    def test_non_bytes_type_error(self):
        with self.assertRaises(TypeError):
            decode_audit_proof_bundle("not bytes")
        with self.assertRaises(TypeError):
            decode_audit_proof_bundle(bytearray(self.wire))

    def test_bad_tag(self):
        rest = self.wire[len(BUNDLE_TAG):]
        with self.assertRaises(ValueError):
            decode_audit_proof_bundle(b"ts/apb/v2" + rest)
        with self.assertRaises(ValueError):
            decode_audit_proof_bundle(b"x" + self.wire[1:])

    def test_truncation(self):
        for cut in range(len(BUNDLE_TAG), len(self.wire)):
            with self.assertRaises(ValueError, msg=f"cut={cut}"):
                decode_audit_proof_bundle(self.wire[:cut])

    def test_trailing_bytes(self):
        with self.assertRaises(ValueError):
            decode_audit_proof_bundle(self.wire + b"\x00")
    ...
    def test_declared_proof_frame_length_mismatch(self):
        ...
        with self.assertRaises(ValueError):
            decode_audit_proof_bundle(bytes(bad))
    ...
    def test_non_positive_and_unsorted_signer_ids_rejected(self):
        ...
        for label, bodies in cases.items():
            ...
            with self.assertRaises(ValueError, msg=label):
                decode_audit_proof_bundle(bytes(out))


class VerifyBundleTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.other = make_key(seed_offset=10)
    ...
    def test_honest_bundles_verify_for_every_size_and_index(self):
        for n in range(1, 9):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(records, index, self.key, seed=6000 + n)
                self.assertTrue(
                    verify_audit_proof_bundle(bundle, self.key),
                    msg=f"n={n} index={index}",
                )

    def test_result_equals_check_proof(self):
        for n in range(1, 9):
            records = self.records[:n]
            for index in range(n):
                bundle = sealed_bundle(records, index, self.key, seed=7000 + n)
                self.assertEqual(
                    verify_audit_proof_bundle(bundle, self.key),
                    check_proof(
    ...
    def test_wrong_key_returns_false(self):
        bundle = sealed_bundle(self.records, 3, self.key, seed=7100)
        self.assertFalse(
            verify_audit_proof_bundle(bundle, self.other)
        )

    def test_tampered_signature_returns_false(self):
        bundle = sealed_bundle(self.records, 3, self.key, seed=7200)
        # A valid R of a different real round is still a subgroup element,
        # so the check runs and returns False rather than raising.
        other_message, _ = make_proof(self.records[:3], 1)
        other_signature = sign_message(self.key, other_message, seed=7210)
        tampered = AuditProofBundle(
            bundle.proof,
            AggregateSignature(
                R=other_signature.R,
        ...
        self.assertFalse(
            verify_audit_proof_bundle(tampered, self.key)
        )

    def test_signature_over_another_statement_returns_false(self):
        # The root binds (n, tree), so proofs of two different trees
        # are not interchangeable even under the same key.
        bundle_a = sealed_bundle(self.records[:4], 0, self.key, seed=7300)
        bundle_b = sealed_bundle(self.records[:6], 1, self.key, seed=7400)
        mixed = AuditProofBundle(bundle_a.proof, bundle_b.signature)
        self.assertFalse(
            verify_audit_proof_bundle(mixed, self.key)
        )

    def test_tampered_sibling_returns_false(self):
        bundle = sealed_bundle(self.records, 2, self.key, seed=7450)
        if not bundle.proof.p:
        ...
        proof = AuditProof(
            ...
            m=bundle.proof.m,
            a=bundle.proof.a,
            p=(b"x" * 32,) + bundle.proof.p[1:],
        )
        self.assertFalse(
            verify_audit_proof_bundle(
                AuditProofBundle(proof, bundle.signature), self.key
            )
        )

    def test_decoded_bundle_verifies_identically(self):
        bundle = sealed_bundle(self.records, 4, self.key, seed=7500)
        decoded = decode_audit_proof_bundle(
            encode_audit_proof_bundle(bundle)
        )
        self.assertEqual(
    ...
    def test_non_bundle_type_error(self):
        ...
        for bad in ((proof, signature), "bundle", None, proof):
            with self.assertRaises(TypeError, msg=repr(bad)):
                verify_audit_proof_bundle(bad, self.key)
    ...
    def test_bad_nested_structure_value_error(self):
        ...
        signature = AggregateSignature(R=5, z=7, signer_ids=(1, 3))
        bundle = AuditProofBundle(bad_proof, signature)
        with self.assertRaises(ValueError):
            verify_audit_proof_bundle(bundle, self.key)
