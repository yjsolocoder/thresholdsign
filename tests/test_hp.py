...
def bundles_of_size(n, key, seed_base=61000):
    """Build ``n`` distinct structurally legal, key-verifying HBPB bundles."""
    combos = ((0, 2, 5), (1, 3), (0, 1, 2, 3, 4, 5, 6, 7), (4,))
    return tuple(
        signed_hbpb(
            hbapb_archive_of_size(8, key, seed_base=seed_base + position * 2000),
...
def archive_of_size(n, key, seed_base=61000):
...
def signed_proof(archive, indices, key, *, seed=62000):
...
class HPDataTest(unittest.TestCase):
    ...
    def test_frozen_value_equality_and_hash(self):
        key = make_key()
        bundles = bundles_of_size(2, key, seed_base=61100)
    ...
    def test_construction_does_not_validate(self):
        key = make_key()
        bundles = bundles_of_size(1, key, seed_base=61150)
        HP((9,), 1, bundles, ())
...
class MakeHPTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.archive = archive_of_size(8, self.key, seed_base=61200)
    ...
    def test_illegal_nested_bundle_errors(self):
        ...
        with self.assertRaises(TypeError):
            make_hp(archive, (0,))
        bad_shape_bundle = HBPB(
            HBP(indices=(), total=0, bundles=(), siblings=()),
            DUMMY_OUTER_SIGNATURE,
        )
        archive = HBPBArchive(
            (bad_shape_bundle,) + self.archive.items[1:], self.archive.signature
        )
        with self.assertRaises(ValueError):
            make_hp(archive, (0,))
...
class CheckHPTest(unittest.TestCase):
    def setUp(self):
        self.key = make_key()
        self.archive = archive_of_size(8, self.key, seed_base=61300)
    ...
    def test_one_signature_covers_every_subset(self):
        ...
        signature = sign_message(self.key, message, seed=61350)
    ...
    def test_wrong_signature_and_wrong_key_return_false(self):
        ...
        other_signature = sign_message(self.key, other_message, seed=61370)
    ...
    def test_unverifying_nested_bundle_returns_false(self):
        ...
        foreign = signed_hbpb(
            hbapb_archive_of_size(8, other_key, seed_base=61380),
            (0,),
            other_key,
            seed=61390,
        )
    ...
    def test_bad_bundle_does_not_mask_illegal_signature_or_key(self):
        ...
        other_key = make_other_key()
        foreign = signed_hbpb(
            hbapb_archive_of_size(8, other_key, seed_base=61400),
            (0,),
            other_key,
            seed=61401,
        )
