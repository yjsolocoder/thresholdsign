...
class BundleDataTest(unittest.TestCase):
    def test_field_order_and_positional_construction(self):
        self.assertEqual(
            [field.name for field in dataclasses.fields(AuditExtensionProofBundle)],
...
class RoundTripTest(unittest.TestCase):
    ...
    def test_zero_z_encodes_as_single_body_byte(self):
        bundle = sealed_bundle(self.key, self.records, 3, seed=4200)
...
class StructureOnlyTest(unittest.TestCase):
    def test_decode_does_not_parse_leaves_or_check_signatures(self):
        ...
        proof = AuditExtensionProof(1, (b"a" * 32, b"b" * 32, b"c" * 32))
    ...
    def test_signature_field_type_errors(self):
        ...
        for bad in (
            ...
            for fields in (
                ...
                bundle = AuditExtensionProofBundle(self.bundle.proof, *fields)
                with self.assertRaises(TypeError, msg=repr(bad)):
                    encode_audit_extension_proof_bundle(bundle)

    def test_bad_nested_proof_structure_value_error(self):
        for bad_proof in (
            AuditExtensionProof(0, (b"a" * 32, b"b" * 32)),
            AuditExtensionProof(2, (b"a" * 32, b"b" * 32)),
            AuditExtensionProof(1, ()),
            AuditExtensionProof(1, (b"a" * 31, b"b" * 32)),
        ):
...
class VerifyBundleTest(unittest.TestCase):
    ...
    def test_swapped_signatures_return_false(self):
        bundle = sealed_bundle(self.key, self.records, 3, seed=7150)
    ...
    def test_signature_over_another_statement_returns_false(self):
        # The statements bind (n, root), so signatures of two different
        # record sequences are not interchangeable even under the same key.
        bundle_a = sealed_bundle(self.key, self.records[:6], 3, seed=7300)
        bundle_b = sealed_bundle(self.key, self.records[:5], 2, seed=7400)
    ...
    def test_tampered_leaf_returns_false(self):
        bundle = sealed_bundle(self.key, self.records, 3, seed=7450)
    ...
    def test_non_bundle_type_error(self):
        ...
        for bad in ((proof, signature, signature), "bundle", None, proof):
            with self.assertRaises(TypeError, msg=repr(bad)):
