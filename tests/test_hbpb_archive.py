...
def make_signed_bundle(n, indices, key, *, seed_base, seed):
...
def build_bundles(key):
    """Three distinct structurally legal, key-verifying HBPB bundles."""
    return (
        make_signed_bundle(4, (0, 2), key, seed_base=50100, seed=50200),
        make_signed_bundle(5, (1, 3), key, seed_base=50110, seed=50210),
        make_signed_bundle(7, (3, 6), key, seed_base=50120, seed=50220),
    )
...
def make_archive(items, key, *, signer_ids=(1, 3), seed=50300):
...
class ArchiveShapeTest(unittest.TestCase):
    ...
    def test_fields_have_no_defaults(self):
        with self.assertRaises(TypeError):
            HBPBArchive()
        with self.assertRaises(TypeError):
            HBPBArchive(build_bundles(make_key())[:1])
...
class ArchiveMessageTest(unittest.TestCase):
    ...
    def test_frames_are_existing_hbpb_transport_encodings(self):
        ...
        for item in self.items_a:
            self.assertTrue(
                encode_hbpb(item).startswith(b"ts/hbpb/v1")
            )
    ...
    def test_non_bundle_element_type_error(self):
        ...
        # An HBP proof is not an HBPB bundle.
        with self.assertRaises(TypeError):
            hbpba_message(
    ...
    def test_count_at_two_pow_64_raises_value_error(self):
        ...
        huge = HugeTuple(self.items_a)
        with self.assertRaises(ValueError):
            hbpba_message(huge, self.key.public_key)
        with self.assertRaises(ValueError):
            encode_hbpba(
                HBPBArchive(huge, self.items_a[0].signature)
            )

    def test_illegal_nested_bundle_value_error(self):
        bad_bundle = HBPB("not-a-proof", DUMMY_SIGNATURE)
        # A non-HBP proof field is a TypeError from encode_hbpb; use a
        # structurally illegal HBP proof for the ValueError path.
        ...
        with self.assertRaises(ValueError):
            hbpba_message((bad_value_bundle,), self.key.public_key)
        with self.assertRaises(ValueError):
            hbpba_message(
...
class VerifyArchiveShapeAndErrorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.archive = make_archive(self.items, self.key, seed=50400)
    ...
    def test_count_at_two_pow_64_raises_value_error(self):
        ...
        with self.assertRaises(ValueError):
            verify_hbpba(
                HBPBArchive(
                    HugeTuple(self.items), self.archive.signature
                ),
                self.key,
            )
...
class VerifyArchiveVerdictTest(unittest.TestCase):
    def setUp(self):
        ...
        self.items = build_bundles(self.key)
        self.archive = make_archive(self.items, self.key, seed=50500)
    ...
    def test_single_item_archive_is_true(self):
        archive = make_archive(self.items[:1], self.key, seed=50510)
    ...
    def test_tampered_inner_root_signature_is_false(self):
        ...
        archive = make_archive(
            (bad_bundle,) + self.items[1:], self.key, seed=50530
        )
        self.assertFalse(verify_hbpba(archive, self.key))
    ...
    def test_outer_signature_over_another_item_set_is_false(self):
        foreign = self._outer_signature_over(
            (self.items[2], self.items[0]), seed=50540
        )
        self.assertFalse(
            verify_hbpba(
    ...
    def test_deleting_an_item_is_false(self):
        ...
        foreign = self._outer_signature_over(self.items[:2], seed=50550)
        self.assertFalse(
            verify_hbpba(
    ...
    def test_inserting_an_item_is_false(self):
        extra = make_signed_bundle(
            6, (0, 2, 4), self.key, seed_base=50560, seed=50561
        )
        foreign = self._outer_signature_over(
            self.items + (extra,), seed=50562
        )
        self.assertFalse(
            verify_hbpba(
                HBPBArchive(self.items, foreign),
    ...
    def test_reordering_items_is_false(self):
        foreign = self._outer_signature_over(
            tuple(reversed(self.items)), seed=50570
        )
        self.assertFalse(
            verify_hbpba(
                HBPBArchive(self.items, foreign),
    ...
    def test_replacing_an_item_is_false(self):
        replacement = make_signed_bundle(
            3, (0, 1, 2), self.key, seed_base=50580, seed=50581
        )
    ...
    def test_bad_bundle_does_not_mask_illegal_outer_signature(self):
        ...
        bad_bundle = make_signed_bundle(
            4, (0, 2), self.other, seed_base=50590, seed=50591
        )
    ...
    def test_bad_bundle_does_not_mask_illegal_key(self):
        bad_bundle = make_signed_bundle(
            4, (0, 2), self.other, seed_base=50600, seed=50601
        )
        archive = make_archive(
            (bad_bundle, self.items[0]), self.key, seed=50602
        )
